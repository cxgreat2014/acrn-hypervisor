...
def run_cmd_list(cmd_list):
    for i, cmd in enumerate(cmd_list):
        ...
        if ret != 0:
            print("cmd(%s) run in dir(%s) failed and exit" %
                  (cmd["cmd"], cmd["dir"]))
            exit(-1)
    return
...
def build_acrn():
    ...
    if load_dict["sync_acrn_code"] == "true":
        ...
        add_cmd_list(cmd_list, "git clone %s" %
                     load_dict["acrn_repo"], cur_dir)
        add_cmd_list(
            cmd_list,
            "git checkout -b mybranch %s" % load_dict["release_version"],
            hv_dir,
        )
    ...
    if load_dict["acrn_patch"]["patch_need"] == "true":
        ...
        for patch in load_dict["acrn_patch"]["patch_list"]:
            ...
            add_cmd_list(cmd_list, "git am %s/%s" %
                         (patch_path, patch), hv_dir)
            ...
            for j in board:
                if board[j] == "true":
                    ...
                    if load_dict["build_cmd"]["build_method"]["use_xml"] == "true":
                        make_cmd = (
                            "make all BOARD_FILE=misc/acrn-config/xmls/board-xmls/%s.xml SCENARIO_FILE=misc/acrn-config/xmls/config-xmls/%s/%s.xml RELEASE=%s"
                            % (j, j, i, release)
                        )
                    else:
                        make_cmd = "make all BOARD=%s SCENARIO=%s RELEASE=%s" % (
                            j,
                            i,
                            release,
                        )
    ...
    for i in range(len(make_cmd_list)):
        ...
        add_cmd_list(
            cmd_list,
            "cp %s acrn_release_img/%s"
            % (load_dictdeb["acrn.bin"]["source"], bin_name),
            cur_dir,
        )
        add_cmd_list(
            cmd_list,
            "cp %s acrn_release_img/%s"
            % (load_dictdeb["acrn.32.out"]["source"], out_name),
            cur_dir,
        )
        ...
        if os.path.exists(load_dictdeb["acrn.efi"]["source"]):
            add_cmd_list(
                cmd_list,
                "cp %s acrn_release_img/%s"
                % (load_dictdeb["acrn.efi"]["source"], efi_name),
                cur_dir,
            )
    run_cmd_list(cmd_list)
    return
...
def create_acrn_kernel_deb():
    ...
    add_cmd_list(cmd_list, "cp acrn-kernel/%s acrn_kernel_deb/" %
                 filename, cur_dir)
    ...
    add_cmd_list(
        cmd_list, "cp acrn-kernel.postinst acrn_kernel_deb/DEBIAN/postinst", cur_dir
    )
    add_cmd_list(cmd_list, "chmod +x acrn_kernel_deb/DEBIAN/postinst", cur_dir)
    add_cmd_list(
        cmd_list, "sed -i 's/\r//' acrn_kernel_deb/DEBIAN/postinst", cur_dir)
    add_cmd_list(cmd_list, "rm acrn_kernel_deb/%s" % filename, cur_dir)
    add_cmd_list(
        cmd_list, "dpkg -b acrn_kernel_deb acrn_kernel_deb_package.deb ", cur_dir
    )
...
def create_acrn_deb():
    ...
    for filename in glob.glob(r"acrn_release_img/acrn.*"):
        add_cmd_list(cmd_list, "cp %s acrn_release_deb/boot" %
                     filename, cur_dir)

    add_cmd_list(
        cmd_list,
        "cp acrn-hypervisor.postinst acrn_release_deb/DEBIAN/postinst",
        cur_dir,
    )
    add_cmd_list(
        cmd_list, "chmod +x acrn_release_deb/DEBIAN/postinst", cur_dir)
    add_cmd_list(
        cmd_list, "sed -i 's/\r//' acrn_release_deb/DEBIAN/postinst", cur_dir)

    add_cmd_list(
        cmd_list, "cp acrn-hypervisor.preinst acrn_release_deb/DEBIAN/preinst", cur_dir
    )
    add_cmd_list(cmd_list, "chmod +x acrn_release_deb/DEBIAN/preinst", cur_dir)
    add_cmd_list(
        cmd_list, "sed -i 's/\r//' acrn_release_deb/DEBIAN/preinst", cur_dir)

    add_cmd_list(
        cmd_list, "dpkg -b acrn_release_deb acrn_deb_package.deb ", cur_dir)
    run_cmd_list(cmd_list)
    return
