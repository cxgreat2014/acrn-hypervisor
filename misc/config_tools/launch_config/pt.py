...
def pass_through_dev(sel, pt_dev, vmid, config):
    ...
    print(
        'echo ${passthru_vpid["%s"]} > /sys/bus/pci/drivers/pci-stub/new_id' % pt_dev,
        file=config,
    )
    print(
        'echo ${passthru_bdf["%s"]} > /sys/bus/pci/devices/${passthru_bdf["%s"]}/driver/unbind'
        % (pt_dev, pt_dev),
        file=config,
    )
    print(
        'echo ${passthru_bdf["%s"]} > /sys/bus/pci/drivers/pci-stub/bind' % pt_dev,
        file=config,
    )
    print("", file=config)
...
def ipu_pt(sel, vmid, config):
    ...
    if bdf_ipu:
        ...
        print(
            '        boot_ipu_option="$boot_ipu_option"" -s {},passthru,{}/{}/{} "'.format(
                slot_ipu, bus, dev, fun
            ),
            file=config,
        )
        print("    fi", file=config)
    ...
    if bdf_ipu_i2c:
        ...
        print(
            "    # please use virtual slot for i2c to make sure that the i2c controller",
            file=config,
        )
        ...
        print(
            '        boot_ipu_option="$boot_ipu_option"" -s {},passthru,{}/{}/{} "'.format(
                slot_ipu_i2c, bus_i2c, dev_i2c, fun_i2c
            ),
            file=config,
        )
        print("    fi", file=config)
    ...
    if bdf_ipu or bdf_ipu_i2c:
        ...
        print(
            '    boot_ipu_option="$boot_ipu_option"" -s {},virtio-ipu "'.format(
                launch_cfg_lib.virtual_dev_slot("virtio-ipu")
            ),
            file=config,
        )
        print("fi", file=config)
...
def cse_pt(sel, vmid, config):
    ...
    if bdf:
        ...
        print(
            '   [[ "$major_ver" == "2" && "$minor_ver" -le "2" ]]; then', file=config)
        ...
        print(
            '    boot_cse_option="$boot_cse_option"" -s {},passthru,{}/{}/{} "'.format(
                slot, bus, dev, fun
            ),
            file=config,
        )
        print("else", file=config)
        print(
            '    boot_cse_option="$boot_cse_option"" -s {},virtio-heci,{}/{}/{} "'.format(
                slot, bus, dev, fun
            ),
            file=config,
        )
...
def audio_pt(uos_type, sel, vmid, config):
    ...
    if bdf_audio:
        ...
        if bdf_codec:
            ...
            if uos_type == "ANDROID":
                print(
                    '    boot_audio_option="-s {},passthru,{}/{}/{},keep_gsi '.format(
                        slot_audio, bus, dev, fun
                    ),
                    end="",
                    file=config,
                )
                ...
                print(
                    '    boot_audio_option="-s {},passthru,{}/{}/{} '.format(
                        slot_audio, bus, dev, fun
                    ),
                    end="",
                    file=config,
            ...
            print(
                '-s {},passthru,{}/{}/{}"'.format(
                    slot_codec, bus_codec, dev_codec, fun_codec
                ),
                file=config,
            )
        else:
        ...
        print("else", file=config)
        print(
            '    boot_audio_option="-s {},virtio-audio"'.format(slot_audio), file=config
        )
        print("fi", file=config)
...
def gen_pt_head(names, dm, sel, vmid, config):
    ...
    for pt_dev in cap_pt:
        if not sel.bdf[pt_dev] or not sel.bdf[pt_dev][vmid]:
            continue
        print('["{}"]="0000:{}"'.format(
            pt_dev, sel.bdf[pt_dev][vmid]), file=config)
    if dm["gvt_args"][vmid] == "gvtd":
