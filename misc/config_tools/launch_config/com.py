...
def is_nuc_whl_linux(names, vmid):
    ...
    if launch_cfg_lib.is_linux_like(uos_type) and board_name not in (
        "apl-mrb",
        "apl-up2",
    ):
        return True
...
def tap_uos_net(names, virt_io, vmid, config):
    ...
    if launch_cfg_lib.is_linux_like(uos_type) or uos_type in ("ANDROID", "ALIOS"):
        ...
        for mount_flag in launch_cfg_lib.MOUNT_FLAG_DIC[vmid]:
            ...
            print('if [ ! -f "/data{}/{}" ]; then'.format(i,
                  rootfs_img), file=config)
...
def off_line_cpus(args, vmid, uos_type, config):
    ...
    if not pcpu_id_list:
        ...
        launch_cfg_lib.ERR_LIST[
            key
        ] = "No available cpu to offline and pass it to vm {}".format(vmid)
    ...
    print(
        "    for j in {}; do".format(" ".join([str(i) for i in pcpu_id_list])),
        file=config,
    )
    ...
    print(
        '            if [ "$online" = "1" ] && [ "$idx" != "0" ]; then', file=config)
    ...
    print("                    echo 0 > ${cpu_path}/$i/online", file=config)
    print(
        "                    online=`cat ${cpu_path}/$i/online`", file=config)
...
def run_container(board_name, uos_type, config):
    ...
    if board_name not in ("apl-mrb", "nuc") or not launch_cfg_lib.is_linux_like(
        uos_type
    ):
        return
    ...
    print(
        'config_src="/usr/share/acrn/samples/{}/runC.json"'.format(board_name),
        file=config,
    )
    ...
    print("", file=config)
    print(
        "input=$(runc list -f table | awk '{print $1}''{print $3}')", file=config)
    ...
    print(
        "echo \"'#runc exec <vmname> bash' to login the container bash\"", file=config
    )
    print("exit", file=config)
    print("}", file=config)
    print("", file=config)
...
def boot_image_type(args, vmid, config):
    ...
    if not args["vbootloader"][vmid] or (
        args["vbootloader"][vmid] and args["vbootloader"][vmid] != "vsbl"
    ):
        return
    ...
    print(
        '  boot_image_option="--vsbl /usr/share/acrn/bios/VSBL_debug.bin"', file=config
    )
    print("else", file=config)
...
def interrupt_storm(pt_sel, config):
    ...
    print(
        "#interrupt storm monitor for pass-through devices, params order:", file=config
    )
    print(
...
def tap_network(virt_io, vmid, config):
    ...
    if tap_net_num >= 1:
        ...
        print(
            "tap_exist=$(ip a | grep \"$tap\" | awk '{print $1}')", file=config)
        ...
        print(
            "br_exist=$(ip a | grep acrn-br0 | awk '{print $1}')", file=config)
...
def wa_usage(uos_type, config):
    if uos_type in ("ANDROID", "ALIOS"):
        ...
        print(
            "echo on > /sys/devices/pci0000:00/0000:00:15.0/power/control", file=config
        )
        print("", file=config)
...
def uos_launch(names, args, virt_io, vmid, config):
    ...
    if uos_type == "CLEARLINUX" and board_name in ("apl-mrb", "nuc"):
        ...
        if board_name == "apl-mrb":
            ...
            if is_mount_needed(virt_io, vmid):
                ...
                if gvt_args == "gvtd" or not gvt_args:
                    print(
                        'launch_{} {} "{}" $debug'.format(
                            launch_uos, vmid, vmid),
                        file=config,
                    )
                else:
                    print(
                        'launch_{} {} "{}" "{}" $debug'.format(
                            launch_uos, vmid, gvt_args, vmid
                        ),
                        file=config,
                    )
                print("", file=config)
            ...
            if gvt_args == "gvtd" or not gvt_args:
                ...
                print(
                    '    launch_{} {} "{}"'.format(launch_uos, vmid, gvt_args),
                    file=config,
                )
            print("fi", file=config)
            return
        ...
        if gvt_args == "gvtd" or not gvt_args:
            ...
            print('launch_{} {} "{}"'.format(
                launch_uos, vmid, gvt_args), file=config)
    else:
        print("", file=config)
        if gvt_args == "gvtd" or not gvt_args:
            print(
                'launch_{} {} "{}" $debug'.format(launch_uos, vmid, vmid), file=config
            )
        else:
            print(
                'launch_{} {} "{}" "{}" $debug'.format(
...
def launch_end(names, args, virt_io, vmid, config):
    ...
    if uos_type in ("CLEARLINUX", "ANDROID", "ALIOS") and not is_nuc_whl_linux(
        names, vmid
    ):
    ...
    if is_mount_needed(virt_io, vmid):
        ...
        for mount_flag in launch_cfg_lib.MOUNT_FLAG_DIC[vmid]:
            ...
            print('  echo "no {} data partition, exit"'.format(
                root_fs), file=config)
            print("  exit", file=config)
            print("fi", file=config)
    ...
    sos_vmid = launch_cfg_lib.get_sos_vmid()
    if (
        args["cpu_sharing"] == "SCHED_NOOP"
        or common.VM_TYPES[vmid + sos_vmid] == "POST_RT_VM"
    ):
...
def set_dm_pt(names, sel, vmid, config, dm):
    ...
    if sel.bdf["usb_xdci"][vmid] and sel.slot["usb_xdci"][vmid]:
        ...
        print(
            "   -s {},passthru,{}/{}/{}{} \\".format(
                sel.slot["usb_xdci"][vmid],
                sel.bdf["usb_xdci"][vmid][0:2],
                sel.bdf["usb_xdci"][vmid][3:5],
                sel.bdf["usb_xdci"][vmid][6:7],
                sub_attr,
            ),
            file=config,
        )
    ...
    if sel.bdf["sd_card"][vmid] and sel.slot["sd_card"][vmid]:
        print(
            "   -s {},passthru,{}/{}/{} \\".format(
                sel.slot["sd_card"][vmid],
                sel.bdf["sd_card"][vmid][0:2],
                sel.bdf["sd_card"][vmid][3:5],
                sel.bdf["sd_card"][vmid][6:7],
            ),
            file=config,
        )
    ...
    if sel.bdf["bluetooth"][vmid] and sel.slot["bluetooth"][vmid]:
        print(
            "   -s {},passthru,{}/{}/{} \\".format(
                sel.slot["bluetooth"][vmid],
                sel.bdf["bluetooth"][vmid][0:2],
                sel.bdf["bluetooth"][vmid][3:5],
                sel.bdf["bluetooth"][vmid][6:7],
            ),
            file=config,
        )
    ...
    if sel.bdf["wifi"][vmid] and sel.slot["wifi"][vmid]:
        if uos_type == "ANDROID":
            print(
                "   -s {},passthru,{}/{}/{},keep_gsi \\".format(
                    sel.slot["wifi"][vmid],
                    sel.bdf["wifi"][vmid][0:2],
                    sel.bdf["wifi"][vmid][3:5],
                    sel.bdf["wifi"][vmid][6:7],
                ),
                file=config,
            )
        else:
    ...
    if sel.bdf["ethernet"][vmid] and sel.slot["ethernet"][vmid]:
        if vmid in dm["enable_ptm"] and dm["enable_ptm"][vmid] == "y":
            print(
                "   -s {},passthru,{}/{}/{},enable_ptm \\".format(
                    sel.slot["ethernet"][vmid],
                    sel.bdf["ethernet"][vmid][0:2],
                    sel.bdf["ethernet"][vmid][3:5],
                    sel.bdf["ethernet"][vmid][6:7],
                ),
                file=config,
            )
        else:
    ...
    if sel.bdf["sata"] and sel.slot["sata"][vmid]:
        print(
            "   -s {},passthru,{}/{}/{} \\".format(
                sel.slot["sata"][vmid],
                sel.bdf["sata"][vmid][0:2],
                sel.bdf["sata"][vmid][3:5],
                sel.bdf["sata"][vmid][6:7],
            ),
            file=config,
        )
    ...
    if sel.bdf["nvme"] and sel.slot["nvme"][vmid]:
        print(
            "   -s {},passthru,{}/{}/{} \\".format(
                sel.slot["nvme"][vmid],
                sel.bdf["nvme"][vmid][0:2],
                sel.bdf["nvme"][vmid][3:5],
                sel.bdf["nvme"][vmid][6:7],
            ),
            file=config,
        )
...
def xhci_args_set(dm, vmid, config):
    ...
    if dm["xhci"][vmid]:
        print(
            "   -s {},xhci,{} \\".format(
                launch_cfg_lib.virtual_dev_slot("xhci"), dm["xhci"][vmid]
            ),
            file=config,
        )
...
def virtio_args_set(dm, virt_io, vmid, config):
    ...
    for input_val in virt_io["input"][vmid]:
        if input_val:
            print(
                "   -s {},virtio-input,{} \\".format(
                    launch_cfg_lib.virtual_dev_slot(
                        "virtio-input{}".format(input_val)),
                    input_val,
                ),
                file=config,
            )
    ...
    for mount_flag in launch_cfg_lib.MOUNT_FLAG_DIC[vmid]:
        ...
        rootfs_img = blk.split(":")[1].strip(":")
        print(
            "   -s {},virtio-blk,/data{}/{} \\".format(
                launch_cfg_lib.virtual_dev_slot(
                    "blk_mount_{}".format(i)), i, rootfs_img
            ),
            file=config,
        )
        i += 1
    ...
    for net in virt_io["network"][vmid]:
        if net:
            print(
                "   -s {},virtio-net,tap_{} \\".format(
                    launch_cfg_lib.virtual_dev_slot(
                        "virtio-net{}".format(net)), net
                ),
                file=config,
            )
...
def get_cpu_affinity_list(cpu_affinity, vmid):
    ...
    for uos_id, cpus in cpu_affinity.items():
        if vmid == uos_id:
            pcpu_id_list = [id for id in list(
                cpu_affinity[uos_id]) if id != None]
    return pcpu_id_list
...
def dm_arg_set(names, sel, virt_io, dm, vmid, config):
    ...
    print(
        "acrn-dm -A -m $mem_size -s 0:0,hostbridge -U {} \\".format(
            scenario_uuid),
        file=config,
    )
    if launch_cfg_lib.is_linux_like(uos_type) or uos_type in ("ANDROID", "ALIOS"):
    ...
    if dm["pm_channel"][vmid] and dm["pm_channel"][vmid] != None:
        ...
        if pm_key == "vuart1(tty)":
            vuart_base = launch_cfg_lib.get_vuart1_from_scenario(
                sos_vmid + vmid)
            if vuart_base == "INVALID_COM_BASE":
                err_key = "uos:id={}:poweroff_channel".format(vmid)
                launch_cfg_lib.ERR_LIST[
                    err_key
                ] = "vuart1 of VM{} in scenario file should select 'SOS_COM2_BASE'".format(
                    sos_vmid + vmid
                )
                return
            ...
            print(
                "   {} \\".format(
                    pm_vuart + launch_cfg_lib.PM_CHANNEL_DIC[pm_key]),
                file=config,
            )
        else:
            print("   {} \\".format(
                launch_cfg_lib.PM_CHANNEL_DIC[pm_key]), file=config)
    ...
    ssram_enabled = "n"
    try:
        ssram_enabled = common.get_hv_item_tag(
            common.SCENARIO_INFO_FILE, "FEATURES", "SSRAM", "SSRAM_ENABLED"
        )
    except:
        pass
    ...
    for vuart_id in dm["communication_vuarts"][vmid]:
        if not vuart_id:
            break
        print(
            "   -s {},uart,vuart_idx:{} \\".format(
                launch_cfg_lib.virtual_dev_slot(
                    "communication_vuart_{}".format(vuart_id)
                ),
                vuart_id,
            ),
            file=config,
