...
LINUX_LIKE_OS = ["CLEARLINUX", "PREEMPT-RT LINUX",
                 "YOCTO", "UBUNTU", "GENERIC LINUX"]
...
def usage(file_name):
    ...
    print(
        "--board <board_info_file> --scenario <scenario_info_file> --launch <launch_info_file> --uosid <uosid id> --out [output folder]"
    )
    ...
    print(
        "uosid :  this is the relative id for post launch vm in scenario info XML:[1..max post launch vm]"
    )
...
def get_param(args):
    ...
    args_list = args[1:]
    (optlist, args_list) = getopt.getopt(
        args_list, "", ["board=", "scenario=", "launch=", "uosid=", "out="]
    )
        ...
        if "--uosid" in args:
            if arg_k == "--uosid":
                ...
                if not vm_th.isnumeric():
                    err_dic[
                        "common error: wrong parameter"
                    ] = "--uosid should be a number"
                    return (
                        err_dic,
                        board_info_file,
                        scenario_info_file,
    ...
    if not os.path.exists(board_info_file):
        err_dic["common error: wrong parameter"] = "{} does not exist!".format(
            board_info_file
        )
        return (
            err_dic,
...
def is_config_file_match():
    ...
    (err_dic, scenario_for_board) = common.get_xml_attrib(
        common.SCENARIO_INFO_FILE, "board"
    )
    (err_dic, board_name) = common.get_xml_attrib(
        common.BOARD_INFO_FILE, "board")
    if scenario_for_board != board_name:
        err_dic[
            "scenario config"
        ] = "The board xml file does not match scenario xml file!"
        match = False

    # check if the board config match launch config
    (err_dic, launch_for_board) = common.get_xml_attrib(
        common.LAUNCH_INFO_FILE, "board"
    )
...
def get_scenario_uuid(uosid, sos_vmid):
    ...
    scenario_uuid = scenario_cfg_lib.VM_DB[common.VM_TYPES[vm_id]
                                           ]["uuid"][i_cnt]
...
def pt_devs_check(bdf_list, vpid_list, item):
    ...
    for vpid_str in vpid_list.values():
        if is_vpid_format(vpid_str):
            ...
            key = "uos:id={},passthrough_devices,{}".format(i_cnt, item)
            ERR_LIST[key] = "Unkonw the Vendor:Product ID format of {} device".format(
                item
            )
...
def mem_size_check(arg_list, item):
    ...
    for arg_str in arg_list.values():
        ...
        if mem_size_set > total_mem_mb:
            key = "uos:id={},{}".format(i_cnt, item)
            ERR_LIST[key] = "{}MB should be less than total memory {}MB".format(
                item)
        i_cnt += 1
...
def virtual_dev_slot(dev):
    ...
    for slot_num in range(base_slot, max_slot):
        if slot_num not in list(PT_SLOT.values()):
            ...
            if (slot_num == 6 and 14 in list(PT_SLOT.values())) or (
                slot_num == 14 and 6 in list(PT_SLOT.values())
            ):
                continue
            if (slot_num == 7 and 15 in list(PT_SLOT.values())) or (
                slot_num == 15 and 7 in list(PT_SLOT.values())
            ):
                continue
...
def pt_devs_check_audio(audio_map, audio_codec_map):
    ...
    for vmid in list(audio_map.keys()):
        ...
        if not bdf_audio and bdf_codec:
            key = "uos:id={},passthrough_devices,{}".format(
                vmid, "audio_codec")
            ERR_LIST[
                key
            ] = "Audio codec device should be pass through together with Audio devcie!"
    ...
    for dev in bdf_dic.keys():
        ...
        for vm_i in dev_bdf_dic.keys():
            ...
            if dev_bdf in bdf_used:
                key = "uos:id={},{},{}".format(
                    vm_i, "passthrough_devices", dev)
                ERR_LIST[
                    key
                ] = "You select the same device for {} pass-through !".format(dev)
                return
            else:
...
def get_gpu_vpid():
    ...
    vpid = ""
    vpid_lines = board_cfg_lib.get_info(
        common.BOARD_INFO_FILE, "<PCI_VID_PID>", "</PCI_VID_PID>"
    )
    gpu_bdf = get_gpu_bdf()
...
def set_shm_regions(launch_item_values, scenario_info):
    ...
    try:
        raw_shmem_regions = common.get_hv_item_tag(
            scenario_info, "FEATURES", "IVSHMEM", "IVSHMEM_REGION"
        )
        vm_types = common.get_leaf_tag_map(scenario_info, "vm_type")
        shm_enabled = common.get_hv_item_tag(
            scenario_info, "FEATURES", "IVSHMEM", "IVSHMEM_ENABLED"
        )
    except:
        return
            ...
            if shm_enabled == "y":
                for shmem_region in raw_shmem_regions:
                    ...
                    try:
                        ...
                        vm_id_list = [x.strip()
                                      for x in shm_splited[2].split(":")]
                        if str(vm_id) in vm_id_list:
                            launch_item_values[shm_region_key].append(
                                ",".join([name, size])
                            )
...
def set_pci_vuarts(launch_item_values, scenario_info):
    try:
        ...
        for vm in list(common.get_config_root(scenario_info)):
            if (
                vm.tag == "vm"
                and scenario_cfg_lib.VM_DB[vm_types[int(vm.attrib["id"])]]["load_type"]
                == "POST_LAUNCHED_VM"
            ):
                uos_id = int(vm.attrib["id"]) - sos_vm_id
                pci_vuart_key = (
                    "uos:id={},communication_vuarts,communication_vuart".format(
                        uos_id)
                )
                for elem in list(vm):
...
def check_shm_regions(launch_shm_regions, scenario_info):
    ...
    for uos_id, shm_regions in launch_shm_regions.items():
        ...
        for shm_region in shm_regions:
            if (
                shm_region_key not in launch_item_values.keys()
                or shm_region not in launch_item_values[shm_region_key]
            ):
                ERR_LIST[shm_region_key] = (
                    "shm {} should be configured in scenario setting and the size should be decimal"
...
def check_console_vuart(launch_console_vuart, vuart0, scenario_info):
    ...
    for uos_id, console_vuart_enable in launch_console_vuart.items():
        ...
        if console_vuart_enable == "Enable" and vuart0[uos_id] == "Enable":
            ERR_LIST[key] = (
                "vuart0 and console_vuart of uos {} should not be enabled "
                "at the same time".format(uos_id)
            )
            return
        if (
            console_vuart_enable == "Enable"
            and int(uos_id) in vuarts.keys()
            and 0 in vuarts[uos_id]
            and vuarts[uos_id][0]["base"] == "INVALID_PCI_BASE"
        ):
            ERR_LIST[
                key
            ] = "console_vuart of uos {} should be enabled in scenario setting".format(
                uos_id
            )
            return
...
def check_communication_vuart(launch_communication_vuarts, scenario_info):
    ...
    for uos_id, vuart_list in launch_communication_vuarts.items():
        vuart_key = "uos:id={},communication_vuarts,communication_vuart".format(
            uos_id)
        for vuart_id in vuart_list:
            ...
            if int(vuart_id) not in vuarts[uos_id].keys():
                ERR_LIST[vuart_key] = (
                    "communication_vuart {} of uos {} should be configured"
                    "in scenario setting.".format(vuart_id, uos_id)
                )
                return
            if int(vuart_id) == 1 and vuarts[uos_id][1]["base"] != "INVALID_PCI_BASE":
                if (
                    uos_id in vuart1_setting.keys()
                    and vuart1_setting[uos_id]["base"] != "INVALID_COM_BASE"
                ):
                    ERR_LIST[vuart_key] = (
                        "uos {}'s communication_vuart 1 and legacy_vuart 1 should "
...
def check_enable_ptm(launch_enable_ptm, scenario_info):
    ...
    for uos_id, enable_ptm in launch_enable_ptm.items():
        ...
        if enable_ptm == "y" and str(uos_id) not in enable_ptm_vm_list:
            ERR_LIST[key] = "PTM of uos:{} set to 'n' in scenario xml".format(
                uos_id)
