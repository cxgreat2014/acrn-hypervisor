...
ACRN_CONFIG_TARGET = ""
SOURCE_ROOT_DIR = os.path.join(os.path.dirname(
...
DATACHECK_SCHEMA_FILE = SOURCE_ROOT_DIR + \
    "misc/config_tools/schema/datachecks.xsd"


PY_CACHES = [
    "__pycache__",
    "../board_config/__pycache__",
    "../scenario_config/__pycache__",
]
GUEST_FLAG = [
    "0",
    "0UL",
...
def usage(file_name):
    ...
    print(
        "--board <board_info_file> --scenario <scenario_info_file> --out [output folder]"
    )
...
def get_param(args):
    ...
    try:
        (optlist, args_list) = getopt.getopt(
            args_list, "", ["hv=", "board=", "scenario=", "out="]
        )
    ...
    for par_k, par_v in params.items():
        ...
        if not par_v:
            usage(args[0])
            err_dic["wrong usage"] = "Parameter for {} should not empty".format(
                par_k)
        ...
        while True:
            ...
            if "board=" in line or "scenario=" in line:
                ...
                if attrib not in line:
                    err_dic[
                        "common error"
                    ] = "The {} attribute is not in xml file".format(attrib)
...
def get_node(xpath, etree):
    ...
    assert (
        len(result) <= 1
    ), f"Internal error: multiple element nodes are found for {xpath}"
...
def update_text(xpath, value, etree, overwrite=False):
    ...
    assert (
        len(result) == 1
    ), "Internal error: cannot set text to multiple nodes at a time"
...
def append_node(xpath, value, etree, **attribute):
    ...
    while ancestor_level < len(parts):
        ...
        assert (
            len(result) <= 1
        ), "Internal error: cannot append element nodes to multiple ancestors"
        if len(result) == 1:
            ancestor = result[0]
            break
        ancestor_level += 1
    ...
    ), f"Internal error: cannot find an existing ancestor for {xpath}"
        ...
        for sub in item:
            ...
            if sub.tag == branch_tag:
                ...
                for leaf in sub:
                    if (
                        leaf.tag == tag_str
                        and tag_str not in MULTI_ITEM
                        and sub.tag not in ["legacy_vuart", "vuart"]
                    ):
...
def get_vuart_info_id(config_file, idx):
    ...
    for item in root:
        ...
        for sub in item:
            ...
            for leaf in sub:
                if (
                    sub.tag in ["legacy_vuart", "vuart"]
                    and int(sub.attrib["id"]) == idx
                ):
                        ...
                        if not leaf_str:
                            ...
                            for leaf_s in leaf:
                                if (
                                    leaf_s.tag == leaf_str
                                    and leaf_s.text
                                    and leaf_s.text != None
                                ):
                                    if (
                                        leaf_str == "CLOS_MASK"
                                        or leaf_str == "MBA_DELAY"
                                        or leaf_str == "IVSHMEM_REGION"
                                    ):
    ...
    if not os.path.exists(path):
        try:
            subprocess.check_call(
                "mkdir -p {}".format(path), shell=True, stdout=subprocess.PIPE
            )
...
def get_avl_dev_info(bdf_desc_map, pci_sub_class):
    ...
    for sub_class in pci_sub_class:
        for pci_desc_value in bdf_desc_map.values():
            pci_desc_sub_class = " ".join(
                pci_desc_value.strip().split(":")[1].split()[1:]
            )
