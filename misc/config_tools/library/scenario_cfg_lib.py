...
VM_DB = {
    ...
    "SAFETY_VM": {
        ...
        "uuid": UUID_DB["SAFETY_VM"],
    },
    "PRE_RT_VM": {
        "load_type": "PRE_LAUNCHED_VM",
        "severity": "SEVERITY_RTVM",
        "uuid": UUID_DB["PRE_RT_VM"],
    },
    "PRE_STD_VM": {
        "load_type": "PRE_LAUNCHED_VM",
        "severity": "SEVERITY_STANDARD_VM",
        "uuid": UUID_DB["PRE_STD_VM"],
    },
    "POST_STD_VM": {
        "load_type": "POST_LAUNCHED_VM",
        "severity": "SEVERITY_STANDARD_VM",
        "uuid": UUID_DB["POST_STD_VM"],
    },
    "POST_RT_VM": {
        "load_type": "POST_LAUNCHED_VM",
        "severity": "SEVERITY_RTVM",
        "uuid": UUID_DB["POST_RT_VM"],
    },
    "KATA_VM": {
        "load_type": "POST_LAUNCHED_VM",
        "severity": "SEVERITY_STANDARD_VM",
        "uuid": UUID_DB["KATA_VM"],
    },
    "PRE_RT_VM": {
        "load_type": "PRE_LAUNCHED_VM",
        "severity": "SEVERITY_RTVM",
...
def get_shmem_regions(raw_shmem_regions):
    ...
    for raw_shmem_region in raw_shmem_regions:
        if raw_shmem_region and raw_shmem_region.strip():
            ...
            if len(shm_splited) == 3 and (
                shm_splited[0].strip() != ""
                and shm_splited[1].strip() != ""
                ...
                for i in range(len(vmid_list)):
                    ...
                    if int_vm_id not in shmem_regions.keys():
                        shmem_regions[int_vm_id] = [
                            ",".join(
                                [
                                    name,
                                    size,
                                    ":".join(
                                        vmid_list[0:i] + vmid_list[i + 1:]),
                                ]
                            )
                        ]
                    else:
                        shmem_regions[int_vm_id].append(
                            ",".join(
                                [
                                    name,
                                    size,
...
def get_pci_vuart_num(vuarts):
    ...
    for vm_i in vuart0_setting:
        ...
        if (
            "POST_LAUNCHED_VM" == VM_DB[vm_type]["load_type"]
            and 0 in vuarts[vm_i].keys()
        ...
        if (
            vuart0_setting[vm_i]["base"] != "INVALID_COM_BASE"
            and 0 in vuarts[vm_i].keys()
            and vuarts[vm_i][0]["base"] != "INVALID_PCI_BASE"
        ):
            vuarts_num[vm_i] -= 1
    for vm_i in vuart1_setting:
        ...
        if (
            vuart1_setting[vm_i]["base"] != "INVALID_COM_BASE"
            and 1 in vuarts[vm_i].keys()
...
def get_pci_dev_num_per_vm():
    ...
    pci_items = common.get_leaf_tag_map(
        common.SCENARIO_INFO_FILE, "pci_devs", "pci_dev"
    )
    ...
    shmem_enabled = common.get_hv_item_tag(
        common.SCENARIO_INFO_FILE, "FEATURES", "IVSHMEM", "IVSHMEM_ENABLED"
    )
    ...
    for vm_i, vm_type in common.VM_TYPES.items():
        if "POST_LAUNCHED_VM" == VM_DB[vm_type]["load_type"]:
            ...
            if shmem_enabled == "y" and vm_i in shmem_num.keys():
                shmem_num_i = shmem_num[vm_i]
            pci_dev_num_per_vm[vm_i] = pt_pci_num[vm_i] + \
                shmem_num_i + vuarts_num[vm_i]
...
def check_board_private_info():
    ...
    for tag_str in dev_private_tags:
        dev_setting = common.get_leaf_tag_map(
            common.SCENARIO_INFO_FILE, branch_tag, tag_str
        )
        private_info[tag_str] = dev_setting

    if not private_info["rootfs"] and err_dic:
        ERR_LIST[
            "vm:id=0,boot_private,rootfs"
        ] = "The board have to chose one rootfs partition"
...
def load_vm_check(load_vms, item):
    ...
    if len(kata_vm_ids) > len(UUID_DB["KATA_VM"]):
        ...
        ERR_LIST[key] = "KATA VM number should not be greater than {}".format(
            len(UUID_DB["KATA_VM"])
        )
        return
    ...
    if len(rt_vm_ids) > len(UUID_DB["POST_RT_VM"]):
        ...
        ERR_LIST[key] = "POST RT VM number should not be greater than {}".format(
            len(UUID_DB["POST_RT_VM"])
        )
        return
    ...
    if len(post_vm_ids) > len(UUID_DB["POST_STD_VM"]):
        ...
        ERR_LIST[key] = "POST Standard vm number should not be greater than {}".format(
            len(UUID_DB["POST_STD_VM"])
        )
        return

    max_pre_launch_vms = (
        len(UUID_DB["PRE_STD_VM"])
        + len(UUID_DB["SAFETY_VM"])
        + len(UUID_DB["PRE_RT_VM"])
    )
    if len(pre_vm_ids) > max_pre_launch_vms:
        key = "vm:id={},{}".format(pre_vm_ids[0], item)
        ERR_LIST[key] = "PRE Launched VM number should not be greater than {}".format(
            max_pre_launch_vms
        )
        return
...
def vm_cpu_affinity_check(config_file, id_cpus_per_vm_dic, item):
    ...
    cpu_sharing = common.get_hv_item_tag(
        common.SCENARIO_INFO_FILE, "FEATURES", "SCHEDULER"
    )
    if cpu_sharing == "SCHED_NOOP":
        cpu_sharing_enabled = False

    cpu_affinity = common.get_leaf_tag_map(
        config_file, "cpu_affinity", "pcpu_id")
    for vm_i in id_cpus_per_vm_dic.keys():
        for cpu in id_cpus_per_vm_dic[vm_i]:
            if cpu is not None and cpu in use_cpus and not cpu_sharing_enabled:
                ...
                err_dic[
                    key
                ] = "The same pcpu was configurated in <pcpu_id>/<cpu_affinity>, but CPU sharing is disabled by 'SCHED_NOOP'. Please re-configurate them!"
                return err_dic
            else:
                use_cpus.append(cpu)
    ...
    if pre_launch_cpus:
        ...
        for pcpu in pre_launch_cpus:
            if pre_launch_cpus.count(pcpu) >= 2:
                ...
                err_dic[
                    key
                ] = "Pre_launched_vm vm should not have the same cpus assignment"
...
def mem_size_check(id_hpa_size_dic, prime_item, item):
    ...
    for id_key, hpa_size in id_hpa_size_dic.items():
        ...
        if (
            hpa_sz_strip_ul not in START_HPA_SIZE_LIST
            and hpa_sz_strip_u not in START_HPA_SIZE_LIST
        ):
            key = "vm:id={},{},{}".format(id_key, prime_item, item)
...
def os_kern_name_check(id_kern_name_dic, prime_item, item):
    ...
    for id_key, kern_name in id_kern_name_dic.items():
        if len(kern_name) > 32 or len(kern_name) == 0:
            ...
            ERR_LIST[
                key
            ] = "VM os config kernel name length should be in range [1,32] bytes"
...
def os_kern_mod_check(id_kern_mod_dic, prime_item, item):
    ...
    for id_key, kern_mod in id_kern_mod_dic.items():
        if len(kern_mod) > 32 or len(kern_mod) == 0:
            ...
            ERR_LIST[
                key
            ] = "VM os config kernel mod tag should be in range [1,32] bytes"
    ...
    for id_key, pci_bdf_devs_list in pci_bdf_devs.items():
        for pci_bdf_dev in pci_bdf_devs_list:
            if pci_bdf_dev and pci_bdf_dev not in bdf_desc_map.keys():
                ...
                ERR_LIST[key] = "The {} is unkonw device of PCI".format(
                    pci_bdf_dev)
...
def avl_pci_devs():
    ...
    pci_dev_list = []
    (bdf_desc_map, bdf_vpid_map) = board_cfg_lib.get_pci_info(common.BOARD_INFO_FILE)
    pci_dev_list = common.get_avl_dev_info(
        bdf_desc_map, PT_SUB_PCI["ethernet"])
...
def check_vuart(v0_vuart, v1_vuart):
    ...
    for vm_i, t_vm_id in vm_target_id_dic.items():
        ...
        for j in range(idx + 1, len(target_id_keys)):
            ...
            if (
                int(vm_target_id_dic[vm_i]) == vm_j
                and int(vm_target_id_dic[vm_j]) == vm_i
            ):
                connect_set = True
...
def get_legacy_vuart1_target_dict(legacy_vuart1):
    ...
    for vm_i, vuart_dict in legacy_vuart1.items():
        ...
        if vm_i not in vuart1_target_dict:
            ...
            raise ValueError(
                "vm id {} has more than one legacy vuart 1".format(vm_i))
...
def check_vuart_id(vuart_id):
    if not isinstance(vuart_id, int):
        raise ValueError(
            "vuart_id must be int: {}, {!r}".format(type(vuart_id), vuart_id)
        )
...
def check_vuart_id_count(vm_pci_vuarts, legacy_vuart0, legacy_vuart1):
    ...
    if vuart_cnt > common.MAX_VUART_NUM:
        raise XmlError(
            "enables more than {} vuarts, total number: {}".format(
                common.MAX_VUART_NUM, vuart_cnt
            )
        )
...
def check_against_coexistence(vm_pci_vuarts, vm_legacy_vuart, legacy_vuart_idx):
    ...
    if legacy_base not in VUART_BASE:
        raise LegacyVuartError(
            "legacy vuart base should be one of {}".format(
                ", ".join(VUART_BASE))
        )
    ...
    if pci_vuart_base not in PCI_VUART_BASE:
        raise PciVuartError(
            "pci vuart base should be one of {}, last call: {!r}".format(
                ", ".join(PCI_VUART_BASE), pci_vuart_base
            )
        )
    if pci_vuart_base == INVALID_PCI_BASE:
        return
    raise PciVuartError(
        "cannot enable legacy vuart {} and this vuart {} at the same time".format(
            legacy_vuart_idx, legacy_vuart_idx
        )
    )
...
def check_pci_vuart_base(pci_vuart):
    if not isinstance(pci_vuart, dict):
        raise TypeError(
            "pci_vuart should be a dict: {}, {!r}".format(
                type(pci_vuart), pci_vuart)
        )
    if "base" not in pci_vuart:
        raise ValueError(
            "base should be in vuart: keys_found={}".format(pci_vuart.keys())
        )

    pci_vuart_base_str = pci_vuart["base"]
    if pci_vuart_base_str not in PCI_VUART_BASE:
        raise ValueError(
            "base should be one of {}, last called:  {!r}".format(
                ", ".join(PCI_VUART_BASE), pci_vuart_base_str
            )
        )
...
def get_target_vm_id(vuart, vm_id):
    if not isinstance(vuart, dict):
        raise TypeError(
            "vuart should be a dict: {}, {!r}".format(type(vuart), vuart))
    if not isinstance(vm_id, int):
        raise TypeError(
            "vm_id should be an int: {}, {!r}".format(type(vm_id), vm_id))
    if F_TARGET_VM_ID not in vuart:
        raise ValueError(
            "target_vm_id should be in vuart: keys_found={}".format(
                vuart.keys())
        )
...
def get_target_uart_id(vuart):
    ...
    if F_TARGET_UART_ID not in vuart:
        raise ValueError(
            "target_uart_id should be in vuart: keys_found={}".format(
                vuart.keys())
        )
...
def check_pci_vuart(pci_vuarts, legacy_vuart0, legacy_vuart1):
    ...
    for vm_id, vm_pci_vuarts in pci_vuarts.items():
        ...
        try:
            vuart_id = 0
            check_against_coexistence(
                vm_pci_vuarts, legacy_vuart0.get(vm_id), vuart_id)
            vuart_id = 1
            check_against_coexistence(
                vm_pci_vuarts, legacy_vuart1.get(vm_id), vuart_id)
            key = "vm:id={}".format(vm_id)
            check_vuart_id_count(
                vm_pci_vuarts, legacy_vuart0.get(
                    vm_id), legacy_vuart1.get(vm_id)
            )
        except XmlError as exc:
        ...
        for vuart_id, pci_vuart in vm_pci_vuarts.items():
            ...
            try:
                err_key = "vm:id={},communication_vuart:id={},target_vm_id".format(
                    vm_id, vuart_id
                )
                target_vm_id = get_target_vm_id(pci_vuart, vm_id)
                err_key = "vm:id={},communication_vuart:id={},target_uart_id".format(
                    vm_id, vuart_id
                )
            ...
            if vuart_id in vm_target_dict[vm_id]:
                raise ValueError(
                    "Duplicated vm id {} and vuart id {}".format(
                        vm_id, vuart_id)
                )
            else:
                vm_target_dict[vm_id][vuart_id] = (
                    target_vm_id, target_uart_id)

            if vm_id not in vm_visited:
                vm_visited[vm_id] = {}
            if vuart_id in vm_visited[vm_id]:
                raise ValueError(
                    "vuart {} of vm {} is duplicated".format(vuart_id, vm_id)
                )
            else:
                vm_visited[vm_id][vuart_id] = -1

    legacy_vuart1_target_dict, legacy_vuart1_visited = get_legacy_vuart1_target_dict(
        legacy_vuart1
    )
    ...
    for vm_id, target in legacy_vuart1_target_dict.items():
        try:
            ...
            is_target_vm_available(
                target_vm_id, vm_visited, legacy_vuart1_visited)
            err_key = "vm:id={},legacy_vuart:id=1,target_uart_id".format(vm_id)
            target_uart_id = int(target[1])
            check_target_connection(
                vm_id, target_vm_id, target_uart_id, vm_visited, legacy_vuart1_visited
            )
...
def is_target_vm_available(target_vm_id, vm_visited, legacy_vuart1_visited):
    if not isinstance(target_vm_id, int):
        raise TypeError(
            "target_vm_id should be an int: {}, {!r}".format(
                type(target_vm_id), target_vm_id
            )
        )
    if not isinstance(vm_visited, dict):
        raise TypeError(
            "vm_visited should be a dict: {}, {!r}".format(
                type(vm_visited), vm_visited)
        )
    if not isinstance(legacy_vuart1_visited, dict):
        raise TypeError(
            "legacy_vuart1_visited should be a dict: {}, {!r}".format(
    ...
    if target_vm_id in vm_visited:
        ...
        raise TargetError(
            "target vm {} disables legacy vuart 1 and all communication vuarts".format(
                target_vm_id
            )
        )
...
def check_target_connection(
    ...
    if not isinstance(target_uart_id, int):
        raise TypeError(
            "target_uart_id should be an int: {}, {!r}".format(
                type(target_uart_id), target_uart_id
            )
        )
    ...
    if vm_visited.get(target_vm_id) and target_uart_id in vm_visited[target_vm_id]:
        ...
        if connected_vm > -1:
            raise TargetError(
                "target vm{} : vuart{} is connected to vm {}".format(
                    target_vm_id, target_uart_id, connected_vm
                )
            )
        else:
        ...
        raise TargetError(
            "target vm{}'s vuart{} is not present".format(
                target_vm_id, target_uart_id)
        )
...
def vcpu_clos_check(cpus_per_vm, clos_per_vm, prime_item, item):
    ...
    for vm_i, vcpus in cpus_per_vm.items():
        ...
        if clos_per_vm_len != len(vcpus):
            ...
            ERR_LIST[
                key
            ] = "'vcpu_clos' number should be equal 'pcpu_id' number for VM{}".format(
                vm_i
            )
            return
        ...
        if board_cfg_lib.is_cdp_enabled() and common_clos_max != 0:
            for clos_val in clos_per_vm[vm_i]:
                ...
                if int(clos_val) >= common_clos_max:
                    ...
                    ERR_LIST[
                        key
                    ] = "CDP_ENABLED=y, the clos value should not be greater than {} for VM{}".format(
...
def share_mem_check(
    shmem_regions, raw_shmem_regions, vm_type_info, prime_item, item, sub_item
):
    ...
    if shmem_region_num > MAX_SHMEM_REGION_NUM:
        key = "hv,{},{},{},{}".format(
            prime_item, item, sub_item, MAX_SHMEM_REGION_NUM)
        ERR_LIST[
            key
        ] = "The number of hv-land shmem regions should not be greater than {}.".format(
        ...
        for shm_str in shm_list:
            ...
            if index == -1:
                try:
                    for i in range(len(raw_shmem_regions)):
                        if (
                            raw_shmem_regions[i].split(",")[0].strip()
            ...
            if len(shm_str_splited) < 3:
                ERR_LIST[key] = (
                    "The name, size, communication VM IDs of the share memory should be separated "
            ...
            try:
                ...
                ERR_LIST[
                    key
                ] = "The shared memory region should be configured with format like this: hv:/shm_region_0, 2, 0:2"
                return
            ...
            for vmid in vmid_list:
                try:
                    ...
                    ERR_LIST[
                        key
                    ] = "The communication VM IDs of the share memory should be decimal and separated by comma."
                    return
            if not int_vmid_list:
                ...
                return
            if curr_vm_id in int_vmid_list or len(set(int_vmid_list)) != len(
                int_vmid_list
            ):
                ERR_LIST[
                    key
                ] = "The communication VM IDs of the share memory should not be duplicated."
                return
            for target_vm_id in int_vmid_list:
                if (
                    curr_vm_id not in vm_type_info.keys()
                    or target_vm_id not in vm_type_info.keys()
                ):
                    ERR_LIST[
                        key
                    ] = "Shared Memory can be only configured for existed VMs."
                    return

            if name == "" or size == "":
                ERR_LIST[
                    key
                ] = "The name, size of the share memory should not be empty."
                return
            name_len = len(name)
            if name_len > 32 or name_len == 0:
                ERR_LIST[
                    key
                ] = "The size of share Memory name should be in range [1,32] bytes."
                return
            ...
            try:
                ...
                ERR_LIST[
                    key
                ] = "The size of the shared memory region should be a decimal number."
                return
            if int_size < 0x200000 or int_size > 0x20000000:
                ERR_LIST[
                    key
                ] = "The size of the shared memory region should be in the range of [2MB, 512MB]."
                return
            if not ((int_size & (int_size - 1) == 0) and int_size != 0):
                ERR_LIST[
                    key
                ] = "The size of share Memory region should be a power of 2."
                return
            ...
            if shmem_names[name] > len(vmid_list) + 1:
                ERR_LIST[
                    key
                ] = "The names of share memory regions should not be duplicated: {}".format(
                    name
                )
                return
    ...
    for shm_i, shm_list in shmem_regions.items():
        for shm_str in shm_list:
            ...
            if "IVSHMEM_" + name in board_cfg_lib.PCI_DEV_BAR_DESC.shm_bar_dic.keys():
                ...
                if (
                    0 in bar_attr_dic.keys()
                    and int(bar_attr_dic[0].addr, 16) < 0x80000000
                ) or (
                    2 in bar_attr_dic.keys()
                    and int(bar_attr_dic[2].addr, 16) < 0x100000000
                ):
                    ERR_LIST[
                        key
                    ] = "Failed to get the start address of the shared memory, please check the size of it."
                    return
...
def check_p2sb(enable_p2sb):
    ...
    for vm_i, p2sb in enable_p2sb.items():
        ...
        if p2sb and not VM_DB[common.VM_TYPES[0]]["load_type"] == "PRE_LAUNCHED_VM":
            ERR_LIST[
                "vm:id=0,p2sb"
            ] = "p2sb passthru can only be enabled for Pre-launched VM"
            return
        ...
        if p2sb and board_cfg_lib.is_tpm_passthru():
            ERR_LIST[
                "vm:id=0,p2sb"
            ] = "Cannot enable p2sb and tpm passthru at the same time"
            return
...
def check_pt_intx(phys_gsi, virt_gsi):
    ...
    for (id1, p), (id2, v) in zip(phys_gsi.items(), virt_gsi.items()):
        ...
        if len(p) > 120:
            ERR_LIST[
                "vm:id=0,pt_intx"
            ] = "# of phys_gsi and virt_gsi pairs must not be greater than 120"
            return
...
def get_valid_ttys_for_sos_vuart(ttys_n):
    ...
    ttys_lines = board_cfg_lib.get_info(
        common.BOARD_INFO_FILE, "<TTYS_INFO>", "</TTYS_INFO>"
    )
    if ttys_lines:
        vuart0_valid.clear()
    ...
    if not vuart1_valid:
        common.print_yel(
            "ttyS are fully used. ttyS0 is used for hv_console, ttyS1 is used for vuart1!",
            warn=True,
        )
        vuart1_valid = ["ttyS0", "ttyS1", "ttyS2", "ttyS3"]
