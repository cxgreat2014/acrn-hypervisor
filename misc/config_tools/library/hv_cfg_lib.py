...
def is_numeric_check(str_value, prime_item, item):
    ...
    if not str_hex_0x.isnumeric() and not str_hex_0X.isnumeric():
        if not isinstance(int(str_hex_0x, 16), int) and not isinstance(
            int(str_hex_0X, 16), int
        ):
            key = "hv,{},{}".format(prime_item, item)
...
def range_check(str_value, prime_item, item, range_val):
    ...
    if value < range_val["min"] or value > range_val["max"]:
        key = "hv,{},{}".format(prime_item, item)
        ERR_LIST[key] = "{} should be in range[{},{}]".format(
            item, range_val["min"], range_val["max"]
        )
...
def scheduler_check(sel_str, feat, feat_scheduler):
    ...
    if sel_str not in SCHEDULER_TYPE:
        ...
        ERR_LIST[key] = "{} should be in {}".format(
            feat_scheduler, SCHEDULER_TYPE)
...
def cat_max_mask_check(cat_mask_list, feature, cat_str, max_mask_str):
    ...
    (res_info, rdt_res_clos_max, clos_max_mask_list) = board_cfg_lib.clos_info_parser(
        common.BOARD_INFO_FILE
    )
    if not board_cfg_lib.is_rdt_enabled() or (
        "L2" not in res_info and "L3" not in res_info
    ):
        return
    ...
    if clos_max_set_entry != cat_max_mask_settings_len:
        ...
        ERR_LIST[
            key
        ] = "Number of Cache mask entries should be equal to MAX_CACHE_CLOS_NUM_ENTRIES={}".format(
            clos_max_set_entry
        )
        return
    ...
    for val_str in cat_mask_list:
        ...
        if not is_contiguous_bit_set(value):
            key = "hv,{},{},{}".format(feature, cat_str, max_mask_str)
            ERR_LIST[key] = "CLOS_MASK {} should be contiguous bit set.".format(
                max_mask_str, clos_max_mask_str
            )
            return
...
def mba_delay_check(mba_delay_list, feature, mba_str, max_mask_str):
    ...
    if clos_max != mba_delay_settings_len:
        ...
        ERR_LIST[
            key
        ] = "Number of MBA delay entries should be equal to MAX_MBA_CLOS_NUM_ENTRIES={}".format(
            clos_max
        )
        return
...
def max_msix_table_num_check(max_msix_table_num, cap_str, max_msi_num_str):
    native_max_msix_line = board_cfg_lib.get_info(
        common.BOARD_INFO_FILE, "<MAX_MSIX_TABLE_NUM>", "</MAX_MSIX_TABLE_NUM>"
    )
