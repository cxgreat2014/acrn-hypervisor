...
def get_memory(hv_info, config):
    ...
    if hv_ram_size > HV_RAM_SIZE_MAX:
        common.print_red(
            "requested RAM size should be smaller then {}".format(
                HV_RAM_SIZE_MAX),
            err=True,
        )
        err_dic[
            "board config: total vm number error"
    ...
    if not hv_info.mem.hv_ram_start:
        ...
        print("CONFIG_HV_RAM_START={}".format(
            hv_info.mem.hv_ram_start), file=config)
    if not hv_info.mem.hv_ram_size:
        print("CONFIG_HV_RAM_SIZE={}".format(hex(hv_ram_size)), file=config)
    else:
        print("CONFIG_HV_RAM_SIZE={}".format(
            hv_info.mem.hv_ram_size), file=config)

    print(
        "CONFIG_PLATFORM_RAM_SIZE={}".format(hv_info.mem.platform_ram_size), file=config
    )
    print("CONFIG_LOW_RAM_SIZE={}".format(
        hv_info.mem.low_ram_size), file=config)
    print("CONFIG_STACK_SIZE={}".format(hv_info.mem.stack_size), file=config)
    print("CONFIG_IVSHMEM_ENABLED={}".format(
        hv_info.mem.ivshmem_enable), file=config)
...
def get_serial_console(config):
    ...
    if serial_type == "portio":
        ...
        if serial_value:
            print("CONFIG_SERIAL_MMIO_BASE={}".format(
                serial_value), file=config)
...
def get_features(hv_info, config):
    ...
    print("CONFIG_MULTIBOOT2={}".format(
        hv_info.features.multiboot2), file=config)
    print("CONFIG_RDT_ENABLED={}".format(
        hv_info.features.rdt_enabled), file=config)
    if hv_info.features.rdt_enabled == "y":
        print("CONFIG_CDP_ENABLED={}".format(
            hv_info.features.cdp_enabled), file=config)
    else:
        print("CONFIG_CDP_ENABLED=n", file=config)
    print(
        "CONFIG_HYPERV_ENABLED={}".format(hv_info.features.hyperv_enabled), file=config
    )
    print(
        "CONFIG_IOMMU_ENFORCE_SNP={}".format(
            hv_info.features.iommu_enforce_snp),
        file=config,
    )
    print(
        "CONFIG_ACPI_PARSE_ENABLED={}".format(
            hv_info.features.acpi_parse_enabled),
        file=config,
    )
    print(
        "CONFIG_L1D_FLUSH_VMENTRY_ENABLED={}".format(
            hv_info.features.l1d_flush_vmentry_enabled
        ),
        file=config,
    )
    print(
        "CONFIG_MCE_ON_PSC_WORKAROUND_DISABLED={}".format(
            hv_info.features.mce_on_psc_workaround_disabled
        ),
        file=config,
    )
    if hv_info.features.ssram_enabled in ["y", "n"]:
        print(
            "CONFIG_SSRAM_ENABLED={}".format(hv_info.features.ssram_enabled),
            file=config,
        )
...
def get_capacities(hv_info, config):
    ...
    print("CONFIG_IOMMU_BUS_NUM={}".format(
        hv_info.cap.iommu_bus_num), file=config)
    print("CONFIG_MAX_IOAPIC_NUM={}".format(
        hv_info.cap.max_ioapic_num), file=config)
    print("CONFIG_MAX_IR_ENTRIES={}".format(
        hv_info.cap.max_ir_entries), file=config)
    print("CONFIG_MAX_PCI_DEV_NUM={}".format(
        hv_info.cap.max_pci_dev_num), file=config)
    print(
        "CONFIG_MAX_IOAPIC_LINES={}".format(hv_info.cap.max_ioapic_lines), file=config
    )
    print(
        "CONFIG_MAX_PT_IRQ_ENTRIES={}".format(hv_info.cap.max_pt_irq_entries),
        file=config,
    )
    ...
    print("CONFIG_MAX_MSIX_TABLE_NUM={}".format(
        max_msix_table_num), file=config)
    print(
        "CONFIG_MAX_EMULATED_MMIO_REGIONS={}".format(
...
def get_log_opt(hv_info, config):
    ...
    print("CONFIG_NPK_LOGLEVEL_DEFAULT={}".format(
        hv_info.log.level.npk), file=config)
    print("CONFIG_MEM_LOGLEVEL_DEFAULT={}".format(
        hv_info.log.level.mem), file=config)
    print("CONFIG_LOG_DESTINATION={}".format(hv_info.log.dest), file=config)
    print(
        "CONFIG_CONSOLE_LOGLEVEL_DEFAULT={}".format(hv_info.log.level.console),
        file=config,
    )
