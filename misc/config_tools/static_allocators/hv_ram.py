...
def fn(board_etree, scenario_etree, allocation_etree):
    ...
    for id in common.VM_TYPES:
        if (
            common.VM_TYPES[id] in scenario_cfg_lib.VM_DB
            and scenario_cfg_lib.VM_DB[common.VM_TYPES[id]]["load_type"]
            == "POST_LAUNCHED_VM"
        ):
            post_launched_vm_num += 1
    hv_ram_size = (
        common.HV_BASE_RAM_SIZE
        + common.POST_LAUNCHED_VM_RAM_SIZE * post_launched_vm_num
    )

    ivshmem_enabled = common.get_node(
        "//IVSHMEM_ENABLED/text()", scenario_etree)
    total_shm_size = 0
    if ivshmem_enabled == "y":
        ...
        for raw_shm in raw_shmem_regions:
            ...
            if (
                len(raw_shm_splited) == 3
                and raw_shm_splited[0].strip() != ""
                and raw_shm_splited[1].strip() != ""
                and len(raw_shm_splited[2].strip().split(":")) >= 1
            ):
                try:
    ...
    avl_start_addr = board_cfg_lib.find_avl_memory(
        ram_range, str(total_size), hv_start_offset
    )
    ...
    common.append_node(
        "/acrn-config/hv/MEMORY/HV_RAM_START", hex(
            hv_start_addr), allocation_etree
    )
    common.append_node(
        "/acrn-config/hv/MEMORY/HV_RAM_SIZE", hex(
            hv_ram_size), allocation_etree
    )
