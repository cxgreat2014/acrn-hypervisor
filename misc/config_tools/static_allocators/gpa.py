...
class AddrWindow(namedtuple("AddrWindow", ["start", "end"])):
    ...
    def from_str(cls, value):
        ...
        if match:
            return AddrWindow(
                start=int(match.group("start"), 16), end=int(match.group("end"), 16)
            )
        else:
    ...
    def overlaps(self, other):
        if not isinstance(other, AddrWindow):
            raise TypeError(
                "overlaps() other must be an AddrWindow: {}".format(type(other))
            )
...
def insert_vuart_to_dev_dict(scenario_etree, devdict_32bits):
    console_vuart = scenario_etree.xpath(
        f"./console_vuart[base != 'INVALID_PCI_BASE']/@id"
    )
    communication_vuarts = scenario_etree.xpath(
        f".//communication_vuart[base != 'INVALID_PCI_BASE']/@id"
    )
...
def insert_legacy_vuart_to_dev_dict(vm_node, devdict_io_port):
    legacy_vuart = vm_node.xpath(
        f".//legacy_vuart[base = 'CONFIG_COM_BASE']/@id")
    for vuart_id in legacy_vuart:
        devdict_io_port[
            (f"{LEGACY_VUART}_{vuart_id}", "base")
        ] = LEGACY_VUART_IO_PORT_SIZE
...
def insert_pt_devs_to_dev_dict(
    board_etree, vm_node_etree, devdict_32bits, devdict_64bits
):
    ...
    for pt_dev in pt_devs:
        ...
        if pt_dev_node is not None:
            ...
            for pt_dev_resource in pt_dev_resources:
                ...
                if bar_width == "32":
                    devdict_32bits[(f"{dev_name}", f"{bar_region}")] = int(
                        bar_len, 16)
                else:
                    devdict_64bits[(f"{dev_name}", f"{bar_region}")] = int(
                        bar_len, 16)
...
def get_pt_devs_io_port(board_etree, vm_node_etree):
    ...
    for pt_dev in pt_devs:
        ...
        if pt_dev_node is not None:
            pt_dev_resources = pt_dev_node.xpath(
                ".//resource[@type = 'io_port' and @id[starts-with(., 'bar')]]"
            )
            for pt_dev_resource in pt_dev_resources:
                dev_name = str(bdf)
                bar_region = pt_dev_resource.get("id")
                devdict[(f"{dev_name}", f"{bar_region}")] = int(
                    pt_dev_resource.get("min"), 16
                )
    return devdict
...
def insert_vmsix_to_dev_dict(pt_dev_node, devdict):
    ...
    if (vendor, identifier) in KNOWN_CAPS_PCI_DEVS_DB.get("VMSIX"):
        bar_regions = pt_dev_node.xpath(
            ".//resource[@type = 'memory' and @width]")
        bar_32bits = [
            bar_region.get("id")
            for bar_region in bar_regions
            if bar_region.get("width") == "32"
        ]
        bar_32bits_idx_list = [int(bar.split("bar")[-1]) for bar in bar_32bits]
        bar_64bits = [
            bar_region.get("id")
            for bar_region in bar_regions
            if bar_region.get("width") == "64"
        ]
        bar_64bits_idx_list_1 = [int(bar.split("bar")[-1])
                                 for bar in bar_64bits]
        bar_64bits_idx_list_2 = [idx + 1 for idx in bar_64bits_idx_list_1]

        bar_regions_io_port = pt_dev_node.xpath(
            ".//resource[@type = 'io_port' and @id[starts-with(., 'bar')]]/@id"
        )
        bar_io_port_idx_list = [
            int(bar.split("bar")[-1]) for bar in bar_regions_io_port
        ]

        used_bar_index = set(
            bar_32bits_idx_list
            + bar_64bits_idx_list_1
            + bar_64bits_idx_list_2
            + bar_io_port_idx_list
        )
        ...
        try:
            ...
            raise lib.error.ResourceError(
                f"Cannot allocate a bar index for vmsix supported device: {vendor}:{identifier}, used bar idx list: {used_bar_index}"
            )
...
def get_devs_mem_native(board_etree, mems):
    nodes = board_etree.xpath(
        f"//resource[@type = 'memory' and @len != '0x0' and @id and @width]"
    )
    dev_list = []
    for node in nodes:
        start = node.get("min")
...
def get_devs_io_port_native(board_etree, io_port_range):
    nodes = board_etree.xpath(
        f"//device/resource[@type = 'io_port' and @len != '0x0' and @id]"
    )
    dev_list = []
    for node in nodes:
        start = node.get("min")
...
def get_devs_mem_passthrough(board_etree, scenario_etree):
    ...
    for vm_type in lib.lib.PRE_LAUNCHED_VMS_TYPE:
        pt_devs = scenario_etree.xpath(
            f"//vm[vm_type = '{vm_type}']/pci_devs/pci_dev/text()"
        )
        for pt_dev in pt_devs:
            bdf = pt_dev.split()[0]
            bus = int(bdf.split(":")[0], 16)
            dev = int(bdf.split(":")[1].split(".")[0], 16)
            func = int(bdf.split(":")[1].split(".")[1], 16)
            resources = board_etree.xpath(
                f"//bus[@address = '{hex(bus)}']/device[@address = '{hex((dev << 16) | func)}'] \
...
def get_pt_devs_io_port_passthrough(board_etree, scenario_etree):
    ...
    for vm_type in lib.lib.PRE_LAUNCHED_VMS_TYPE:
        ...
        for vm_node in vm_nodes:
            dev_list_per_vm = get_pt_devs_io_port_passthrough_per_vm(
                board_etree, vm_node
            )
...
def get_pci_hole_native(board_etree):
    resources = board_etree.xpath(
        f"//bus[@type = 'pci']/device[@address]/resource[@type = 'memory' and @len != '0x0']"
    )
    resources_hostbridge = board_etree.xpath(
        "//bus[@address = '0x0']/resource[@type = 'memory' and @len != '0x0' and not(starts-with(@id, 'bar')) and not(@width)]"
    )
    low_mem = set()
    high_mem = set()
    for resource_hostbridge in resources_hostbridge:
        ...
        if (
            start is not None
            and end is not None
            and int(start, 16) >= PCI_HOLE_THRESHOLD
        ):
            for resource in resources:
...
def get_io_port_range_native(board_etree):
    resources_hostbridge = board_etree.xpath(
        "//bus[@address = '0x0']/resource[@type = 'io_port' and @len != '0x0']"
    )
    ...
    for resource_hostbridge in resources_hostbridge:
        ...
        if (
            start is not None
            and end is not None
            and int(start, 16) >= IO_PORT_THRESHOLD
            and int(end, 16) <= IO_PORT_MAX_ADDRESS
        ):
...
def create_device_node(allocation_etree, vm_id, devdict):
    for dev in devdict:
        ...
        vm_node = common.get_node(
            f"/acrn-config/vm[@id = '{vm_id}']", allocation_etree)
        if vm_node is None:
            vm_node = common.append_node(
                "/acrn-config/vm", None, allocation_etree, id=vm_id
            )
        dev_node = common.get_node(f"./device[@name = '{dev_name}']", vm_node)
        if dev_node is None:
            dev_node = common.append_node(
                "./device", None, vm_node, name=dev_name)
        if common.get_node(f"./bar[@id='{bar_region}']", dev_node) is None:
            common.append_node(f"./bar", hex(bar_base),
                               dev_node, id=bar_region)
...
def create_vuart_node(allocation_etree, vm_id, devdict):
    for dev in devdict:
        ...
        vuart_node = common.get_node(
            f"./legacy_vuart[@id = '{vuart_id}']", vm_node)
        if vuart_node is None:
            vuart_node = common.append_node(
                "./legacy_vuart", None, vm_node, id=vuart_id
            )
...
def create_native_pci_hole_node(allocation_etree, low_mem, high_mem):
    common.append_node(
        "/acrn-config/hv/MMIO/MMIO32_START",
        hex(low_mem[0].start).upper(),
        allocation_etree,
    )
    common.append_node(
        "/acrn-config/hv/MMIO/MMIO32_END",
        hex(low_mem[0].end + 1).upper(),
        allocation_etree,
    )
    if len(high_mem):
        common.append_node(
            "/acrn-config/hv/MMIO/MMIO64_START",
            hex(high_mem[0].start).upper(),
            allocation_etree,
        )
        common.append_node(
            "/acrn-config/hv/MMIO/MMIO64_END",
            hex(high_mem[0].end + 1).upper(),
            allocation_etree,
        )
        common.append_node(
            "/acrn-config/hv/MMIO/HI_MMIO_START",
            hex(high_mem[0].start).upper(),
            allocation_etree,
        )
        common.append_node(
            "/acrn-config/hv/MMIO/HI_MMIO_END",
            hex(high_mem[0].end + 1).upper(),
            allocation_etree,
        )
    else:
        common.append_node(
            "/acrn-config/hv/MMIO/MMIO64_START", "~0".upper(), allocation_etree
        )
        common.append_node("/acrn-config/hv/MMIO/MMIO64_END",
                           "~0", allocation_etree)
        common.append_node(
            "/acrn-config/hv/MMIO/HI_MMIO_START", "~0".upper(), allocation_etree
        )
...
def get_free_addr(windowslist, used, size, alignment):
    ...
    for w in windowslist:
        ...
        if window.overlaps(w):
            return window
    raise lib.error.ResourceError(
        f"Not enough address window for a device size: {size}, free address windows: {windowslist}, used address windos{used}"
    )
...
def allocate_pci_bar(board_etree, scenario_etree, allocation_etree):
    ...
    create_native_pci_hole_node(
        allocation_etree, native_low_mem, native_high_mem)
    ...
    for vm_node in vm_nodes:
        ...
        insert_ivsheme_to_dev_dict(
            scenario_etree, devdict_32bits, devdict_64bits, vm_id
        )
        insert_pt_devs_to_dev_dict(
            board_etree, vm_node, devdict_32bits, devdict_64bits)
        ...
        if vm_type is not None and lib.lib.is_pre_launched_vm(vm_type):
            low_mem = [
                AddrWindow(
                    start=PRE_LAUNCHED_VM_LOW_MEM_START,
                    end=PRE_LAUNCHED_VM_LOW_MEM_END - 1,
                )
            ]
            high_mem = [
                AddrWindow(
                    start=PRE_LAUNCHED_VM_HIGH_MEM_START,
            ...
            mem_passthrough = get_devs_mem_passthrough(
                board_etree, scenario_etree)
            ...
            used_low_mem = [
                mem for mem in used_low_mem_native if mem not in mem_passthrough
            ]
            used_high_mem = [
                mem for mem in used_high_mem_native if mem not in mem_passthrough
            ]
        else:
            # fall into else when the vm_type is post-launched vm, no mmio allocation is needed
            continue

        devdict_base_32_bits = alloc_addr(
            low_mem, devdict_32bits, used_low_mem, VBAR_ALIGNMENT
        )
        devdict_base_64_bits = alloc_addr(
            low_mem + high_mem,
            devdict_64bits,
...
def allocate_io_port(board_etree, scenario_etree, allocation_etree):
    ...
    for vm_node in vm_nodes:
        ...
        if vm_type is not None and lib.lib.is_sos_vm(vm_type):
            ...
            io_port_passthrough = get_pt_devs_io_port_passthrough(
                board_etree, scenario_etree
            )
            used_io_port_list_native = get_devs_io_port_native(
                board_etree, io_port_range_list_native
            )
            # release the passthrough devices io port address from SOS
            used_io_port_list = [
                io_port
                for io_port in used_io_port_list_native
                if io_port not in io_port_passthrough
            ]
        else:
            io_port_range_list = [
                AddrWindow(start=IO_PORT_THRESHOLD, end=IO_PORT_MAX_ADDRESS)
            ]
            used_io_port_list = get_pt_devs_io_port_passthrough_per_vm(
                board_etree, vm_node
            )

        devdict_base_io_port = alloc_addr(
            io_port_range_list, devdict_io_port, used_io_port_list, 0
        )
...
def allocate_ssram_region(board_etree, scenario_etree, allocation_etree):
    ...
    if enabled == "y":
        pre_rt_vms = common.get_node(
            "//vm[vm_type ='PRE_RT_VM']", scenario_etree)
        if pre_rt_vms is not None:
            ...
            if l3_sw_sram:
                ...
                if allocation_vm_node is None:
                    allocation_vm_node = common.append_node(
                        "/acrn-config/vm", None, allocation_etree, id=vm_id
                    )
                common.append_node("./ssram/start_gpa",
                                   hex(start), allocation_vm_node)
                common.append_node("./ssram/end_gpa",
                                   hex(end), allocation_vm_node)
...
def allocate_log_area(board_etree, scenario_etree, allocation_etree):
    ...
    if common.get_node("//capability[@id='log_area']", board_etree) is not None:
        log_area_min_len_native = int(
            common.get_node(
                f"//log_area_minimum_length/text()", board_etree), 16
        )
        log_area_start_address = (
            common.round_up(VIRT_ACPI_NVS_ADDR, 0x10000) + RESERVED_NVS_AREA
        )
        allocation_vm_node = common.get_node(
            f"/acrn-config/vm[@id = '0']", allocation_etree
        )
        if allocation_vm_node is None:
            allocation_vm_node = common.append_node(
                "/acrn-config/vm", None, allocation_etree, id="0"
            )
        common.append_node(
            "./log_area_start_address",
