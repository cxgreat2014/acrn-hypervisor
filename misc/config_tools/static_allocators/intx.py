...
def get_native_valid_irq():
    ...
    """
    val_irq = []
    irq_info_lines = board_cfg_lib.get_info(
        common.BOARD_INFO_FILE, "<AVAILABLE_IRQ_INFO>", "</AVAILABLE_IRQ_INFO>"
    )
...
def alloc_irq(irq_list):
    try:
        ...
        raise lib.error.ResourceError(
            "Cannot allocate legacy irq, the available irq list: {}, {}".format(
                e, irq_list
            )
        ) from e
...
def remove_irq(irq_list, irq):
    try:
        ...
        raise ValueError(
            "Cannot remove irq:{} from available irq list:{}, {}".format(
                irq, e, irq_list
            )
        ) from e
...
def create_vuart_irq_node(etree, vm_id, vm_type, vuart_id, irq):
    allocation_vm_node = common.get_node(
        f"/acrn-config/vm[@id = '{vm_id}']", etree)
    if allocation_vm_node is None:
        allocation_vm_node = common.append_node(
            "/acrn-config/vm", None, etree, id=vm_id
        )
    if common.get_node("./vm_type", allocation_vm_node) is None:
        common.append_node("./vm_type", vm_type, allocation_vm_node)
    if (
        common.get_node(
            f"./legacy_vuart[@id = '{vuart_id}']", allocation_vm_node)
        is None
    ):
        common.append_node("./legacy_vuart", None,
                           allocation_vm_node, id=vuart_id)

    common.append_node(
        f"./legacy_vuart[@id = '{vuart_id}']/irq", irq, allocation_vm_node
    )
...
def alloc_legacy_vuart_irqs(board_etree, scenario_etree, allocation_etree):
    ...
    for vm_node in vm_node_list:
        ...
        irq_list = (
            get_native_valid_irq()
            if vm_type == "SOS_VM"
            else [f"{d}" for d in list(range(1, 15))]
        )
        ...
        for legacy_vuart_id in legacy_vuart_id_list:
            if legacy_vuart_id == "0" and vm_type == "SOS_VM":
                ...
                legacy_vuart_node_irq_text = common.get_node(
                    f"legacy_vuart[@id = '{legacy_vuart_id}']/irq/text()", vm_node
                )
                if (
                    legacy_vuart_node_irq_text == "COM1_IRQ"
                    or legacy_vuart_node_irq_text == "SOS_COM1_IRQ"
                    or legacy_vuart_node_irq_text == "COM3_IRQ"
                    or legacy_vuart_node_irq_text == "SOS_COM3_IRQ"
                ):
                    legacy_vuart_irq = "4"
                    if legacy_vuart_irq in irq_list:
                        remove_irq(irq_list, legacy_vuart_irq)
                elif (
                    legacy_vuart_node_irq_text == "COM2_IRQ"
                    or legacy_vuart_node_irq_text == "SOS_COM2_IRQ"
                    or legacy_vuart_node_irq_text == "COM4_IRQ"
                    or legacy_vuart_node_irq_text == "SOS_COM4_IRQ"
                ):
                    legacy_vuart_irq = "3"
                    if legacy_vuart_irq in irq_list:
                        remove_irq(irq_list, legacy_vuart_irq)
...
def alloc_device_irqs(board_etree, scenario_etree, allocation_etree):
    ...
    device_nodes = set(
        board_etree.xpath(
            "//device[count(resource[@type='irq' or @type='interrupt_pin']) > 0 and count(capability[@id='MSI' or @id='MSI-X']) = 0]"
        )
    )
    ...
    for vm in scenario_etree.xpath("//vm"):
        ...
        if lib.lib.is_pre_launched_vm(vm_type):
            ...
            if pt_intx_text is not None:
                pt_intx_mapping = dict(
                    eval(f"[{pt_intx_text.replace(')(', '), (')}]"))
                for irq in pt_intx_mapping.keys():
                    irq_allocation[vm_id][irq].append(
                        "(Explicitly assigned in scenario configuration)"
                    )
            for pci_dev in vm.xpath("pci_devs/pci_dev/text()"):
                ...
                device_node = common.get_node(
                    f"//bus[@address='{hex(bdf.bus)}']/device[@address='{address}']",
                    board_etree,
                )
            ...
            lapic_passthru_flag = common.get_node(
                "guest_flags[guest_flag='GUEST_FLAG_LAPIC_PASSTHROUGH']", vm
            )
            if lapic_passthru_flag is not None and irq_allocation[vm_id]:
                for irq, devices in irq_allocation[vm_id].items():
                    print(
                        f"Interrupt line {irq} is used by the following device(s).")
                    for device in devices:
                        print(f"\t{device}")
                raise lib.error.ResourceError(
                    f"Pre-launched VM {vm_id} with LAPIC_PASSTHROUGH flag cannot use interrupt lines."
                )
    ...
    if service_vm_id >= 0:
        ...
        for device_node in device_nodes:
            ...
            if (
                bus.tag == "bus"
                and bus.get("type") == "pci"
                and device_node.get("address") is not None
            ):
    ...
    for vm1, vm2 in combinations(irq_allocation.keys(), 2):
        common_irqs = set(irq_allocation[vm1].keys()) & set(
            irq_allocation[vm2].keys())
    ...
    for vm_id, alloc in irq_allocation.items():
        ...
        if vm_node is None:
            vm_node = common.append_node(
                "/acrn-config/vm", None, allocation_etree, id=str(vm_id)
            )
        pt_intx_text = common.get_node(
            f"//vm[@id='{vm_id}']/pt_intx/text()", scenario_etree
        )
        pt_intx_mapping = (
            dict(eval(f"[{pt_intx_text.replace(')(', '), (')}]"))
            if pt_intx_text is not None
            else {}
        )
        for irq, devs in alloc.items():
            for dev in devs:
                if dev.startswith(
                    "("
                ):  # Allocation in the scenario configuration need not go to allocation.xml
                    continue
                bdf = dev.split(" ")[0]
                dev_name = f"PTDEV_{bdf}"
                dev_node = common.get_node(
                    f"device[@name = '{dev_name}']", vm_node)
                if dev_node is None:
