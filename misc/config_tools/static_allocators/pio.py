...
def alloc_pio(pio_list):
    try:
        ...
        raise lib.error.ResourceError(
            "Cannot allocate a pio base, the available pio base list:{}, {}".format(
                e, pio_list
            )
        )
...
def remove_pio(pio_list, base):
    try:
        ...
        raise ValueError(
            "Cannot remove a pio base:{} from the available pio base list:{}, {}".format(
                base, e, pio_list
            )
        ) from e
...
def assign_legacy_vuart_io_port(vm_node, legacy_vuart_id):
    legacy_vuart_base = ""
    legacy_vuart_node_base_text = common.get_node(
        f"./legacy_vuart[@id = '{legacy_vuart_id}']/base/text()", vm_node
    )
    if (
        legacy_vuart_node_base_text == "COM1_BASE"
        or legacy_vuart_node_base_text == "SOS_COM1_BASE"
    ):
        legacy_vuart_base = "0x3F8"
    elif (
        legacy_vuart_node_base_text == "COM2_BASE"
        or legacy_vuart_node_base_text == "SOS_COM2_BASE"
    ):
        legacy_vuart_base = "0x2F8"
    elif (
        legacy_vuart_node_base_text == "COM3_BASE"
        or legacy_vuart_node_base_text == "SOS_COM3_BASE"
    ):
        legacy_vuart_base = "0x3E8"
    elif (
        legacy_vuart_node_base_text == "COM4_BASE"
        or legacy_vuart_node_base_text == "SOS_COM4_BASE"
    ):
