...
def scenarios():
    ...
    if board_type is not None:
        default_xml_config = XmlConfig(
            os.path.join(current_app.config.get(
                "DEFAULT_CONFIG_PATH"), board_type)
        )
        ...
        if default_xml_config.list_all(xml_type="scenario")[0]:
            scenario_list = (
                default_xml_config.list_all(xml_type="scenario")[0],
                xml_config.list_all(xml_type="scenario")[0],
            )
        else:
            scenario_list = (
                generic_config_list[0],
                xml_config.list_all(xml_type="scenario")[0],
            )
        if default_xml_config.list_all(xml_type="uos_launcher")[0]:
            ...
            launch_list = (
                generic_config_list[1],
                xml_config.list_all(xml_type="uos_launcher")[0],
            )

    return render_template(
        "scenario.html",
        board_info_list=get_board_list(),
...
def scenario(scenario_name):
    ...
    if board_info is not None and board_type is not None:
        scenario_file_path = os.path.join(
            current_app.config.get(
                "CONFIG_PATH"), board_type, scenario_name + ".xml"
        )
    ...
    for xpath in xpath_dict:
        if xpath_dict[xpath]["enumeration"] and len(xpath.split("/")) > 2:
            scenario_item_values[",".join(xpath.split("/")[2:])] = xpath_dict[xpath][
                "enumeration"
            ]
...
def launch(launch_name):
    ...
    if board_info is not None and scenario_name is not None:
        ...
        launch_item_values = get_launch_item_values(
            os.path.join(
                current_app.config.get(
                    "CONFIG_PATH"), board_type, board_info + ".xml"
            ),
            scenario_file,
        )
    ...
    return render_template(
        ...
        launches=launch_list,
        launch=launch_name,
        root=launch_config.get_curr_root(),
...
def save_scenario():
    ...
    scenario_path = os.path.join(
        current_app.config.get("CONFIG_PATH"), board_type)
                ...
                if elem.tag in ["communication_vuart", "console_vuart"]:
                    ...
                    for key in scenario_config_data:
                        if key.find(vuart_key) >= 0:
                            ...
                            break
                    if delete_flag:
                        scenario_config.delete_curr_elem(
                            *tuple(vuart_key.split(",")))
    ...
    for key in scenario_config_data:
        ...
        if key not in [
            ...
            if isinstance(scenario_config_data[key], list):
                scenario_config.set_curr_list(
                    scenario_config_data[key], *tuple(key.split(","))
                )
            elif key.find("communication_vuart") >= 0:
                try:
                    scenario_config.get_curr_elem(*tuple(key.split(",")))
                except:
                    communication_vuart_id = key.split(
                        ",")[1].split("=")[1].strip()
                    communication_vuart_1_item = scenario_config.get_curr_elem(
                        *tuple([key.split(",")[0], "communication_vuart:id=1"])
                    )
                    communication_vuart_i_item = copy.deepcopy(
                        communication_vuart_1_item
                    )
                    communication_vuart_i_item.attrib["id"] = communication_vuart_id
                    curr_index = 0
                    elem_list = list(
                        scenario_config.get_curr_elem(key.split(",")[0]))
                    for elem in reversed(elem_list):
                        curr_index += 1
                        if elem.tag in ["communication_vuart", "console_vuart"]:
                            break
                    scenario_config.insert_curr_elem(
                        len(elem_list) - curr_index + 1,
                        communication_vuart_i_item,
                        key.split(",")[0],
                    )
                finally:
                    scenario_config.set_curr_value(
                        scenario_config_data[key], *tuple(key.split(","))
                    )
            else:
                scenario_config.set_curr_value(
        ...
        if generator == "remove_vm_kata":
            ...
            if len(vm_list) >= MAX_VM_NUM:
                return {
                    "status": "fail",
                    "error_list": {
                        "error": "Can not add a new VM. Max VM number is {}.".format(
                            MAX_VM_NUM
                        )
                    },
                }
            ...
            add_scenario_config = get_generic_scenario_config(
    ...
    scenario_config.set_curr_attr(
        "scenario", scenario_config_data["new_scenario_name"])
    ...
    rename = False
    try:
        if generator is None or not (
            generator.startswith(
                "add_vm:") or generator.startswith("remove_vm:")
        ):
    ...
    if src_path is not None and str(src_path).strip() != "":
        ...
        copyfile(
            os.path.join(
                scenario_path, scenario_config_data["new_scenario_name"] + ".xml"
            ),
            os.path.join(
                src_path, scenario_config_data["new_scenario_name"] + ".xml"),
        )
...
def save_launch():
    ...
    for key in launch_config_data:
        ...
        if key not in [
            ...
            if isinstance(launch_config_data[key], list):
                launch_config.set_curr_list(
                    launch_config_data[key], *tuple(key.split(","))
                )
            else:
                launch_config.set_curr_value(
                    launch_config_data[key], *tuple(key.split(","))
                )
        ...
        if generator.startswith("add_vm:"):
            ...
            for i in range(len(post_vm_list)):
                ...
                if add_vm_id == vm_info[0]:
                    add_launch_id = i + 1
                    break
            add_launch_config = get_generic_scenario_config(
    ...
    launch_config.set_curr_attr(
        "uos_launcher", str(len(list(launch_config.get_curr_root())))
    )
    ...
    try:
        ...
        return {
            ...
            "rename": rename,
            "error_list": {"launch config error": str(error)},
        }
    ...
    if src_path is not None and str(src_path).strip() != "":
        ...
        copyfile(
            ...
            os.path.join(
                src_path, launch_config_data["new_launch_name"] + ".xml"),
        )
...
def check_setting_exist():
    ...
    setting_path = os.path.join(
        current_app.config.get("CONFIG_PATH"), board_info)
    if src_path is not None and str(src_path).strip() != "":
        setting_path = src_path

    if "old_scenario_name" in list(config_data.keys()) and "new_scenario_name" in list(
        config_data.keys()
    ):
        if config_data["old_scenario_name"] != config_data[
            "new_scenario_name"
        ] and os.path.isfile(
            os.path.join(
                setting_path, config_data["new_scenario_name"] + ".xml")
        ):
            return {"exist": "yes"}
        else:
            return {"exist": "no"}
    elif "old_launch_name" in list(config_data.keys()) and "new_launch_name" in list(
        config_data.keys()
    ):
        if config_data["old_launch_name"] != config_data[
            "new_launch_name"
        ] and os.path.isfile(
            os.path.join(setting_path, config_data["new_launch_name"] + ".xml")
        ):
            return {"exist": "yes"}
        else:
            return {"exist": "no"}
        ...
        if os.path.isfile(
            os.path.join(setting_path, config_data["create_name"] + ".xml")
        ):
            return {"exist": "yes"}
        else:
            return {"exist": "no"}
    ...
    if create_config_data["type"] == "launch":
        ...
        if mode == "create":
            ...
            src_file_name = os.path.join(
                current_app.config.get("DEFAULT_CONFIG_PATH"),
                "generic_board",
                template_file_name + ".xml",
            )
        else:  # load
            src_file_name = os.path.join(
                current_app.config.get("DEFAULT_CONFIG_PATH"),
        ...
        if os.path.normcase(setting_path) != os.path.normcase(
            os.path.join(current_app.config.get("CONFIG_PATH"), board_type)
        ):
            copyfile(
                os.path.join(
        ...
        if mode == "create":
            ...
            elem_clos_max = scenario_config.get_curr_elem(
            ...
            cdp_enabled = scenario_config.get_curr_value(
                "hv", "FEATURES", "RDT", "CDP_ENABLED"
            )
            (num_clos_mask, num_mba_delay) = get_num_of_rdt_res(
                board_info, cdp_enabled)
            for i in range(num_clos_mask):
                scenario_config.clone_curr_elem(
                    elem_clos_max, "hv", "FEATURES", "RDT")
...
def generate_src():
    ...
    board_info_xml = os.path.join(
        current_app.config.get("CONFIG_PATH"), board_type, board_info + ".xml"
    )
    scenario_setting = generator_config_data["scenario_setting"]
    scenario_setting_xml = os.path.join(
        current_app.config.get(
            "CONFIG_PATH"), board_type, scenario_setting + ".xml"
    )
    ...
    if "launch_setting" in generator_config_data:
        ...
        launch_setting_xml = os.path.join(
            current_app.config.get(
                "CONFIG_PATH"), board_type, launch_setting + ".xml"
        )
    msg = {}
    error_list = {}
    status = "success"
    if src_type == "generate_config_src":
        try:
            ...
            error_list = ui_entry_api(
                board_info_xml, scenario_setting_xml, src_path)
        except Exception as error:
            status = "fail"
        ...
        try:
            ...
            error_list = ui_entry_api(
                board_info_xml, scenario_setting_xml, launch_setting_xml, src_path
            )
        except Exception as error:
            status = "fail"
    ...
    if request.method == "POST":
        ...
        if file and "." in file.filename and file.filename.rsplit(".", 1)[1] in ["xml"]:
            ...
            for config_name in os.listdir(default_config_path):
                if os.path.isdir(
                    os.path.join(default_config_path, config_name)
            ...
            board_type = None
            if (
                board_info_root is not None
                and "board" in board_info_root.attrib
            ...
            if not board_type:
                ...
                return {
                    "status": "Error on parsing Board info\n"
            ...
            if board_type not in board_type_list:
                info = board_type
                for generic_name in os.listdir(
                    os.path.join(default_config_path, "generic_board")
                ):
                    generic_file = os.path.join(
                        default_config_path, "generic_board", generic_name
                    )
                    if os.path.isfile(generic_file):
                        ...
                        if (
                            "scenario" not in xml_config_root.attrib
                        ...
                        xml_config.save(
                            generic_name.rsplit(".", 1)[0], user_defined=False
                        )
...
def upload_scenario():
    ...
    if request.method == "POST":
        ...
        if file and "." in file.filename and file.filename.rsplit(".", 1)[1] in ["xml"]:
            ...
            tmp_scenario_file = os.path.join(
                current_app.config.get(
                    "CONFIG_PATH"), board_type, tmp_scenario_name
            )
...
def upload_launch():
    ...
    if request.method == "POST":
        ...
        if file and "." in file.filename and file.filename.rsplit(".", 1)[1] in ["xml"]:
            ...
            tmp_launch_file = os.path.join(
                current_app.config.get(
                    "CONFIG_PATH"), board_type, tmp_launch_name
            )
            if os.path.isfile(tmp_launch_file):
                os.remove(tmp_launch_file)
            ...
            if not error_list:
                ...
                if os.path.isfile(new_launch_path):
                    new_launch_name = (
                        new_launch_name + "_" + datetime.now().strftime("%Y%m%d%H%M%S")
                    )
                    rename = True
    ...
    if scenario_config is not None and scenario_config.get_curr_root() is not None:
        for vm in list(scenario_config.get_curr_root()):
            if vm.tag == "vm" and "id" in vm.attrib:
                vm_id = vm.attrib["id"]
                for item in list(vm):
                    if item.tag == "vm_type" and item.text in [
                        "POST_STD_VM",
                        "POST_RT_VM",
                    ]:
...
def get_generic_scenario_config(scenario_config, add_vm_type=None):
    ...
    if add_vm_type is not None:
        ...
        config_path = os.path.join(
            current_app.config.get("DEFAULT_CONFIG_PATH"), "generic_board"
        )
        ...
        if os.path.isfile(os.path.join(config_path, vm_dict[add_vm_type][0] + ".xml")):
            ...
            generic_scenario_config = set_default_config(
                generic_scenario_config)
            return generic_scenario_config.get_curr_elem(vm_dict[add_vm_type][1])
        else:
            return None
    config_path = os.path.join(
        current_app.config.get("CONFIG_PATH"), "generic_board")
    generic_scenario_config = XmlConfig(config_path)
    for file in os.listdir(config_path):
        if (
            os.path.isfile(os.path.join(config_path, file))
            and os.path.splitext(file)[1] == ".xml"
        ):
            generic_scenario_config.set_curr(os.path.splitext(file)[0])
            generic_scenario_config_root = generic_scenario_config.get_curr_root()
            if (
                "scenario" in generic_scenario_config_root.attrib
...
def get_board_config(board_info):
    ...
    if board_info is not None:
        board_config = XmlConfig(
            os.path.join(current_app.config.get(
                "CONFIG_PATH"), board_info), board_info
        )
...
def get_num_of_rdt_res(board_file_name, cdp_enalbed):
    ...
    if "MBA" not in dict_rdt_res_clos_max.keys():
        num_mba_delay = 0
        if (
            "L2" not in dict_rdt_res_clos_max.keys()
            and "L3" not in dict_rdt_res_clos_max.keys()
        ):
            num_clos_max = 0
        else:
        ...
        if (
            ...
            if cdp_enalbed is not None and cdp_enalbed.strip().lower() == "y":
                for key in dict_rdt_res_clos_max.keys():
                    if key not in ["MBA"]:
                        dict_rdt_res_clos_max[key] = int(
                            dict_rdt_res_clos_max[key] / 2)
                ...
                if item.tag == "CLOS_INFO":
                    for line in item.text.split("\n"):
                        ...
                        if line.startswith("rdt resources supported"):
                            try:
                                rdt_res_supported = line.split(":")[
                                    1].split(",")
                            except:
                                pass
                        if line.startswith("rdt resource clos max:"):
                            try:
                                rdt_res_clos_max = line.split(
                                    ":")[1].split(",")
                                break
                            except:
                                pass
...
def set_default_config(config):
    ...
    return set_default_config_for_curr_layer(
        config, config_root, "/" + config_root.tag, xpath_dict
    )
...
def set_default_config_for_curr_layer(config, elem, key, xpath_dict):
    for sub_elem in list(elem):
        ...
        if (
            sub_key in xpath_dict.keys()
            and xpath_dict[sub_key]
            and xpath_dict[sub_key]["default"]
        ):
            config.set_curr_value(
                xpath_dict[sub_key]["default"], *tuple(sub_key.split("/")[2:])
            )
        set_default_config_for_curr_layer(
            config, sub_elem, sub_key, xpath_dict)
    return config
...
def get_generic_config_list():
    ...
    for config_name in os.listdir(default_config_path):
        if os.path.isfile(os.path.join(default_config_path, config_name)):
            ...
            if (
                "scenario" in xml_config_root.attrib
                and "uos_launcher" not in xml_config_root.attrib
            ):
                secenario_config_list.append(config_name.rsplit(".", 1)[0])
            elif (
                "scenario" in xml_config_root.attrib
                and "uos_launcher" in xml_config_root.attrib
            ):
