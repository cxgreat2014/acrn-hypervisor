$().ready(function () {
    ...
    $.ajax({
      ...
      success: function (result) {
        ...
        if (result.rename == true) {
          alert(
            'Scenario setting existed, import successfully with a new name: ' +
              file_name
          )
        } else {
        ...
        window.location =
          'http://' + window.location.host + '/scenario/' + file_name
      },
      error: function (e) {
        console.log(e.status)
        console.log(e.responseText)
  ...
  $('#launch_file').change(function () {
    ...
    $.ajax({
      ...
      success: function (result) {
        ...
        if (result.rename == true) {
          alert(
            'Launch setting existed, import successfully with a new name: ' +
              file_name
          )
        } else {
        ...
        window.location =
          'http://' + window.location.host + '/launch/' + file_name
      },
      error: function (e) {
        console.log(e.status)
        console.log(e.responseText)
  ...
  $('#remove_scenario').on('click', function () {
    ...
    const board_info = $('select#board_info').val()
    ...
    $.ajax({
      ...
      success: function (result) {
        ...
        if (status == 'success') {
          alert(
            'Remove current scenario setting from acrn-config app successfully.'
          )
          window.location = window.location =
            'http://' + window.location.host + '/scenario'
        } else {
          alert(
            'Remove current scenario setting from acrn-config app failed:\n' +
              info
          )
        }
      },
  ...
  $('#remove_launch').on('click', function () {
    ...
    $.ajax({
      ...
      success: function (result) {
        ...
        if (status == 'success') {
          alert(
            'Remove current launch setting from acrn-config app successfully.'
          )
          window.location = window.location =
            'http://' + window.location.host + '/launch'
        } else {
          alert(
            'Remove current launch setting from acrn-config app failed:\n' +
              info
          )
        }
      },
  ...
  $('#create_btn').on('click', function () {
    const type = $(this).data('id')
    const create_name = $('#create_name').val()
  ...
  $('#load_scenario_btn').on('click', function () {
    const type = $(this).data('id')
    const default_load_name = $('#load_scenario_name').val()
    const load_name = $('#load_scenario_name2').val()
    create_setting(type, default_load_name, load_name, 'load')
  })

  $('#load_launch_btn').on('click', function () {
    const type = $(this).data('id')
    const default_load_name = $('#load_launch_name').val()
    const load_name = $('#load_launch_name2').val()
    create_setting(type, default_load_name, load_name, 'load')
  })

  $(document).on('click', '#add_vm', function () {
    const curr_vm_id = $(this).data('id')
  ...
  $(document).on('click', '#remove_vm', function () {
    const remove_confirm_message = 'Do you want to delete this VM?'
  ...
  $(document).on('change', 'select#scenario_name', function () {
    ...
    $.ajax({
      ...
      success: function (result) {
        ...
        for (i in vm_list) {
          const option = vm_list[i][1] + ' ( ID : ' + vm_list[i][0] + ' )'
          $('select#add_launch_type')
            .append('<option value="' + option + '">' + option + '</option>')
  ...
  $(document).on(
    'change',
    "select[ID*='communication_vuart'][ID$='base']",
    function () {
      const id = $(this).attr('id')
      const value = $(this).val()
      show_com_target(id, value)
  ...
  $("select[ID*='communication_vuart'][ID$='base']").each(function (
    index,
    item
  ) {
    const id = $(item).attr('id')
  ...
  $(document).on(
    'change',
    "select[ID^='uos'][ID$='console_vuart']",
    function () {
      let id = $(this).attr('id')
      const value = $(this).val()
      if (value == 'Enable') {
  ...
  $(document).on('click', "button:contains('+')", function () {
    if ($(this).text() != '+') return
    const curr_item_id = $(this).attr('id')
    const curr_id = curr_item_id.substr(curr_item_id.lastIndexOf('_') + 1)
    const config_item = $(this).parent().parent()
    const config_item_added = config_item.clone()
    var config_vm = config_item.parent()
    const vcpu_index_list = []
    const vuart_index_list = []
    config_vm.children().each(function () {
      if ($(this).find("button:contains('+')").size() > 0) {
        const btn_add_vm_id = $(this).find("button:contains('+')").attr('id')
        if (btn_add_vm_id.indexOf('add_communication_vuart') >= 0) {
          vuart_index_list.push(
            parseInt(btn_add_vm_id.substr(btn_add_vm_id.lastIndexOf('_') + 1))
          )
        } else {
          vcpu_index_list.push(
            parseInt(btn_add_vm_id.substr(btn_add_vm_id.lastIndexOf('_') + 1))
          )
        }
      }
    ...
    const id_pre_added = curr_item_id.substr(0, curr_item_id.lastIndexOf('_'))
    config_item_added
      .find("button:contains('+')")
      .attr('id', id_pre_added + '_' + id_added)
    config_item_added
      .find("button:contains('-')")
      .attr('id', id_pre_added.replace('add_', 'remove_') + '_' + id_added)
    const curr_err_id = config_item_added.find('p').attr('id')
    config_item_added
      .find('p')
      .attr(
        'id',
        curr_err_id.replace(',' + curr_id + '_', ',' + id_added + '_')
      )
    config_item_added.find("button:contains('-')").prop('disabled', false)
    config_item_added.find('.bootstrap-select').replaceWith(function () {
      return $('select', this)
    })
    config_item_added
      .find('.selectpicker')
    ...
    if (curr_item_id.indexOf('add_communication_vuart') >= 0) {
      const config_item_target_vm = config_item.next().clone()
      const config_item_target_vuart = config_item.next().next().clone()
      const curr_vuart_id = parseInt(curr_id)
      config_item_added
        .find('label:first')
        .text(
          config_item_added
            .find('label:first')
            .html()
            .replace(curr_vuart_id, id_added)
        )
      const orig_id_list = [
        'base_label1',
        'base_label2',
        ...
        'target_vm_id',
        'target_vm_id_err',
        'target_uart_id_label1',
        'target_uart_id_label2',
        'target_uart_id_config',
        'target_uart_id',
        'target_uart_id_err'
      ]
      for (var i = 0, len = 4; i < len; i++) {
        const orig_base_item = config_item_added.find(
          '[id$=' + orig_id_list[i] + ']'
        )
        orig_base_item.attr(
          'id',
          orig_base_item
            .attr('id')
            .replace(
              curr_vuart_id + ',' + orig_id_list[i],
              id_added + ',' + orig_id_list[i]
            )
        )
      }
      for (var i = 4, len = 9; i < len; i++) {
        const orig_target_vm_item = config_item_target_vm.find(
          '[id$=' + orig_id_list[i] + ']'
        )
        orig_target_vm_item.attr(
          'id',
          orig_target_vm_item
            .attr('id')
            .replace(
      ...
      for (var i = 9, len = orig_id_list.length; i < len; i++) {
        const orig_target_vuart_item = config_item_target_vuart.find(
          '[id$=' + orig_id_list[i] + ']'
        )
        orig_target_vuart_item.attr(
          'id',
          orig_target_vuart_item
            .attr('id')
            .replace(
              curr_vuart_id + ',' + orig_id_list[i],
              id_added + ',' + orig_id_list[i]
    ...
    if (curr_item_id.indexOf('add_vcpu') >= 0) {
      var config_vm = config_item.parent()
      let curr_vcpu_index = vcpu_index_list.indexOf(parseInt(curr_id))
      let vcpu_clos_item = config_vm
        .find("label:contains('vcpu_clos')")
        .first()
        .parent()
      ...
      const vcpu_clos_item_added = vcpu_clos_item.clone()
      vcpu_clos_item_added.find('label:first').text('')
      vcpu_clos_item_added.find('.bootstrap-select').replaceWith(function () {
        return $('select', this)
      })
      vcpu_clos_item_added
        .find('.selectpicker')
        .val('default')
        .selectpicker('deselectAll')
  ...
  $(document).on('click', "button:contains('-')", function () {
    ...
    if (curr_item_id.indexOf('remove_vcpu') >= 0) {
      const config_vm = config_item.parent()
      const vcpu_index_list = []
      config_vm.children().each(function () {
        if ($(this).find("button:contains('+')").size() > 0) {
          const btn_del_vm_id = $(this).find("button:contains('+')").attr('id')
          vcpu_index_list.push(
            parseInt(btn_del_vm_id.substr(btn_del_vm_id.lastIndexOf('_') + 1))
          )
        }
      })
      var curr_item_id = $(this).attr('id')
      const curr_id = parseInt(
        curr_item_id.substr(curr_item_id.lastIndexOf('_') + 1)
      )
...
function show_com_target (id, value) {
  ...
  let id2 = id.replace('base', 'target_vm_id')
  let id3 = id.replace('base', 'target_uart_id')
  id2 = escape_str(id2)
  id3 = escape_str(id3)
...
function create_setting (type, default_name, name, mode) {
  const board_info = $('text#board_type').text()
  if (board_info == null || board_info == '') {
    alert('Please select one board info before this operation.')
    return
  }
    ...
    success: function (result) {
      ...
      if (exist == 'yes') {
        overwrite_confirm_message =
          'Setting name: ' +
          create_config.create_name +
          ' existed in ' +
          'acrn-hypervisor/../user_config/' +
          board_info +
          '/.\n' +
          ...
          success: function (result) {
            ...
            if (
              ...
              (JSON.stringify(error_list) == '{}' ||
                JSON.stringify(error_list) == 'null')
            ) {
              alert('create a new setting successfully.')
            } else {
              alert(
                'create a new setting failed. \nError list:\n' +
                  JSON.stringify(error_list)
              )
            }
...
function save_scenario (generator = null) {
  ...
  $('input').each(function () {
    ...
    const value = $(this).val()
    if (
      id.indexOf('CLOS_MASK') >= 0 ||
      id.indexOf('MBA_DELAY') >= 0 ||
      id.indexOf('IVSHMEM_REGION') >= 0
    ) {
      ...
      id != 'new_scenario_name2' &&
      id != 'board_info_file' &&
      id != 'board_info_upload' &&
      id != 'scenario_file' &&
      id != 'create_name' &&
      id != 'load_scenario_name2' &&
      id != 'load_launch_name2'
    ) {
      scenario_config[id] = value
    }
  })
  ...
  $('select').each(function () {
    ...
    const value = $(this).val()
    if (
      id.indexOf('pcpu_id') >= 0 ||
      id.indexOf('pci_dev') >= 0 ||
      id.indexOf('vcpu_clos') >= 0
    ) {
      if (id in scenario_config) {
        scenario_config[id].push(value)
      } else {
        ...
        $.ajax({
          ...
          success: function (result) {
            ...
            if (no_err == true && status == 'success') {
              const src_path = $('input#src_path').val()
              ...
              file_name = result.file_name
              validate_message =
                'Scenario setting saved successfully with name: ' +
                file_name +
                '\ninto ' +
                xml_path
              if (result.rename == true) {
                validate_message =
                  'Scenario setting existed, saved successfully with a new name: ' +
                  file_name +
                  '\ninto acrn-hypervisor/../user_config/' +
                  board_info +
                  '/.'
              }
              if (generator == 'generate_config_src') {
                ...
                if (src_path == null || src_path == '') {
                  overwrite_confirm_message =
                    'The Source Path for configuration files is not set.\n' +
                    ...
                    success: function (result) {
                      ...
                      if (
                        ...
                        if (src_path == null || src_path == '') {
                          alert(
                            generator +
                              ' successfully into acrn-hypervisor/build/hypervisor/configs/board/ and acrn-hypervisor/build/hypervisor/configs/scenarios/ '
                          )
                        } else {
...
function save_launch (generator = null) {
  const board_info = $('text#board_type').text()
  const scenario_name = $('select#scenario_name').val()
  if (
    board_info == null ||
    board_info == '' ||
    scenario_name == null ||
  ...
  $('input').each(function () {
    ...
    if (
      id.indexOf('virtio_devices,network') >= 0 ||
      id.indexOf('virtio_devices,block') >= 0 ||
  ...
  $('select').each(function () {
    ...
    if (
      id.indexOf('pcpu_id') >= 0 ||
      id.indexOf('shm_region') >= 0 ||
      id.indexOf('communication_vuart') >= 0 ||
      id.indexOf('pci_dev') >= 0
    ) {
      if (id in launch_config) {
        launch_config[id].push(value)
  ...
  $.ajax({
    ...
    success: function (result) {
      ...
      if (exist == 'yes') {
        overwrite_confirm_message =
          'Setting name: existed in the XML path\n' +
          'Do you want to overwrite it?\nClick OK to overwrite it; click Cancel to rename it.'
        if (!confirm(overwrite_confirm_message)) {
          create_flag = false
        }
        ...
        $.ajax({
          ...
          success: function (result) {
            ...
            if (no_err == true && status == 'success') {
              ...
              file_name = result.file_name
              validate_message =
                'Launch setting saved successfully with name: ' +
                file_name +
                '\nto ' +
                xml_path
              if (result.rename == true) {
                validate_message =
                  'Launch setting existed, saved successfully with a new name: ' +
                  file_name +
                  '\nto  ' +
                  xml_path
              }
              if (generator == 'generate_launch_script') {
                ...
                if (src_path == null || src_path == '') {
                  overwrite_confirm_message =
                    'The Source Path for launch scripts is not set.\n' +
                    'Do you want to generate them into the default path: acrn-hypervisor/../user_config/' +
                    board_info +
                    '/output/,\n' +
                    'and overwrite the old ones?\nClick OK to overwrite them; click Cancel to edit the Source Path.'
                  if (!confirm(overwrite_confirm_message)) {
