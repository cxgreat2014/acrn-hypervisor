            ...
            for line in src.readlines():
                if re.search(p_xsdt_addr, line):
                    lines.append(
                        re.sub(
                            p_xsdt_addr,
                            "XSDT Address : {0:016X}".format(ACPI_XSDT_ADDR),
                            line,
                        )
                    )
                else:
            ...
            for line in src.readlines():
                if re.search(p_fadt_addr, line):
                    lines.append(
                        re.sub(
                            p_fadt_addr,
                            "ACPI Table Address   0 : {0:016X}".format(
                                ACPI_FADT_ADDR),
                            line,
                        )
                    )
                    ...
                    lines.append(
                        re.sub(
                            p_mcfg_addr,
                            "ACPI Table Address   1 : {0:016X}".format(
                                ACPI_MCFG_ADDR),
                            line,
                        )
                    )
                    ...
                    lines.append(
                        re.sub(
                            p_madt_addr,
                            "ACPI Table Address   2 : {0:016X}".format(
                                ACPI_MADT_ADDR),
                            line,
                        )
                    )
                    ...
                    if "TPM2" in passthru_devices:
                        lines.append(
                            re.sub(
                                p_tpm2_addr,
                                "ACPI Table Address   3 : {0:016X}".format(
                                    ACPI_TPM2_ADDR
                                ),
                                line,
                            )
                    ...
                    if "PTCT" in passthru_devices or "RTCT" in passthru_devices:
                        lines.append(
                            re.sub(
                                p_rtct_addr,
                                "ACPI Table Address   4 : {0:016X}".format(
                                    ACPI_RTCT_ADDR
                                ),
                                line,
                            )
            ...
            for line in src.readlines():
                if re.search(p_facs_addr, line):
                    lines.append(
                        re.sub(
                            p_facs_addr,
                            "FACS Address : {0:08X}".format(ACPI_FACS_ADDR),
                            line,
                        )
                    )
                    ...
                    lines.append(
                        re.sub(
                            p_dsdt_addr,
                            "DSDT Address : {0:08X}".format(ACPI_DSDT_ADDR),
                            line,
                        )
                    )
                else:
            ...
            for line in src.readlines():
                if re.search(p_base_addr, line):
                    ...
                    lines.append(
                        re.sub(
                            p_segment_group_num,
                            "Segment Group Number : {0:04X}".format(0),
                            line,
                        )
                    )
                    ...
                    lines.append(
                        re.sub(
                            p_start_bus_num,
                            "Start Bus Number : {0:02X}".format(0),
                            line,
                        )
                    )
                    ...
                    lines.append(
                        re.sub(
                            p_end_bus_num, "End Bus Number : {0:02X}".format(
                                0xFF), line
                        )
                    )
                else:
...
def gen_madt(dest_vm_acpi_path, max_cpu_num, apic_ids):
    ...
    # dup len
    p_lapic_len = r"\[0001\]                       Length : ([0-9a-fA-F]{2})"
    p_lapic_len_index = 0
    p_lapic_flags_index = 0
    p_lapic_process_id = (
        r"\[0001\]                 Processor ID : (\d+)"  # dup processor
    )
            ...
            for line in src.readlines():
                if re.search(p_lapic_addr, line):
                    ...
                    lines.append(
                        re.sub(
                            p_lapic_addr,
                            "Local Apic Address : {0:08X}".format(0xFEE00000),
                            line,
                        )
                    )
                    ...
                    if lapic_index == 1 and flags_index == 0:
                        lines.append(
                            re.sub(
                                p_flags,
                                "[0004]        Flags (decoded below) : {0:08X}".format(
                                    0x1
                                ),
                                line,
                            )
                    ...
                    lines.append(
                        re.sub(
                            p_lapic_id,
                            "Local Apic ID : {0:02X}".format(apic_ids[0]),
                            line,
                        )
                    )
                    ...
                    lines.append(
                        re.sub(
                            p_ioapic_type,
                            "Subtable Type : {0:02X} [I/O APIC]".format(
                    ...
                    lines.append(
                        re.sub(p_ioapic_id,
                               "I/O Apic ID : {0:02X}".format(0x01), line)
                    )
                elif re.search(p_ioapic_addr, line):
                    lines.append(
                        re.sub(
                            p_ioapic_addr,
                            "[0004]                      Address : {0:02X}".format(
                                VIOAPIC_BASE
                            ),
                            line,
                        )
                    ...
                    if lapic_nmi_index == 1:
                        lines.append(
                            re.sub(
                                p_lapic_nmi_type,
                                "Subtable Type : {0:02X} [Local APIC NMI]".format(
                    ...
                    if lapic_nmi_index == 1 and p_lapic_nmi_flags_index == 0:
                        lines.append(
                            re.sub(
                                p_lapic_nmi_flags,
                                "[0002]        Flags (decoded below) : {0:04X}".format(
                                    0x5
                                ),
                                line,
                            )
                    ...
                    if lapic_nmi_index == 1:
                        lines.append(
                            re.sub(
                                p_lapic_nmi_lint,
                                "Interrupt Input LINT : {0:02X}".format(0x1),
                                line,
                            )
                        )
                    else:
            ...
            for line in src.readlines():
                if re.search(p_control_addr, line):
                    lines.append(
                        re.sub(
                            p_control_addr,
                            "Control Address : {0:016X}".format(0xFED40040),
                            line,
                        )
                    )
                    ...
                    lines.append(
                        re.sub(
                            p_start_method, "Start Method : {0:02X}".format(
                                0x7), line
                        )
                    )
                else:
...
def encode_eisa_id(s):
    ...
    encoded = [
        ...
        | (
            chars[1] >> 3
        ),  # Bit 6:2 is char[0]; Bit 1:0 is the higher 2 bits of char[1].
        ((chars[1] & 0x7) << 5)
        # Bit 7:5 is the lower 3 bits of char[1]; Bit 4:0 is char[2].
        | (chars[2]),
        # Bit 7:4 is digits[0]; Bit 3:0 is digits[1]
        (digits[0] << 4) | (digits[1]),
        # Bit 7:4 is digits[2]; Bit 3:0 is digits[2]
        (digits[2] << 4) | (digits[3]),
    ]
...
def gen_root_pci_bus(path, prt_packages):
    ...
    tree = builder.DefDevice(
        ...
        path,
        builder.TermList(
            builder.DefName("_HID", builder.DWordConst(
                encode_eisa_id("PNP0A08"))),
            builder.DefName("_CID", builder.DWordConst(
                encode_eisa_id("PNP0A03"))),
        ...
        if device not in result:
            ...
            for node in device.findall("dependency"):
                if node.get("type") in types_in_scope:
                    peer_device = common.get_node(
                        f"//device[acpi_object='{node.text}']", board_etree
                    )
...
class ObjectCollector:
    ...
    def __discard_external_objects(tree):
        ...
        objects_tree.children = list(
            filter(lambda x: x.label != "DefExternal", objects_tree.children)
        )
        return tree
    ...
    def __get_scope_contents(self, termlist, path):
        ...
        for scope in scopes:
            for term in ret:
                if (
                    term.label in ["DefScope", "DefDevice"]
                    and term.NameString.value == scope
                ):
                    ret = term.TermList.children
                    break
            else:
                tree = builder.DefScope(
                    builder.PkgLength(), scope, builder.TermList())
                ret.append(tree)
    ...
    def get_term_list(self):
        ...
        for scope, objects in sorted(self.__objects.items(), key=lambda p: p[0]):
            if scope.startswith("\\"):
                ...
                print(
                    f"Relative scope is unexpected: {scope}. The objects will be added to the root scope."
                )
                acc.extend(objects)
...
def gen_dsdt(board_etree, scenario_etree, allocation_etree, vm_id, dest_path):
    ...
    for pci_dev in scenario_etree.xpath(f"//vm[@id='{vm_id}']/pci_devs/pci_dev/text()"):
        ...
        if m:
            ...
            if device_node is not None and alloc_node is not None:
                assert (
                    int(alloc_node.find("bus").text, 16) == 0
                ), "Virtual PCI devices must be on bus 0."
                ...
                if template is not None:
                    tree = parse_tree(
                        "DefDevice", bytes.fromhex(template.text))
                    vaddr = (vdev << 16) | vfunc
                    add_or_replace_object(
                        tree, builder.DefName(
                            "_ADR", builder.build_value(vaddr))
                    )
    ...
    has_tpm2 = common.get_node(
        f"//vm[@id='{vm_id}']//TPM2/text()", scenario_etree)
    if has_tpm2 == "y":
        # TPM2 devices should have "MSFT0101" as hardware id or compatible ID
        template = common.get_node(
            "//device[@id='MSFT0101']/aml_template", board_etree)
        if template is None:
            template = common.get_node(
    ...
    s5_object = builder.DefName(
        ...
        builder.DefPackage(
            ...
            2,
            builder.PackageElementList(
                builder.build_value(5), builder.build_value(0)),
        ),
    )
...
def main(args):
    ...
    if out is None or out == "":
        DEST_ACPI_PATH = os.path.join(
            VM_CONFIGS_PATH, "scenarios", scenario_name)
    else:
        DEST_ACPI_PATH = os.path.join(
            common.SOURCE_ROOT_DIR, out, "scenarios", scenario_name
        )
    ...
    for vm in scenario_root.findall("vm"):
        ...
        if (vm_type_node is not None) and (
            vm_type_node.text in ["PRE_STD_VM", "SAFETY_VM", "PRE_RT_VM"]
        ):
            dict_passthru_devices[vm_id] = []
            for pci_dev_node in vm.findall("pci_devs/pci_dev"):
                if (
                    pci_dev_node is not None
                    and pci_dev_node.text is not None
                    and pci_dev_node.text.strip()
                ):
    ...
    for vm_id, passthru_devices in dict_passthru_devices.items():
        if kern_args[int(vm_id)].find("reboot=acpi") == -1 and kern_type[
            int(vm_id)
        ] in ["KERNEL_BZIMAGE"]:
        ...
        if PASSTHROUGH_RTCT is True and vm_id == PRELAUNCHED_RTVM_ID:
            for f in RTCT:
                if os.path.isfile(
                    os.path.join(VM_CONFIGS_PATH,
                                 "acpi_template", board_type, f)
                ):
        ...
        if vm_id in dict_pcpu_list:
            ...
            print(
                "generate ASL code of ACPI tables for VM {} into {}".format(
