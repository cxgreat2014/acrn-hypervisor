...
def asl_to_aml(dest_vm_acpi_path, dest_vm_acpi_bin_path):
    ...
    for acpi_table in ACPI_TABLE_LIST:
        if acpi_table[0] == "tpm2.asl":
            if "tpm2.asl" in os.listdir(dest_vm_acpi_path):
                ...
                if rc == 0 and os.path.isfile(
                    os.path.join(dest_vm_acpi_path, acpi_table[1])
                ):
                    shutil.move(
                        os.path.join(dest_vm_acpi_path, acpi_table[1]),
    ...
    os.chdir(curr_path)
    if not rmsg:
        print("compile ACPI ASL code to {} successfully".format(
...
def tpm2_acpi_gen(acpi_bin, board_etree, scenario_etree, allocation_etree):
    tpm2_enabled = common.get_node(
        "//vm[@id = '0']/mmio_resources/TPM2/text()", scenario_etree
    )
    if tpm2_enabled is not None and tpm2_enabled == "y":
        tpm2_node = common.get_node(
            "//device[@id = 'MSFT0101' or compatible_id = 'MSFT0101']", board_etree
        )
        if tpm2_node is not None:
            _data_len = (
                0x4C
                if common.get_node("//capability[@id = 'log_area']", board_etree)
                is not None
                else 0x40
            )
            ...
            ctype_data.start_method = int(
                common.get_node(
                    "//capability[@id = 'start_method']/value/text()", tpm2_node
                ),
                16,
            )
            start_method_parameters = tpm2_node.xpath("//parameter/text()")
            for i in range(len(start_method_parameters)):
                ctype_data.start_method_specific_parameters[i] = int(
                    start_method_parameters[i], 16
                )
            if (
                common.get_node("//capability[@id = 'log_area']", board_etree)
                is not None
            ):
                ctype_data.log_area_minimum_length = int(
                    common.get_node(
                        "//log_area_minimum_length/text()", allocation_etree
                    ),
                    16,
                )
                ctype_data.log_area_start_address = int(
                    common.get_node(
                        "//log_area_start_address/text()", allocation_etree
                    ),
                    16,
                )
            ctype_data.header.checksum = (
                ~(sum(lib.cdata.to_bytes(ctype_data))) + 1
            ) & 0xFF
            acpi_bin.seek(ACPI_TPM2_ADDR_OFFSET)
            acpi_bin.write(lib.cdata.to_bytes(ctype_data))
        else:
            logging.warning(
                "Passtrhough tpm2 is enabled in scenario but the device is not presented on board."
            )
            logging.warning(
                "Check there is tpm2 device on board and re-generate the xml using board inspector with --advanced option."
            )
...
def aml_to_bin(
    ...
    with open(acpi_bin_file, "wb") as acpi_bin:
        ...
        with open(
            os.path.join(dest_vm_acpi_bin_path, ACPI_TABLE_LIST[1][1]), "rb"
        ) as asl:
            acpi_bin.write(asl.read())

        acpi_bin.seek(ACPI_FADT_ADDR_OFFSET)
        with open(
            os.path.join(dest_vm_acpi_bin_path, ACPI_TABLE_LIST[2][1]), "rb"
        ) as asl:
            acpi_bin.write(asl.read())

        acpi_bin.seek(ACPI_MCFG_ADDR_OFFSET)
        with open(
            os.path.join(dest_vm_acpi_bin_path, ACPI_TABLE_LIST[3][1]), "rb"
        ) as asl:
            acpi_bin.write(asl.read())

        acpi_bin.seek(ACPI_MADT_ADDR_OFFSET)
        with open(
            os.path.join(dest_vm_acpi_bin_path, ACPI_TABLE_LIST[4][1]), "rb"
        ) as asl:
            acpi_bin.write(asl.read())

        acpi_bin.seek(ACPI_DSDT_ADDR_OFFSET)
        with open(
            os.path.join(dest_vm_acpi_bin_path, ACPI_TABLE_LIST[6][1]), "rb"
        ) as asl:
            acpi_bin.write(asl.read())
        ...
        if "PTCT" in os.listdir(dest_vm_acpi_path):
            ...
            with open(
                os.path.join(dest_vm_acpi_bin_path,
                             ACPI_TABLE_LIST[7][1]), "rb"
            ) as asl:
                acpi_bin.write(asl.read())
            ...
            with open(
                os.path.join(dest_vm_acpi_bin_path,
                             ACPI_TABLE_LIST[8][1]), "rb"
            ) as asl:
                acpi_bin.write(asl.read())

        vm_id = acpi_bin_name.split(".")[0].split("ACPI_VM")[1]
        if vm_id == "0":
            tpm2_acpi_gen(acpi_bin, board_etree,
                          scenario_etree, allocation_etree)

        acpi_bin.seek(0xFFFFF)
        acpi_bin.write(b"\0")
    shutil.move(acpi_bin_file, os.path.join(
        dest_vm_acpi_bin_path, "..", acpi_bin_name))
    print(
        "write ACPI binary to {} successfully".format(
            os.path.join(dest_vm_acpi_bin_path, "..", acpi_bin_name)
        )
    )
...
def exec_command(cmd):
    ...
    print("exec: ", cmd)
    p_compile_result = (
        r"Compilation successful. (\d+) Errors, (\d+) Warnings, (\d+) Remarks"
    )
    cmd_list = cmd.split()
    rc = 1
    r_lines = []
...
def main(args):
    ...
    if args.asl is None:
        ...
        DEST_ACPI_PATH = os.path.join(
            common.SOURCE_ROOT_DIR, args.asl, "scenarios", scenario_name
        )
    if args.out is None:
        hypervisor_out = os.path.join(
            common.SOURCE_ROOT_DIR, "build", "hypervisor")
    else:
        hypervisor_out = args.out
    DEST_ACPI_BIN_PATH = os.path.join(hypervisor_out, "acpi")

    allocation_etree = lxml.etree.parse(
        os.path.join(hypervisor_out, "configs", "allocation.xml")
    )
    ...
    if not check_iasl():
        print(
            "Please install iasl tool with version >= 20190703 from https://www.acpica.org/downloads before ACPI generation."
        )
        return 1

    for config in os.listdir(DEST_ACPI_PATH):
        if os.path.isdir(os.path.join(DEST_ACPI_PATH, config)) and config.startswith(
            "ACPI_VM"
        ):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        usage="python3 bin_gen.py --board [board] --scenario [scenario]"
    ...
    parser.add_argument(
        "--board",
        required=True,
        help="the XML file summarizing characteristics of the target board",
    )
    parser.add_argument(
        "--scenario",
        required=True,
        help="the XML file specifying the scenario to be set up",
    )
    parser.add_argument(
        "--asl", default=None, help="the input folder to store the ACPI ASL code. "
    )
    parser.add_argument(
        "--out",
        default=None,
        help="the output folder to store the ACPI binary code. "
