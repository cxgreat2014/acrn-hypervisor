...
def gen_dmar_structure(config):
    ...
    dmar_info_lines = board_cfg_lib.get_info(
    ...
    if not dmar_info_lines:
        ...
        print(
            '#error "DMAR info is not available, please set ACPI_PARSE_ENABLED to y. \\',
            file=config,
        )
    ...
    for i_drhd_cnt in range(drhd_cnt):
        ...
        print(
            "static struct dmar_dev_scope drhd{}_dev_scope[DRHD{}_DEV_CNT] = {{".format(
                i_drhd_cnt, i_drhd_cnt
            ),
            file=config,
        )
        for i_dev_cnt in range(dev_cnt):
            print("\t{", file=config)
            print(
                "\t\t.type   = DRHD{}_DEVSCOPE{}_TYPE,".format(
                    i_drhd_cnt, i_dev_cnt),
                file=config,
            )
            print(
                "\t\t.id     = DRHD{}_DEVSCOPE{}_ID,".format(
                    i_drhd_cnt, i_dev_cnt),
                file=config,
            )
            print(
                "\t\t.bus    = DRHD{}_DEVSCOPE{}_BUS,".format(
                    i_drhd_cnt, i_dev_cnt),
                file=config,
            )
            print(
                "\t\t.devfun = DRHD{}_DEVSCOPE{}_PATH,".format(
                    i_drhd_cnt, i_dev_cnt),
                file=config,
            )
            print("\t},", file=config)
    ...
    print(
        "static struct dmar_drhd drhd_info_array[DRHD_COUNT] = {", file=config)
    for i_drhd_cnt in range(drhd_cnt):
        print("\t{", file=config)
        print("\t\t.dev_cnt       = DRHD{}_DEV_CNT,".format(
            i_drhd_cnt), file=config)
        print("\t\t.segment       = DRHD{}_SEGMENT,".format(
            i_drhd_cnt), file=config)
        print("\t\t.flags         = DRHD{}_FLAGS,".format(i_drhd_cnt), file=config)
        print("\t\t.reg_base_addr = DRHD{}_REG_BASE,".format(
            i_drhd_cnt), file=config)
        print("\t\t.ignore        = DRHD{}_IGNORE,".format(
            i_drhd_cnt), file=config)
        print("\t\t.devices       = drhd{}_dev_scope".format(
            i_drhd_cnt), file=config)
        print("\t},", file=config)
...
def populate_clos_mask_msr(rdt_res, cat_mask_list, config):
    ...
    for cat_mask in cat_mask_list:
        ...
        print(
            "\t\t.msr_index = MSR_IA32_{0}_MASK_BASE + {1},".format(
                rdt_res, idx),
            file=config,
        )
        print("\t},", file=config)
...
def gen_rdt_res(config):
    ...
    cat_mask_list = common.get_hv_item_tag(
        common.SCENARIO_INFO_FILE, "FEATURES", "RDT", "CLOS_MASK"
    )
    mba_delay_list = common.get_hv_item_tag(
        common.SCENARIO_INFO_FILE, "FEATURES", "RDT", "MBA_DELAY"
    )

    if (
        common_clos_max > MSR_IA32_L2_MASK_END - MSR_IA32_L2_MASK_BASE
        or common_clos_max > MSR_IA32_L3_MASK_END - MSR_IA32_L3_MASK_BASE
    ):
        err_dic[
            "board config: generate board.c failed"
        ] = "CLOS MAX should be less than reserved adress region length of L2/L3 cache"
        return err_dic

    print("\n#ifdef CONFIG_RDT_ENABLED", file=config)
    if len(rdt_resources) == 0 or common_clos_max == 0:
        print(
            "struct platform_clos_info platform_{0}_clos_array[MAX_CACHE_CLOS_NUM_ENTRIES];".format(
                "l2"
            ),
            file=config,
        )
        ...
        print(
            "struct platform_clos_info platform_{0}_clos_array[MAX_MBA_CLOS_NUM_ENTRIES];".format(
                "mba"
            ),
            file=config,
        )
        ...
        for idx, rdt_res in enumerate(rdt_resources):
            if rdt_res == "L2":
                rdt_res_str = "l2"
                print(
                    "struct platform_clos_info platform_{0}_clos_array[{1}] = {{".format(
                        rdt_res_str, "MAX_CACHE_CLOS_NUM_ENTRIES"
                    ),
                    file=config,
                )
                ...
                print(
                    "struct platform_clos_info platform_{0}_clos_array[{1}] = {{".format(
                        rdt_res_str, "MAX_MBA_CLOS_NUM_ENTRIES"
                    ),
                    file=config,
                )
                err_dic = populate_mba_delay_mask(
                    rdt_res, mba_delay_list, config)
                print("};\n", file=config)
                res_present[RDT.MBA.value] = 1
            else:
                err_dic[
                    "board config: generate board.c failed"
                ] = "The input of {} was corrupted!".format(common.BOARD_INFO_FILE)
                return err_dic

        if res_present[RDT.L2.value] == 0:
            print(
                "struct platform_clos_info platform_{0}_clos_array[{1}];".format(
                    "l2", "MAX_CACHE_CLOS_NUM_ENTRIES"
                ),
                file=config,
            )
    ...
    for data_l in data_lines:
        if line_i == 0:
            if "not available" in data_l:
                ...
                print(
                    "static const struct acrn_{}state_data board_cpu_{}x[0];".format(
                ...
                print(
                    "static const struct acrn_{}state_data board_cpu_{}x[{}] = {{".format(
                        domain_str, domain_str, data_len
                    ),
                    file=config,
                )
...
def gen_px_cx(config):
    ...
    cx_lines = board_cfg_lib.get_info(
        common.BOARD_INFO_FILE, "<CX_INFO>", "</CX_INFO>")
    px_lines = board_cfg_lib.get_info(
        common.BOARD_INFO_FILE, "<PX_INFO>", "</PX_INFO>")
...
def gen_pci_hide(config):
    ...
    if (
        ...
        in list(board_cfg_lib.KNOWN_HIDDEN_PDEVS_BOARD_DB.keys())
        and board_cfg_lib.KNOWN_HIDDEN_PDEVS_BOARD_DB[board_cfg_lib.BOARD_NAME] != 0
    ):
        hidden_pdev_list = board_cfg_lib.KNOWN_HIDDEN_PDEVS_BOARD_DB[
            board_cfg_lib.BOARD_NAME
        ]
        ...
        print(
            "const union pci_bdf plat_hidden_pdevs[MAX_HIDDEN_PDEVS_NUM];", file=config
        )
    ...
    for dev, bdf_list in known_caps_pci_devs.items():
        if dev == "VMSIX":
            ...
            for i in range(bdf_list_len):
                ...
                print(
                    "#define VMSIX_ON_MSI_DEV{}\t.bdf.bits = {{.b = 0x{}U, .d = 0x{}U, .f =0x{}U}},".format(
                        i, b, d, f
                    ),
                    file=config,
                )
