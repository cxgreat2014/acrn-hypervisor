...
class MmioWindow(namedtuple("MmioWindow", ["start", "end"])):
    ...
    def overlaps(self, other):
        if not isinstance(other, MmioWindow):
            raise TypeError(
                "overlaps() other must be an MmioWindow: {}".format(type(other))
            )
...
def get_devs_per_vm_with_key(pci_devs, keywords):
    ...
    for vm_i, pci_devs_list in pci_devs.items():
        devicelist[vm_i] = [d for d in pci_devs_list if d in keywords]
    return devicelist
    ...
    if bdf in pci_bar_dic.keys():
        ...
        if board_cfg_lib.is_matched_board(("ehl-crb-b")):
            for vm_i in pci_devs_per_vm:
                if bdf in pci_devs_per_vm[vm_i]:
                    if (
                        scenario_cfg_lib.VM_DB[common.VM_TYPES[vm_i]
                                               ]["load_type"]
                        == "PRE_LAUNCHED_VM"
                    ):
                        is_vmsix = True
                        ...
                        free = get_free_mmio(
                            [MmioWindow(start=common.SIZE_2G,
                                        end=common.SIZE_4G - 1)],
                        ...
                        free = MmioWindow(
                            free_vbar_start_addr, free_vbar_end_addr)
        ...
        for bar_i in bar_list:
            if not bar_attr.remappable:
                print(
                    "/* TODO: add {} 64bit BAR support */".format(
                        tmp_sub_name),
                    file=config,
                )
        ...
        if is_vmsix:
            ...
            print(
                "{}.vbar_base[{}] = {}{}UL".format(
                    align, next_bar_idx, ptdev_mmio_str, hex(free.start)
                ),
                file=config,
            )
        print("", file=config)
    ...
    for vm_id in vuarts.keys():
        ...
        for vuart_id in vuarts[vm_id].keys():
            ...
            if scenario_cfg_lib.VM_DB[vm_type]["load_type"] == "SOS_VM":
                ...
                free_bar0 = MmioWindow(
                    free_bar0_start_addr, free_bar0_end_addr)
                ...
                free_bar1 = MmioWindow(
                    free_bar1_start_addr, free_bar1_end_addr)
            ...
            print(
                "#define VM%s"
                % (str(vm_id) + "_VUART_%-28s")
                % (str(vuart_id) + "_VBAR"),
                "       .vbar_base[0] = {:#x}UL, \\".format(free_bar0.start),
                file=config,
            )
            print(
                "{}.vbar_base[1] = {:#x}UL".format(" " * 54, free_bar1.start),
                file=config,
            )
            print("", file=config)
    ...
    for vm_id, vm_type in common.VM_TYPES.items():
        ...
        for shm in shmems:
            ...
            if scenario_cfg_lib.VM_DB[vm_type]["load_type"] == "SOS_VM":
                ...
                free_bar2 = get_free_mmio(
                    sos_mmio_range,
                    mmiolist_per_vm[vm_id],
                    BAR2_SHEMEM_ALIGNMENT + int_size,
                )
                free_bar2_start_addr = (
                    common.round_up(free_bar2.start,
                                    BAR2_SHEMEM_ALIGNMENT) + 0xC
                )
                free_bar2_end_addr = free_bar2_start_addr + int_size - 1
                free_bar2 = MmioWindow(
                    free_bar2_start_addr, free_bar2_end_addr)
                mmiolist_per_vm[vm_id].append(free_bar2)
                mmiolist_per_vm[vm_id].sort()
                print(
                    "#define SOS_IVSHMEM_DEVICE_%-19s" % (str(idx) + "_VBAR"),
                    "       .vbar_base[0] = {:#x}UL, \\".format(
                        free_bar0.start),
                    file=config,
                )
                print(
                    "{}.vbar_base[1] = {:#x}UL, \\".format(
                        " " * 54, free_bar1.start),
                    file=config,
                )
                print(
                    "{}.vbar_base[2] = {:#x}UL".format(
                        " " * 54, free_bar2.start),
                    file=config,
                )
                print("", file=config)
                ...
                mmioRange = [MmioWindow(
                    start=common.SIZE_2G, end=common.SIZE_4G - 1)]
                # vbar[0] for shared memory is 4k
                free_bar0 = get_free_mmio(
                    mmioRange,
                    mmiolist_per_vm[vm_id],
                ...
                # vbar[2] for shared memory is specified size in MB
                free_bar2 = get_free_mmio(
                    mmioRange, mmiolist_per_vm[vm_id], BAR2_SHEMEM_ALIGNMENT + int_size
                )
                free_bar2_start_addr = (
                    common.round_up(free_bar2.start,
                                    BAR2_SHEMEM_ALIGNMENT) + 0xC
                ...
                mmiolist_per_vm[vm_id].sort()
                print(
                    "#define IVSHMEM_DEVICE_%-23s" % (str(idx) + "_VBAR"),
                    "       .vbar_base[0] = {:#x}UL, \\".format(
                        free_bar0.start),
                    file=config,
                )
...
def get_free_mmio(windowslist, used, size):
    ...
    for w in windowslist:
        ...
        if window.overlaps(w):
            return window
    raise ValueError(
        "Not enough mmio window for a device size {}: {}".format(size, window)
    )
...
def generate_file(config):
    ...
    non_matching_mmios = [
        w for w in non_matching_mmios if any((w.overlaps(w2) for w2 in matching_mmios))
    ]
    ...
    ivshmem_enabled = common.get_hv_item_tag(
    ...
    for cnt_sub_name in board_cfg_lib.SUB_NAME_COUNT.keys():
        ...
        for bdf, bar_attr in board_cfg_lib.PCI_DEV_BAR_DESC.pci_dev_dic.items():
            ...
            write_vbar(
                i_cnt,
                bdf,
                board_cfg_lib.PCI_DEV_BAR_DESC.pci_bar_dic,
                bar_attr,
                pci_devs_per_vm,
                mmiolist_per_vm,
                matching_mmios,
