    ...
    for thread in threads:
        ...
        while True:
            ...
            if n is None:
                ...
                add_child(
                    n, "complex_cache_indexing", str(
                        leaf_4.complex_cache_indexing)
                )
                ...
                if cache_level == 2:
                    leaf_10 = (
                        parse_cpuid(0x10, 2, cpu_id)
                        if leaf_10.l2_cache_allocation == 1
                        else None
                    )
                elif cache_level == 3:
                    leaf_10 = (
                        parse_cpuid(0x10, 1, cpu_id)
                        if leaf_10.l3_cache_allocation == 1
                        else None
                    )
                else:
                    leaf_10 = None
                if leaf_10 is not None:
                    ...
                    add_child(
                        cap, "capacity_mask_length", str(
                            leaf_10.capacity_mask_length)
                    )
            ...
            for entry in rtct.entries:
                if entry.type == acpiparser.rtct.ACPI_RTCT_V1_TYPE_SoftwareSRAM:
                    ...
                    if cache_node is None:
                        ...
                        continue
                    cap = add_child(cache_node, "capability",
                                    None, id="Software SRAM")
                    add_child(cap, "start", "0x{:08x}".format(entry.base))
                    add_child(
                        cap, "end", "0x{:08x}".format(
                            entry.base + entry.size - 1)
                    )
