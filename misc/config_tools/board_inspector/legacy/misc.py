...
def is_bdf_format(bdf):
    ...
    if (
        len(bdf) == 7
        and len(bdf.split(":")[0]) == 2
        and len(bdf.split(":")[1].split(".")[0]) == 2
        and len(bdf.split(".")[1]) == 1
    ):
        is_bdf = True
...
def dump_ttys_info(ttys_list, config):
    for ttys in ttys_list:
        ...
        if ttys_type[serial_type] == "PORT":
            ...
            print(
                "\tseri:{} type:portio base:{} irq:{}".format(ttys, base, irq),
                file=config,
            )
            ...
            if bdf:
                print(
                    '\tseri:{} type:mmio base:{} irq:{} bdf:"{}"'.format(
                        ttys, base, irq, bdf
                    ),
                    file=config,
                )
                ...
                print(
                    "\tseri:{} type:mmio base:{} irq:{}".format(
                        ttys, base, irq),
                    file=config,
                )
