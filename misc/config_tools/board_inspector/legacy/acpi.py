...
SYS_PATH = ["/proc/cpuinfo", "/sys/firmware/acpi/tables/",
...
def write_reset_reg(rst_reg_addr, rst_reg_space_id, rst_reg_val, config):
    ...
    print(
        "\t#define RESET_REGISTER_ADDRESS  0x{:0>2X}UL".format(rst_reg_addr),
        file=config,
    )
    print(
        "\t#define RESET_REGISTER_SPACE_ID {0}".format(
            SPACE_ID[rst_reg_space_id]),
        file=config,
    )
    ...
    for key, offset in FACP_OFF.items():
        with open(sysnode, "rb") as f_node:
            ...
            if key == "facs_addr":
                ...
                packed_data_32 = (
                    int.from_bytes(packed_data, "little") +
                    WAKE_VECTOR_OFFSET_32
                )
                packed_data_64 = (
                    int.from_bytes(packed_data, "little") +
                ...
                print(
                    "\t#define WAKE_VECTOR_32          0x{:0>2X}UL".format(
                        packed_data_32
                    ),
                    file=config,
                )
                print(
                    "\t#define WAKE_VECTOR_64          0x{:0>2X}UL".format(
                        packed_data_64
                    ),
                    file=config,
                )
                ...
                reset_reg.reset_reg_addr = int.from_bytes(
                    packed_data[4:11], "little")
            elif key == "reset_value":
                packed_data = f_node.read(1)
                reset_reg.reset_reg_val = hex(packed_data[0])

    write_reset_reg(
        reset_reg.reset_reg_addr,
        reset_reg.reset_reg_space_id,
        reset_reg.reset_reg_val,
        config,
    )
...
def read_pm_sstate(sysnode, config):
    ...
    for key, offset in FACP_OFF.items():
        with open(sysnode, "rb") as f_node:
            ...
            if key == "pm1a_evt":
                print(
                    "\t#define PM1A_EVT_SPACE_ID       {0}".format(
                        SPACE_ID[PackedGas.space_id_8b]
                    ),
                    file=config,
                )
                print(
                    "\t#define PM1A_EVT_BIT_WIDTH      {0}U".format(
                        hex(PackedGas.bit_width_8b)
                    ),
                    file=config,
                )
                print(
                    "\t#define PM1A_EVT_BIT_OFFSET     {0}U".format(
                        hex(PackedGas.bit_offset_8b)
                    ),
                    file=config,
                )
                print(
                    "\t#define PM1A_EVT_ADDRESS        {0}UL".format(
                        hex(PackedGas.address_64b)
                    ),
                    file=config,
                )
                print(
                    "\t#define PM1A_EVT_ACCESS_SIZE    {0}U".format(
                        hex(PackedGas.access_size_8b)
                    ),
                    file=config,
                )
            elif key == "pm1a_cnt":
                print(
                    "\t#define PM1A_CNT_SPACE_ID       {0}".format(
                        SPACE_ID[PackedGas.space_id_8b]
                    ),
                    file=config,
                )
                print(
                    "\t#define PM1A_CNT_BIT_WIDTH      {0}U".format(
                        hex(PackedGas.bit_width_8b)
                    ),
                    file=config,
                )
                print(
                    "\t#define PM1A_CNT_BIT_OFFSET     {0}U".format(
                        hex(PackedGas.bit_offset_8b)
                    ),
                    file=config,
                )
                print(
                    "\t#define PM1A_CNT_ADDRESS        {0}UL".format(
                        hex(PackedGas.address_64b)
                    ),
                    file=config,
                )
                print(
                    "\t#define PM1A_CNT_ACCESS_SIZE    {0}U".format(
                        hex(PackedGas.access_size_8b)
                    ),
                    file=config,
                )
                ...
                print(
                    "\t#define PM1B_EVT_SPACE_ID       {0}".format(
                        SPACE_ID[PackedGas.space_id_8b]
                    ),
                    file=config,
                )
                print(
                    "\t#define PM1B_EVT_BIT_WIDTH      {0}U".format(
                        hex(PackedGas.bit_width_8b)
                    ),
                    file=config,
                )
                print(
                    "\t#define PM1B_EVT_BIT_OFFSET     {0}U".format(
                        hex(PackedGas.bit_offset_8b)
                    ),
                    file=config,
                )
                print(
                    "\t#define PM1B_EVT_ADDRESS        {0}UL".format(
                        hex(PackedGas.address_64b)
                    ),
                    file=config,
                )
                print(
                    "\t#define PM1B_EVT_ACCESS_SIZE    {0}U".format(
                        hex(PackedGas.access_size_8b)
                    ),
                    file=config,
                )
                ...
                print(
                    "\t#define PM1B_CNT_SPACE_ID       {0}".format(
                        SPACE_ID[PackedGas.space_id_8b]
                    ),
                    file=config,
                )
                print(
                    "\t#define PM1B_CNT_BIT_WIDTH      {0}U".format(
                        hex(PackedGas.bit_width_8b)
                    ),
                    file=config,
                )
                print(
                    "\t#define PM1B_CNT_BIT_OFFSET     {0}U".format(
                        hex(PackedGas.bit_offset_8b)
                    ),
                    file=config,
                )
                print(
                    "\t#define PM1B_CNT_ADDRESS        {0}UL".format(
                        hex(PackedGas.address_64b)
                    ),
                    file=config,
                )
                print(
                    "\t#define PM1B_CNT_ACCESS_SIZE    {0}U".format(
                        hex(PackedGas.access_size_8b)
                    ),
                    file=config,
                )
...
def decode_sx_pkg(pkg_len, f_node):
    ...
    if hex(pkg_buf[1]) == ACPI_OP["AML_ZERO_OP"] or hex(pkg_buf[1]) == hex(
        ACPI_OP["AML_ONE_OP"]
    ):
        pkg_val_pm1a = pkg_buf[1]
        if hex(pkg_buf[2]) == hex(ACPI_OP["AML_ZERO_OP"]) or hex(pkg_buf[2]) == hex(
            ACPI_OP["AML_ONE_OP"]
        ):
        ...
        if hex(pkg_buf[3]) == hex(ACPI_OP["AML_ZERO_OP"]) or hex(pkg_buf[3]) == hex(
            ACPI_OP["AML_ONE_OP"]
        ):
            pkg_val_pm1b = pkg_buf[3]
            pkg_val_resv = pkg_buf[4:6]
    ...
    with open(sysnode, "rb") as f_node:
        while True:
            ...
            if inc:
                ...
                # decode sx pkg
                (pkg_val_pm1a, pkg_val_pm1b, pkg_val_resv, need_break) = decode_sx_pkg(
                    pkg_len, f_node
                )
                if need_break:
                    break
            ...
            if sx_name == "_S3_":
                ...
                print(
                    "\t#define S3_PKG_VAL_PM1A         {0}".format(
                        hex(S3_PKG.val_pm1a))
                    + "U",
                    file=config,
                )
                print(
                    "\t#define S3_PKG_VAL_PM1B         {0}".format(
                        S3_PKG.val_pm1b)
                    + "U",
                    file=config,
                )
                print(
                    "\t#define S3_PKG_RESERVED         {0}".format(
                        hex(int.from_bytes(S3_PKG.reserved, "little"))
                    )
                    + "U",
                    file=config,
                )
            ...
            if sx_name == "_S5_":
                ...
                print(
                    "\t#define S5_PKG_VAL_PM1A         {0}".format(
                        hex(S5_PKG.val_pm1a))
                    + "U",
                    file=config,
                )
                print(
                    "\t#define S5_PKG_VAL_PM1B         {0}".format(
                        S5_PKG.val_pm1b)
                    + "U",
                    file=config,
                )
                print(
                    "\t#define S5_PKG_RESERVED         {0}".format(
                        hex(int.from_bytes(S5_PKG.reserved, "little"))
                    )
                    + "U",
                    file=config,
                )
...
def store_cx_data(sysnode1, sysnode2, config):
        ...
        with open(sysnode1, "r") as acpi_idle:
            ...
            if idle_driver.find("acpi_idle") == -1:
                parser_lib.print_yel(
                    "The Cx data for ACRN relies on "
                    + "acpi_idle driver but it is not found, ",
                    warn=True,
                    end=False,
                )
                if idle_driver.find("intel_idle") == 0:
                    print(
                        "please add idle=nomwait in kernel "
                        + "cmdline to fall back to acpi_idle driver"
                    )
                else:
                    parser_lib.print_yel(
                        "please make sure ACPI Cstate is enabled in BIOS.", warn=True
                    )
    ...
    for state in state_cpus:
        ...
        if cx_state[state][cpu_state[0]].find(acpi_hw_type[0]) != -1:
            ...
            addr_val = get_value_after_str(
                cx_state[state][cpu_state[0]], acpi_hw_type[1]
            )
            ...
            PackedCx.access_size_8b = 0
            addr_val = get_value_after_str(
                cx_state[state][cpu_state[0]], acpi_hw_type[2]
            )
            PackedCx.address_64b = addr_val
        print(
            "\t{{{{{}, 0x{:0>2X}U, 0x{:0>2X}U, 0x{:0>2X}U, ".format(
                PackedCx.space_id_8b,
                PackedCx.bit_width_8b,
                PackedCx.bit_offset_8b,
                PackedCx.access_size_8b,
            ),
            file=config,
            end="",
        )
        print(
            "0x{:0>2X}UL}}, 0x{:0>2X}U, 0x{:0>2X}U, 0x{:0>2X}U}},\t/* C{} */".format(
                int(str(PackedCx.address_64b), 16),
                cx_state[state]["type"],
                int(cx_state[state][cpu_state[1]]),
                int(cx_state[state][cpu_state[2]]),
                c_cnt,
            ),
            file=config,
        )
...
def store_px_data(sysnode, config):
    ...
    try:
        with open(sysnode + "cpu0/cpufreq/scaling_driver", "r") as f_node:
            ...
            if freq_driver.find("acpi-cpufreq") == -1:
                parser_lib.print_yel(
                    "The Px data for ACRN relies on "
                    + "acpi-cpufreq driver but it is not found, ",
                    warn=True,
                    end=False,
                )
                if freq_driver.find("intel_pstate") == 0:
                    print(
                        "please add intel_pstate=disable in kernel "
                        + "cmdline to fall back to acpi-cpufreq driver"
                    )
                else:
                    parser_lib.print_yel(
                        "please make sure ACPI Pstate is enabled in BIOS.", warn=True
                    )
    ...
    for freq in freqs.split():
        if boost != 0 and i == 0:
            try:
                subprocess.check_call(
                    "/usr/sbin/rdmsr 0x1ad", shell=True, stdout=subprocess.PIPE
                )
            except subprocess.CalledProcessError:
                parser_lib.print_red(
                    "MSR 0x1ad not support in this platform!", err=True
                )
                sys.exit(1)

            res = subprocess.Popen(
                "/usr/sbin/rdmsr 0x1ad",
                shell=True,
        ...
        px_data[freq] = px_tmp
        print(
            "\t{{0x{:0>2X}UL, 0x{:0>2X}UL, 0x{:0>2X}UL, ".format(
                px_data[freq].core_freq,
                px_data[freq].power,
        ...
        print(
            "0x{:0>2X}UL, 0x{:0>6X}UL, 0x{:0>6X}UL}},\t/* P{} */".format(
                px_data[freq].bus_latency,
                px_data[freq].control,
                px_data[freq].status,
                p_cnt,
            ),
            file=config,
...
def store_mmcfg_base_data(mmcfg_node, config):
    ...
    with open(mmcfg_node, "rb") as mmcfg:
        ...
        if mmcfg_len_int > MCFG_ENTRY1_OFFSET:
            parser_lib.print_red(
                "Multiple PCI segment groups is not supported!", err=True
            )
            sys.exit(1)
        ...
        print(
            "\t#define DEFAULT_PCI_MMCFG_BASE   {}UL".format(
                hex(mmcfg_base_addr)),
            file=config,
        )
...
def read_tpm_data(config):
    ...
    """
    try:
        acpi_table_output = subprocess.check_output(
            "ls -l /sys/firmware/acpi/tables/".split()
        ).decode("utf8")
    except:
        acpi_table_output = ""
...
def gen_acpi_info(config):
    ...
    store_cx_data(
        SYS_PATH[2] + "cpuidle/current_driver", SYS_PATH[2] +
        "cpu0/cpuidle/", config
    )
...
def generate_info(board_file):
    ...
    if os.path.isfile(SYS_PATH[1] + "PTCT"):
        ...
        print(
            "PTCT table has been saved to {} successfully!".format(
                os.path.join(out_dir, "PTCT")
            )
        )
    if os.path.isfile(SYS_PATH[1] + "RTCT"):
        shutil.copy(SYS_PATH[1] + "RTCT", out_dir if out_dir != "" else "./")
        print(
            "RTCT table has been saved to {} successfully!".format(
                os.path.join(out_dir, "RTCT")
            )
        )
