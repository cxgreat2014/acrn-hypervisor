...
def get_secondary_bus(dmar_tbl, tmp_dev, tmp_fun):
    ...
    while True:
        ...
        if not found_pci_bdf:
            ...
            fun = int(line.strip().split()[0].split(
                ":")[1].split(".")[1].strip(), 16)
            ...
            if "00:" == line.strip().split()[0]:
                ...
                if (
                    len(line.split()) >= 16
                    and int(line.strip().split()[15], 16) & 0x7 == PCI_BRIDGE_HEADER
                ):
...
def walk_pci_bus(tmp_pdf, dmar_tbl, dmar_hw_list, drhd_cnt):
    ...
    dmar_tbl.path_offset = dmar_tbl.dev_scope_offset + \
        ctypes.sizeof(DmarDevScope)
    n_cnt = (dmar_tbl.dmar_dev_scope.scope_length -
             ctypes.sizeof(DmarDevScope)) // 2
    while n_cnt:
        ...
        tmp_pdf.path = ((tmp_pdf.device & 0x1F) << 3) | (
            (tmp_pdf.function & 0x7))

        # walk the secondary pci bus
        get_secondary_bus(dmar_tbl, tmp_pdf.device, tmp_pdf.function)

        if (
            (dmar_tbl.dmar_drhd.segment << 16)
            | (dmar_tbl.dmar_dev_scope.bus << 8)
            | tmp_pdf.path
        ) == CONFIG_GPU_SBDF:
...
def walk_dev_scope(dmar_tbl, dmar_dev_list, dmar_hw_list, drhd_cnt):
    ...
    :param drhd_cnt: it is a counter to calculate the DRHD in DMAR table
    """
    dmar_tbl.dev_scope_offset = dmar_tbl.drhd_offset + \
        ctypes.sizeof(DmarHwUnit)
    ...
    while dmar_tbl.dev_scope_offset < scope_end:
        dmar_tbl.dmar_dev_scope = DmarDevScope.from_address(
            dmar_tbl.dev_scope_offset)
        if dmar_tbl.dmar_dev_scope.scope_length <= 0:
            break
        if (
            dmar_tbl.dmar_dev_scope.entry_type
            != ACPI_DEV_SCOPE_TYPE["ACPI_DMAR_SCOPE_TYPE_NOT_USED"]
            and dmar_tbl.dmar_dev_scope.entry_type
            < ACPI_DEV_SCOPE_TYPE["ACPI_DMAR_SCOPE_TYPE_RESERVED"]
        ):
            dmar_tbl.dev_scope_cnt += 1

            # get type and id from device scope
            dmar_dev_list.dev_scope_type_list.append(
                dmar_tbl.dmar_dev_scope.entry_type)
            dmar_dev_list.dev_scope_id_list.append(
                dmar_tbl.dmar_dev_scope.enumeration_id
            )
...
def write_dmar_data(sysnode, config):
    ...
    print("\t#define DRHD_COUNT              {0}U".format(
        drhd_cnt), file=config)
    print("", file=config)
    ...
    for drhd_hw_i in range(drhd_cnt):
        ...
        print(
            ...
            + str(drhd_hw_i)
            + "_DEV_CNT           {0}U".format(
                hex(dmar_dev_list.dev_scope_cnt_list[drhd_hw_i])
            ),
            file=config,
        )
        print(
            ...
            + str(drhd_hw_i)
            + "_SEGMENT           {0}U".format(
                hex(dmar_hw_list.hw_segment_list[drhd_hw_i])
            ),
            file=config,
        )
        print(
            ...
            + str(drhd_hw_i)
            + "_FLAGS             {0}U".format(
                hex(dmar_hw_list.hw_flags_list[drhd_hw_i])
            ),
            file=config,
        )
        print(
            "\t#define DRHD"
            + str(drhd_hw_i)
            + "_REG_BASE          0x{:0>2X}UL".format(
                dmar_hw_list.hw_address_list[drhd_hw_i]
            ),
            file=config,
        )
        if drhd_hw_i in dmar_hw_list.hw_ignore.keys():
            print(
                "\t#define DRHD"
                + str(drhd_hw_i)
                + "_IGNORE            {0}".format(dmar_hw_list.hw_ignore[drhd_hw_i]),
                file=config,
            )
        for dev_scope_i in range(dev_scope_num):
            print(
                "\t#define DRHD" + str(drhd_hw_i) +
                "_DEVSCOPE" + str(dev_scope_i),
                file=config,
                end="",
            )
            print(
                "_TYPE    {0}U".format(
                    hex(
                        dmar_dev_list.dev_scope_type_list[
                            prev_dev_scope_num + dev_scope_i
                        ]
                    )
                ),
                file=config,
            ...
            print(
                "_ID      {0}U".format(
                    hex(
                        dmar_dev_list.dev_scope_id_list[
                            prev_dev_scope_num + dev_scope_i
                        ]
                    )
                ),
            ...
            print(
                "_BUS     {0}U".format(
                    hex(dmar_dev_list.dev_bus_list[prev_dev_scope_num + dev_scope_i])
                ),
                file=config,
            )
            print(
            ...
            print(
                "_PATH    {0}U".format(
                    hex(dmar_dev_list.dev_path_list[prev_dev_scope_num + dev_scope_i])
                ),
                file=config,
            )
