...
class SMBIOS(unpack.Struct):
    def __new__(cls):
        ...
        with open("/sys/firmware/dmi/tables/smbios_entry_point", "rb") as f:
            sm._header_memory = f.read()
        return sm

    def __init__(self):
        super(SMBIOS, self).__init__()
        u = unpack.Unpackable(self._header_memory)
        if self._header_memory[:4] == b"_SM_":
            self.add_field("header", Header_2_1(u))
        elif self._header_memory[:5] == b"_SM3_":
            self.add_field("header", Header_3_0(u))
        else:
            assert False, "Unknown SMBIOS entry point structure"
        with open("/sys/firmware/dmi/tables/DMI", "rb") as f:
            self._structure_memory = f.read()
        u = unpack.Unpackable(self._structure_memory)
        self.add_field(
            "structures",
            unpack.unpack_all(u, _smbios_structures, self),
            unpack.format_each("\n\n{!r}"),
        )
    ...
    def structure_type(self, num):
        ...
        try:
            types_present = [
                self.structures[x].smbios_structure_type
                for x in range(len(self.structures))
            ]
            matrix = dict()
            for index in range(len(types_present)):
                if types_present.count(types_present[index]) == 1:
                    ...
                    if matrix.has_key(types_present[index]):
                        matrix[types_present[index]].append(
...
class Header_2_1(unpack.Struct):
    def __new__(cls, u):
        return super(Header_2_1, cls).__new__(cls)

    def __init__(self, u):
        super(Header_2_1, self).__init__()
...
class Header_3_0(unpack.Struct):
    def __new__(cls, u):
        return super(Header_3_0, cls).__new__(cls)

    def __init__(self, u):
        super(Header_3_0, self).__init__()
        self.raw_data = u.unpack_rest()
        u = unpack.Unpackable(self.raw_data)
        self.add_field("anchor_string", u.unpack_one("5s"))
        self.add_field("checksum", u.unpack_one("B"))
        self.add_field("length", u.unpack_one("B"))
        self.add_field("major_version", u.unpack_one("B"))
        self.add_field("minor_version", u.unpack_one("B"))
        self.add_field("doc_revision", u.unpack_one("B"))
        self.add_field("entry_point_revision", u.unpack_one("B"))
        self.add_field("reserved", u.unpack_one("B"))
        self.add_field("max_structure_size", u.unpack_one("<I"))
        self.add_field("structure_table_address", u.unpack_one("<Q"))
        if not u.at_end():
            self.add_field("data", u.unpack_rest())
...
class SmbiosBaseStructure(unpack.Struct):
    ...
    def __init__(self, u, sm):
        ...
        def unpack_string():
            return b"".join(iter(lambda: u.unpack_one("c"), b"\x00"))

        strings = list(iter(unpack_string, b""))
        if not strings:
            u.skip(1)
...
class BIOSInformation(SmbiosBaseStructure):
    ...
    def __init__(self, u, sm):
        ...
        try:
            ...
            minor_version_str = str(
                sm.header.minor_version
            )  # 34 is .34, 4 is .4, 41 is .41; compare ASCIIbetically to compare initial digits rather than numeric value
            if (sm.header.major_version, minor_version_str) >= (2, "4"):
                characteristic_bytes = 2
            else:
                characteristic_bytes = self.length - 0x12
            self.add_field(
                "characteristics_extensions",
                [u.unpack_one("B") for b in range(characteristic_bytes)],
            )
            if (sm.header.major_version, minor_version_str) >= (2, "4"):
        ...
        try:
            ...
            if self.length > 0x8:
                ...
                self.add_field(
                    "wakeup_type",
                    u.unpack_one("B"),
                    unpack.format_table("{}", wakeup_types),
                )
            if self.length > 0x19:
    ...
    def __init__(self, u, sm):
        ...
        try:
            ...
            if self.length > 0x9:
                ...
                self.add_field(
                    "hosting_board",
                    bool(bitfields.getbits(self.feature_flags, 0)),
                    "feature_flags[0]={}",
                )
                self.add_field(
                    "requires_daughter_card",
                    bool(bitfields.getbits(self.feature_flags, 1)),
                    "feature_flags[1]={}",
                )
                self.add_field(
                    "removable",
                    bool(bitfields.getbits(self.feature_flags, 2)),
                    "feature_flags[2]={}",
                )
                self.add_field(
                    "replaceable",
                    bool(bitfields.getbits(self.feature_flags, 3)),
                    "feature_flags[3]={}",
                )
                self.add_field(
                    "hot_swappable",
                    bool(bitfields.getbits(self.feature_flags, 4)),
                    "feature_flags[4]={}",
                )
            ...
            if self.length > 0xD:
                self.add_field(
                    "board_type",
                    u.unpack_one("B"),
                    unpack.format_table("{}", _board_types),
                )
...
class SystemEnclosure(SmbiosBaseStructure):
    ...
    def __init__(self, u, sm):
        ...
        try:
            ...
            self.add_field(
                "chassis_lock_present",
                bool(bitfields.getbits(self.enumerated_type, 7)),
                "enumerated_type[7]={}",
            )
            board_types = {
                0x01: "Other",
            ...
            self.add_field(
                "system_enclosure_type",
                bitfields.getbits(self.enumerated_type, 6, 0),
                unpack.format_table("enumerated_type[6:0]={}", board_types),
            )
            self.add_field("version", u.unpack_one("B"), self.fmtstr)
            self.add_field("serial_number", u.unpack_one("B"), self.fmtstr)
            self.add_field("asset_tag", u.unpack_one("B"), self.fmtstr)
            ...
            if self.length > 9:
                ...
                self.add_field(
                    "bootup_state",
                    u.unpack_one("B"),
                    unpack.format_table("{}", chassis_states),
                )
                self.add_field(
                    "power_supply_state",
                    u.unpack_one("B"),
                ...
                self.add_field(
                    "security_status",
                    u.unpack_one("B"),
                    unpack.format_table("{}", security_states),
                )
            if self.length > 0xD:
            ...
            if getattr(self, "contained_element_count", 0):
                self.add_field(
                    "contained_elements",
                    tuple(
                        SystemEnclosureContainedElement(
                            u, self.contained_element_length
                        )
                        for i in range(self.contained_element_count)
                    ),
                )
            if self.length > (
                0x15
                + (
                    getattr(self, "contained_element_count", 0)
                    * getattr(self, "contained_element_length", 0)
                )
            ):
                self.add_field("sku_number", u.unpack_one("B"), self.fmtstr)
        except:
...
class SystemEnclosureContainedElement(unpack.Struct):
    def __init__(self, u, length):
        ...
        self.add_field(
            "type_select",
            bitfields.getbits(self.contained_element_type, 7),
            unpack.format_table(
                "contained_element_type[7]={}", type_selections),
        )
        self.add_field("type", bitfields.getbits(
            self.contained_element_type, 6, 0))
        if self.type_select == 0:
            self.add_field(
                "smbios_board_type", self.type, unpack.format_table(
                    "{}", _board_types)
            )
        else:
...
class ProcessorInformation(SmbiosBaseStructure):
    ...
    def __init__(self, u, sm):
        ...
        u = self.u
        try:
            self.add_field("socket_designation",
                           u.unpack_one("B"), self.fmtstr)
            processor_types = {
                0x01: "Other",
                0x02: "Unknown",
            ...
            self.add_field(
                "processor_type",
                u.unpack_one("B"),
                unpack.format_table("{}", processor_types),
            )
            self.add_field("processor_family", u.unpack_one("B"))
            self.add_field("processor_manufacturer",
                           u.unpack_one("B"), self.fmtstr)
            ...
            self.add_field(
                "processor_upgrade",
                u.unpack_one("B"),
                unpack.format_table("{}", processor_upgrades),
            )
            if self.length > 0x1A:
...
# obsolete starting with v2.1
class MemoryControllerInformation(SmbiosBaseStructure):
    ...
    def __init__(self, u, sm):
        ...
        try:
            ...
            self.add_field(
                ...
                u.unpack_one("B"),
                unpack.format_table("{}", _error_detecting_method),
            )
            ...
            self.add_field(
                ...
                u.unpack_one("B"),
                unpack.format_table("{}", _interleaves),
            )
            self.add_field(
                "current_interleave",
                u.unpack_one("B"),
            ...
            self.add_field("max_memory_module_size",
                           u.unpack_one("B"), self.fmtstr)
            ...
            self.add_field(
                "req_voltage_b2",
                bitfields.getbits(self.memory_module_voltage, 2),
                "memory_module_voltage[2]={}",
            )
            self.add_field(
                "req_voltage_b1",
                bitfields.getbits(self.memory_module_voltage, 1),
                "memory_module_voltage[1]={}",
            )
            self.add_field(
                "req_voltage_b0",
                bitfields.getbits(self.memory_module_voltage, 0),
                "memory_module_voltage[0]={}",
            )
            self.add_field("num_associated_memory_slots", u.unpack_one("B"))
            self.add_field(
                "memory_module_configuration_handles",
                u.unpack_one("<(self.num_associated_memory_slots)H"),
            )
            self.add_field(
                "enabled_error_correcting_capabilities", u.unpack_one("B"))
        except:
            self.decode_failure = True
...
# obsolete starting with v2.1
class MemoryModuleInformation(SmbiosBaseStructure):
    ...
    def __init__(self, u, sm):
        ...
        try:
            ...
            _mem_connection = {0: "single", 1: "double-bank"}
            self.add_field("installed_mem", u.unpack_one("B"))
            self.add_field(
                "installed_size",
                bitfields.getbits(self.installed_mem, 6, 0),
                "installed_mem[6:0]={}",
            )
            self.add_field(
                "installed_memory_module_connection",
                bitfields.getbits(self.installed_mem, 7),
                unpack.format_table("installed_mem[7]={}", _mem_connection),
            )
            ...
            self.add_field(
                ...
                bitfields.getbits(self.installed_mem, 7),
                unpack.format_table("enabled_mem[7]={}", _mem_connection),
            )
            self.add_field("error_status", u.unpack_one("B"))
            self.add_field(
                "error_status_info_obstained_from_event_log",
                bool(bitfields.getbits(self.error_status, 2)),
                unpack.format_table("error_status[2]={}", _mem_connection),
            )
            self.add_field(
                "correctable_errors_received",
                bool(bitfields.getbits(self.error_status, 1)),
                unpack.format_table("error_status[1]={}", _mem_connection),
            )
            self.add_field(
                "uncorrectable_errors_received",
                bool(bitfields.getbits(self.error_status, 0)),
                unpack.format_table("error_status[0]={}", _mem_connection),
            )
        except:
            self.decode_failure = True
...
class CacheInformation(SmbiosBaseStructure):
    ...
    def __init__(self, u, sm):
        ...
        try:
            ...
            self.add_field(
                "operational_mode",
                bitfields.getbits(self.cache_configuration, 9, 8),
                unpack.format_table(
                    "cache_configuration[9:8]={}", _operational_mode),
            )
            self.add_field(
                "enabled_at_boot_time",
                bool(bitfields.getbits(self.cache_configuration, 7)),
                "cache_configuration[7]={}",
            )
            _location = {
                0b00: "Internal",
                0b01: "External",
            ...
            self.add_field(
                ...
                bitfields.getbits(self.cache_configuration, 6, 5),
                unpack.format_table("cache_configuration[6:5]={}", _location),
            )
            self.add_field(
                "cache_socketed",
                bool(bitfields.getbits(self.cache_configuration, 3)),
                "cache_configuration[3]={}",
            )
            self.add_field(
                "cache_level",
                bitfields.getbits(self.cache_configuration, 2, 0),
                "cache_configuration[2:0]={}",
            )
            self.add_field("max_cache_size", u.unpack_one("<H"))
            _granularity = {0: "1K granularity", 1: "64K granularity"}
            self.add_field(
                "max_granularity",
                bitfields.getbits(self.cache_configuration, 15),
                unpack.format_table("max_cache_size[15]={}", _granularity),
            )
            self.add_field(
                "max_size_in_granularity",
                bitfields.getbits(self.cache_configuration, 14, 0),
                "max_cache_size[14, 0]={}",
            )
            ...
            if self.installed_size != 0:
                self.add_field(
                    ...
                    bitfields.getbits(self.cache_configuration, 15),
                    unpack.format_table("installed_size[15]={}", _granularity),
                )
                self.add_field(
                    "installed_size_in_granularity",
                    bitfields.getbits(self.cache_configuration, 14, 0),
                    "installed_size[14, 0]={}",
                )
            ...
            if self.length > 0x10:
                ...
                self.add_field(
                    "error_correction",
                    u.unpack_one("B"),
                    unpack.format_table("{}", _error_correction),
                )
            if self.length > 0x10:
                _system_cache_type = {
                    0x01: "Other",
                ...
                self.add_field(
                    "system_cache_type",
                    u.unpack_one("B"),
                    unpack.format_table("{}", _system_cache_type),
                )
            if self.length > 0x12:
                _associativity = {
                    0x01: "Other",
                ...
                self.add_field(
                    "associativity",
                    u.unpack_one("B"),
                    unpack.format_table("{}", _associativity),
                )
...
class PortConnectorInfo(SmbiosBaseStructure):
    ...
    def __init__(self, u, sm):
        ...
        try:
            self.add_field(
                "internal_reference_designator", u.unpack_one("B"), self.fmtstr
            )
            connector_types = {
                0x00: "None",
                0x01: "Centronics",
            ...
            self.add_field(
                ...
                u.unpack_one("B"),
                unpack.format_table("{}", connector_types),
            )
            self.add_field(
                "external_reference_designator", u.unpack_one("B"), self.fmtstr
            )
            self.add_field(
            ...
            self.add_field(
                "port_type", u.unpack_one(
                    "B"), unpack.format_table("{}", port_types)
            )
        except:
...
class SystemSlots(SmbiosBaseStructure):
    ...
    def __init__(self, u, sm):
        ...
        try:
            ...
            self.add_field(
                "slot_type", u.unpack_one(
                    "B"), unpack.format_table("{}", _slot_types)
            )
            ...
            self.add_field(
                "slot_data_bus_width",
                u.unpack_one("B"),
                unpack.format_table("{}", _slot_data_bus_widths),
            )
            _current_usages = {
                0x01: "Other",
                0x02: "Unknown",
            ...
            self.add_field(
                "current_usage",
                u.unpack_one("B"),
                unpack.format_table("{}", _current_usages),
            )
            _slot_lengths = {
                0x01: "Other",
                0x02: "Unknown",
            ...
            self.add_field(
                "slot_length",
                u.unpack_one("B"),
                unpack.format_table("{}", _slot_lengths),
            )
            self.add_field("slot_id", u.unpack_one("<H"))
            self.add_field("characteristics1", u.unpack_one("B"))
            self.add_field(
                "characteristics_unknown",
                bool(bitfields.getbits(self.characteristics1, 0)),
                "characteristics1[0]={}",
            )
            self.add_field(
                "provides_5_0_volts",
                bool(bitfields.getbits(self.characteristics1, 1)),
                "characteristics1[1]={}",
            )
            self.add_field(
                "provides_3_3_volts",
                bool(bitfields.getbits(self.characteristics1, 2)),
                "characteristics1[2]={}",
            )
            self.add_field(
                "shared_slot",
                bool(bitfields.getbits(self.characteristics1, 3)),
                "characteristics1[3]={}",
            )
            self.add_field(
                "supports_pc_card_16",
                bool(bitfields.getbits(self.characteristics1, 4)),
                "characteristics1[4]={}",
            )
            self.add_field(
                "supports_cardbus",
                bool(bitfields.getbits(self.characteristics1, 5)),
                "characteristics1[5]={}",
            )
            self.add_field(
                "supports_zoom_video",
                bool(bitfields.getbits(self.characteristics1, 6)),
                "characteristics1[6]={}",
            )
            self.add_field(
                "supports_modem_ring_resume",
                bool(bitfields.getbits(self.characteristics1, 7)),
                "characteristics1[7]={}",
            )
            if self.length > 0x0C:
                ...
                self.add_field(
                    "supports_PME",
                    bool(bitfields.getbits(self.characteristics2, 0)),
                    "characteristics2[0]={}",
                )
                self.add_field(
                    "supports_hot_plug",
                    bool(bitfields.getbits(self.characteristics2, 1)),
                    "characteristics2[1]={}",
                )
                self.add_field(
                    "supports_smbus",
                    bool(bitfields.getbits(self.characteristics2, 2)),
                    "characteristics2[2]={}",
                )
            if self.length > 0x0D:
                ...
                self.add_field(
                    "device_number",
                    bitfields.getbits(self.device_function_number, 7, 3),
                    "device_function_number[7:3]={}",
                )
                self.add_field(
                    "function_number",
                    bitfields.getbits(self.device_function_number, 2, 0),
                    "device_function_number[2:0]={}",
                )
        except:
            self.decodeFailure = True
...
class OnBoardDevicesInformation(SmbiosBaseStructure):
    ...
    def __init__(self, u, sm):
        ...
        try:
            ...
            self.add_field(
                "device_enabled",
                bool(bitfields.getbits(self.device_type, 7)),
                "device_type[7]={}",
            )
            _device_types = {
                0x01: "Other",
            ...
            self.add_field(
                "type_of_device",
                bitfields.getbits(self.device_type, 6, 0),
                unpack.format_table("device_type[6:0]={}", _device_types),
            )
            self.add_field("description_string",
                           u.unpack_one("B"), self.fmtstr)
        except:
            self.decodeFailure = True
    ...
    def __init__(self, u, sm):
        ...
        try:
            ...
            if self.length > 0x05:
                ...
                self.add_field(
                    "abbreviated_format",
                    bool(bitfields.getbits(self.flags, 0)),
                    "flags[0]={}",
                )
            if self.length > 0x6:
                u.skip(15)
                self.add_field("current_language",
                               u.unpack_one("B"), self.fmtstr)
        except:
            self.decodeFailure = True
    ...
    def __init__(self, u, sm):
        ...
        try:
            ...
            _access_method = {
                ...
                range(0x05, 0x07F): "Available for future assignment",
                range(0x80, 0xFF): "BIOS Vendor/OEM-specific",
            }
            self.add_field(
                "access_method",
                u.unpack_one("B"),
                unpack.format_table("{}", _access_method),
            )
            self.add_field("log_status", u.unpack_one("B"))
            self.add_field(
                "log_area_full",
                bool(bitfields.getbits(self.log_status, 1)),
                "log_status[1]={}",
            )
            self.add_field(
                "log_area_valid",
                bool(bitfields.getbits(self.log_status, 0)),
                "log_status[0]={}",
            )
            ...
            if self.length > 0x14:
                _log_header_formats = {
                    0: "No header",
                    1: "Type 1 log header",
                    range(2, 0x7F): "Available for future assignment",
                    range(0x80, 0xFF): "BIOS vendor or OEM-specific format",
                }
                self.add_field(
                    "log_header_format",
                    u.unpack_one("B"),
                    unpack.format_table("{}", _log_header_formats),
                )
            if self.length > 0x15:
                self.add_field(
                    "num_supported_log_type_descriptors", u.unpack_one("B"))
            if self.length > 0x16:
                self.add_field("length_log_type_descriptor", u.unpack_one("B"))
            if self.length != (
                0x17
                + (
                    self.num_supported_log_type_descriptors
                    * self.length_log_type_descriptor
                )
            ):
                print(
                    "Error: structure length ({}) != 0x17 + (num_supported_log_type_descriptors ({}) * length_log_type_descriptor({}))".format(
                        self.length,
                        self.num_supported_log_type_descriptors,
                        self.length_log_type_descriptor,
                    )
                ...
                print(
                    "num_supported_log_type_descriptors = {}".format(
                        self.num_supported_log_type_descriptors
                    )
                )
                print(
                    "length_log_type_descriptor = {}".format(
                        self.length_log_type_descriptor
                    )
                )
                self.decodeFailure = True
            self.add_field(
                "descriptors",
                tuple(
                    EventLogDescriptor.unpack(u)
                    for i in range(self.num_supported_log_type_descriptors)
                ),
                unpack.format_each("\n{!r}"),
            )
        except:
...
class EventLogDescriptor(unpack.Struct):
    ...
    def _unpack(u):
        _event_log_type_descriptors = {
            ...
            0x17: "System boot",
            range(0x18, 0x7F): "Unused, available for assignment",
            range(0x80, 0xFE): "Availalbe for system- and OEM-specific assignments",
            0xFF: "End of log",
        }
        ...
        _event_log_format = {
            ...
            range(0x80, 0xFF): "OEM assigned",
        }
        yield "variable_data_format_type", u.unpack_one("B"), unpack.format_table(
            "{}", _event_log_format
        )
        ...
        try:
            if self.length > 0x4:
                ...
                self.add_field(
                    "location",
                    u.unpack_one("B"),
                    unpack.format_table("{}", _location_field),
                )
            if self.length > 0x05:
                _use = {
                    0x01: "Other",
                ...
                self.add_field(
                    "use", u.unpack_one("B"), unpack.format_table("{}", _use)
                )
            if self.length > 0x06:
                _error_correction = {
                    0x01: "Other",
            ...
            if self.length > 0x0B:
                self.add_field("memory_error_information_handle",
                               u.unpack_one("<H"))
            if self.length > 0x0D:
...
class MemoryDevice(SmbiosBaseStructure):
    ...
    def __init__(self, u, sm):
        ...
        try:
            if self.length > 0x4:
                self.add_field("physical_memory_array_handle",
                               u.unpack_one("<H"))
            if self.length > 0x6:
                self.add_field("memory_error_information_handle",
                               u.unpack_one("<H"))
            ...
            if self.length > 0xE:
                ...
                self.add_field(
                    "form_factor",
                    u.unpack_one("B"),
                    unpack.format_table("{}", _form_factors),
                )
            if self.length > 0xF:
                self.add_field("device_set", u.unpack_one("B"))
            if self.length > 0x10:
                self.add_field("device_locator",
                               u.unpack_one("B"), self.fmtstr)
            if self.length > 0x11:
            ...
            if self.length > 0x12:
                ...
                self.add_field(
                    "memory_type",
                    u.unpack_one("B"),
                    unpack.format_table("{}", _memory_types),
                )
            if self.length > 0x13:
            ...
            if self.length > 0x1B:
                ...
                self.add_field(
                    "rank",
                    bitfields.getbits(self.attributes, 3, 0),
                    "attributes[3:0]={}",
                )
            if self.length > 0x1C:
                if self.size == 0x7FFF:
                    ...
                    self.add_field(
                        "mem_size",
                        bitfields.getbits(self.type_detail, 30, 0),
                        "type_detail[30:0]={}",
                    )
                else:
                    u.skip(4)
            if self.length > 0x20:
                self.add_field("configured_memory_clock_speed",
                               u.unpack_one("<H"))
            if self.length > 0x22:
        ...
        try:
            if self.length > 0x4:
                ...
                self.add_field(
                    "error_type",
                    u.unpack_one("B"),
                    unpack.format_table("{}", _error_types),
                )
            if self.length > 0x5:
                ...
                self.add_field(
                    "error_granularity",
                    u.unpack_one("B"),
                    unpack.format_table("{}", _error_granularity_field),
                )
            if self.length > 0x6:
                ...
                self.add_field(
                    "error_operation",
                    u.unpack_one("B"),
                    unpack.format_table("{}", _error_operation_field),
                )
            if self.length > 0x7:
                self.add_field("vendor_syndrome", u.unpack_one("<I"))
            if self.length > 0xB:
                self.add_field("memory_array_error_address",
                               u.unpack_one("<I"))
            if self.length > 0xF:
        ...
        try:
            ...
            if self.length > 0xF:
                ...
                if self.starting_address == 0xFFFFFFFF:
                    self.add_field("extended_starting_address",
                                   u.unpack_one("<Q"))
                    if self.length > 0x17:
                        self.add_field("extended_ending_address",
                                       u.unpack_one("<Q"))
                else:
                    u.skip(16)
...
class MemoryDeviceMappedAddress(SmbiosBaseStructure):
    ...
    def __init__(self, u, sm):
        ...
        try:
            ...
            if self.length > 0xE:
                self.add_field(
                    "memory_array_mapped_address_handle", u.unpack_one("<H"))
            if self.length > 0x10:
        ...
        try:
            if self.length > 0x4:
                ...
                self.add_field(
                    ...
                    u.unpack_one("B"),
                    unpack.format_table("{}", _pointing_device_types),
                )
            if self.length > 0x5:
                _interfaces = {
                    0x01: "Other",
                ...
                self.add_field(
                    "interface",
                    u.unpack_one("B"),
                    unpack.format_table("{}", _interfaces),
                )
            if self.length > 0x6:
    ...
    def __init__(self, u, sm):
        ...
        try:
            ...
            if self.length > 0x5:
                self.add_field("manufacturer", u.unpack_one("B"), self.fmtstr)
            if self.length > 0x6:
                self.add_field("manufacturer_date",
                               u.unpack_one("B"), self.fmtstr)
            if self.length > 0x7:
                self.add_field("serial_number", u.unpack_one("B"), self.fmtstr)
            if self.length > 0x8:
            ...
            if self.length > 0x9:
                ...
                self.add_field(
                    "device_chemistry",
                    u.unpack_one("B"),
                    unpack.format_table("{}", _device_chemistry),
                )
            if self.length > 0xA:
            ...
            if self.length > 0xE:
                self.add_field("sbds_version_number",
                               u.unpack_one("B"), self.fmtstr)
            if self.length > 0xF:
                self.add_field("max_error_battery_data",
                               u.unpack_one("B"), self.fmtstr)
            if self.length > 0x10:
            ...
            if self.length > 0x12:
                if self.manufacturer_date == 0:
                    ...
                    self.add_field(
                        "year_biased_by_1980",
                        bitfields.getbits(self.sbds_manufacture_date, 15, 9),
                        "sbds_manufacture_date[15:9]={}",
                    )
                    self.add_field(
                        "month",
                        bitfields.getbits(self.sbds_manufacture_date, 8, 5),
                        "sbds_manufacture_date[8:5]={}",
                    )
                    self.add_field(
                        "date",
                        bitfields.getbits(self.sbds_manufacture_date, 4, 0),
                        "sbds_manufacture_date[4:0]={}",
                    )
                else:
                    u.skip(2)
            if self.length > 0x14:
                if self.device_chemistry == 0x02:
                    self.add_field(
                        "sbds_device_chemistry", u.unpack_one("B"), self.fmtstr
                    )
                else:
                    u.skip(1)
            if self.length > 0x15:
    ...
    def __init__(self, u, sm):
        ...
        try:
            if self.length > 0x4:
                ...
                self.add_field(
                    "contains_watchdog_timer",
                    bool(bitfields.getbits(self.capabilities, 5)),
                    "capabilities[5]={}",
                )
                _boot_option = {
                ...
                self.add_field(
                    ...
                    bitfields.getbits(self.capabilities, 4, 3),
                    unpack.format_table("capabilities[4:3]={}", _boot_option),
                )
                self.add_field(
                    "boot_option_after_watchdog_reset",
                    bitfields.getbits(self.capabilities, 2, 1),
                    unpack.format_table("capabilities[2:1]={}", _boot_option),
                )
                self.add_field(
                    "system_reset_enabled_by_user",
                    bool(bitfields.getbits(self.capabilities, 0)),
                    "capabilities[0]={}",
                )
            if self.length > 0x5:
        ...
        try:
            if self.length > 0x4:
                ...
                self.add_field(
                    ...
                    bitfields.getbits(self.hardware_security_settings, 7, 6),
                    unpack.format_table(
                        "hardware_security_settings[7:6]={}", _status),
                )
                self.add_field(
                    "keyboard_password_status",
                    bitfields.getbits(self.hardware_security_settings, 5, 4),
                    unpack.format_table(
                        "hardware_security_settings[5:4]={}", _status),
                )
                self.add_field(
                    "admin_password_status",
                    bitfields.getbits(self.hardware_security_settings, 3, 2),
                    unpack.format_table(
                        "hardware_security_settings0[3:2]={}", _status),
                )
                self.add_field(
                    "front_panel_reset_status",
                    bitfields.getbits(self.hardware_security_settings, 1, 0),
                    unpack.format_table(
                        "hardware_security_settings[1:0]={}", _status),
                )
        except:
            self.decodeFailure = True
...
class SystemPowerControls(SmbiosBaseStructure):
    ...
    def __init__(self, u, sm):
        ...
        try:
            if self.length > 0x4:
                self.add_field("next_scheduled_poweron_month",
                               u.unpack_one("B"))
                self.add_field(
                    "next_scheduled_poweron_day_of_month", u.unpack_one("B"))
                self.add_field("next_scheduled_poweron_hour",
                               u.unpack_one("B"))
                self.add_field("next_scheduled_poweron_minute",
                               u.unpack_one("B"))
                self.add_field("next_scheduled_poweron_second",
                               u.unpack_one("B"))
        except:
            self.decodeFailure = True
    ...
    def __init__(self, u, sm):
        ...
        try:
            ...
            if self.length > 0x5:
                ...
                self.add_field(
                    "status",
                    bitfields.getbits(self.location_and_status, 7, 5),
                    unpack.format_table(
                        "location_and_status[7:5]={}", _status),
                )
                self.add_field(
                    "location",
                    bitfields.getbits(self.location_and_status, 4, 0),
                    unpack.format_table(
                        "location_and_status[4:0]={}", _location),
                )
            if self.length > 0x6:
    ...
    def __init__(self, u, sm):
        ...
        try:
            ...
            if self.length > 0x6:
                ...
                self.add_field(
                    "status",
                    bitfields.getbits(self.device_type_and_status, 7, 5),
                    unpack.format_table(
                        "device_type_and_status[7:5]={}", _status),
                )
                self.add_field(
                    "device_type",
                    bitfields.getbits(self.device_type_and_status, 4, 0),
                    unpack.format_table(
                        "device_type_and_status[4:0]={}", _type),
                )
            if self.length > 0x7:
...
class OutOfBandRemoteAccess(SmbiosBaseStructure):
    ...
    def __init__(self, u, sm):
        ...
        try:
            if self.length > 0x4:
                self.add_field("manufacturer_name",
                               u.unpack_one("B"), self.fmtstr)
            if self.length > 0x5:
                self.add_field("connections", u.unpack_one("B"))
                self.add_field(
                    "outbound_connection_enabled",
                    bool(bitfields.getbits(self.connections, 1)),
                    "connections[1]={}",
                )
                self.add_field(
                    "inbound_connection_enabled",
                    bool(bitfields.getbits(self.connections, 0)),
                    "connections[0]={}",
                )
        except:
            self.decodeFailure = True
        ...
        try:
            if self.length > 0xA:
                ...
                _boot_status = {
                    ...
                    range(9, 127): "Reserved for future assignment",
                    range(128, 191): "Vendor/OEM-specific implementations",
                    range(192, 255): "Product-specific implementations",
                }
                self.add_field(
                    "boot_status",
                    u.unpack_one("B"),
                    unpack.format_table("{}", _boot_status),
                )
        except:
            self.decodeFailure = True
    ...
    def __init__(self, u, sm):
        ...
        try:
            ...
            if self.length > 0x5:
                ...
                self.add_field(
                    "device_type", u.unpack_one(
                        "B"), unpack.format_table("{}", _type)
                )
            if self.length > 0x6:
            ...
            if self.length > 0xA:
                ...
                self.add_field(
                    "address_type",
                    u.unpack_one("B"),
                    unpack.format_table("{}", _address_type),
                )
        except:
            self.decodeFailure = True
...
class ManagementDeviceThresholdData(SmbiosBaseStructure):
    ...
    def __init__(self, u, sm):
        ...
        try:
            if self.length > 0x4:
                self.add_field("lower_threshold_noncritical",
                               u.unpack_one("<H"))
            if self.length > 0x6:
                self.add_field("upper_threshold_noncritical",
                               u.unpack_one("<H"))
            if self.length > 0x8:
            ...
            if self.length > 0xC:
                self.add_field("lower_threshold_nonrecoverable",
                               u.unpack_one("<H"))
            if self.length > 0xE:
                self.add_field("upper_threshold_nonrecoverable",
                               u.unpack_one("<H"))
        except:
            self.decodeFailure = True
        ...
        try:
            if self.length > 0x4:
                ...
                self.add_field(
                    "channel_type",
                    u.unpack_one("B"),
                    unpack.format_table("{}", _channel_type),
                )
            if self.length > 0x6:
    ...
    def __init__(self, u, sm):
        ...
        try:
            _interface_type = {
                ...
                range(0x04, 0xFF): "Reserved",
            }
            self.add_field(
                "interface_type",
                u.unpack_one("B"),
                unpack.format_table("{}", _interface_type),
            )
            self.add_field("ipmi_specification_revision", u.unpack_one("B"))
            self.add_field(
                "msd_revision",
                bitfields.getbits(self.ipmi_specification_revision, 7, 4),
                "ipmi_specification_revision[7:4]={}",
            )
            self.add_field(
                "lsd_revision",
                bitfields.getbits(self.ipmi_specification_revision, 3, 0),
                "ipmi_specification_revision[3:0]={}",
            )
            ...
            self.add_field(
                "base_address_modifier_interrupt_info", u.unpack_one("B"))
            _reg_spacing = {
            ...
            self.add_field(
                "register_spacing",
                bitfields.getbits(
                    self.base_address_modifier_interrupt_info, 7, 6),
                unpack.format_table(
                    "base_address_modifier_interrupt_info[7:6]={}", _reg_spacing
                ),
            )
            self.add_field(
                "ls_bit_for_addresses",
                bitfields.getbits(
                    self.base_address_modifier_interrupt_info, 4),
                "base_address_modifier_interrupt_info[4]={}",
            )
            self.add_field(
                "interrupt_info_specified",
                bool(bitfields.getbits(self.base_address_modifier_interrupt_info, 3)),
                "base_address_modifier_interrupt_info[3]={}",
            )
            _polarity = {0: "active low", 1: "active high"}
            self.add_field(
                "interrupt_polarity",
                bitfields.getbits(
                    self.base_address_modifier_interrupt_info, 1),
                unpack.format_table(
                    "base_address_modifier_interrupt_info[1]={}", _polarity
                ),
            )
            _interrupt_trigger = {0: "edge", 1: "level"}
            self.add_field(
                "interrupt_trigger_mode",
                bitfields.getbits(
                    self.base_address_modifier_interrupt_info, 0),
                unpack.format_table(
                    "base_address_modifier_interrupt_info[0]={}", _interrupt_trigger
                ),
            )
    ...
    def __init__(self, u, sm):
        ...
        try:
            ...
            if self.length > 0x9:
                self.add_field("asset_tag", u.unpack_one("B"), self.fmtstr)
            if self.length > 0xA:
                self.add_field("model_part_number",
                               u.unpack_one("B"), self.fmtstr)
            if self.length > 0xB:
                self.add_field("revision_level",
                               u.unpack_one("B"), self.fmtstr)
            if self.length > 0xC:
                self.add_field("max_power_capacity", u.unpack_one("<H"))
            if self.length > 0xE:
                self.add_field("power_supply_characteristics",
                               u.unpack_one("<H"))
                _dmtf_power_supply_type = {
                    ...
                    0b111: "Converter",
                    0b1000: "Regulator",
                    range(0b1001, 0b1111): "Reserved",
                }
                self.add_field(
                    "dmtf_power_supply_type",
                    bitfields.getbits(
                        self.power_supply_characteristics, 13, 10),
                    unpack.format_table(
                        "power_supply_characteristics[13:10]={}",
                        _dmtf_power_supply_type,
                    ),
                )
                _status = {
                ...
                self.add_field(
                    "status",
                    bitfields.getbits(self.power_supply_characteristics, 9, 7),
                    unpack.format_table(
                        "power_supply_characteristics[9:7]={}", _status
                    ),
                )
                _dmtf_input_voltage_range_switching = {
                    ...
                    range(0b0111, 0b1111): "Reserved",
                }
                self.add_field(
                    "dmtf_input_voltage_range_switching",
                    bitfields.getbits(self.power_supply_characteristics, 6, 3),
                    unpack.format_table(
                        "power_supply_characteristics[6:3]={}",
                        _dmtf_input_voltage_range_switching,
                    ),
                )
                self.add_field(
                    "power_supply_unplugged",
                    bool(bitfields.getbits(self.power_supply_characteristics, 2)),
                    "power_supply_characteristics[2]={}",
                )
                self.add_field(
                    "power_supply_present",
                    bool(bitfields.getbits(self.power_supply_characteristics, 1)),
                    "power_supply_characteristics[1]={}",
                )
                self.add_field(
                    "power_supply_hot_replaceable",
                    bool(bitfields.getbits(self.power_supply_characteristics, 0)),
                    "power_supply_characteristics[0]={}",
                )
            if self.length > 0x10:
                self.add_field("input_voltage_probe_handle",
                               u.unpack_one("<H"))
            if self.length > 0x12:
                self.add_field("cooling_device_handle", u.unpack_one("<H"))
            if self.length > 0x14:
                self.add_field("input_current_probe_handle",
                               u.unpack_one("<H"))
        except:
            self.decodeFailure = True
...
class AdditionalInformation(SmbiosBaseStructure):
    ...
    def __init__(self, u, sm):
        ...
        try:
            if self.length > 0x4:
                self.add_field(
                    "num_additional_information_entries", u.unpack_one("B"))
            if self.length > 0x5:
                self.add_field(
                    "additional_information_entry_length", u.unpack_one("B"))
...
class OnboardDevicesExtendedInformation(SmbiosBaseStructure):
    ...
    def __init__(self, u, sm):
        ...
        try:
            if self.length > 0x4:
                self.add_field("reference_designation",
                               u.unpack_one("B"), self.fmtstr)
            if self.length > 0x5:
                self.add_field("device_type", u.unpack_one("B"))
                self.add_field(
            ...
            if self.length > 0xA:
                ...
                self.add_field(
                    "device_number",
                    bitfields.getbits(self.device_type, 7, 3),
                    "device_and_function_number[7:3]={}",
                )
                self.add_field(
                    "function_number",
                    bitfields.getbits(self.device_type, 2, 0),
                    "device_and_function_number[2:0]={}",
                )
        except:
            self.decodeFailure = True
        ...
        try:
            if self.length > 0x4:
                ...
                self.add_field(
                    "interface_type",
                    u.unpack_one("B"),
                    unpack.format_table("{}", _interface_types),
                )
            if self.length > 0x5:
                self.add_field("mc_host_interface_data",
                               u.unpack_rest(), self.fmtstr)
        except:
            self.decodeFailure = True
