...
def tpm2_optional_data(data_len):
    ...
    if data_len <= 12:
        ...
        start_method_data_len = 12
        logging.warning(
            f"TPM2 data length: {data_len + 52} is greater than 64 bytes but less than 76 bytes."
        )
        logging.warning(
            f"The TPM2 data is still processed but the 65 to {data_len + 52} bytes are discard."
        )
...
def tpm2_factory(start_method_data_len, has_log_area):
    class TPM2(cdata.Struct):
        ...
        _fields_ = [
            ...
            (
                "start_method_specific_parameters",
                ctypes.c_ubyte * start_method_data_len,
            ),
        ] + (
            [
...
def TPM2(val):
    ...
    if isinstance(val, str):
        ...
        start_method_data_len, has_log_area = tpm2_optional_data(
            len(data) - base_length
        )
        return tpm2_factory(start_method_data_len, has_log_area).from_buffer_copy(data)
    elif isinstance(val, bytearray):
        return (
            tpm2_factory(12, True).from_buffer(val)
            if len(val) > 64
            else tpm2_factory(12, False).from_buffer(val)
        )
