...
class BufferBase(Object):
    ...
    def read_field(self, name):
        ...
        assert (
            offset + bitwidth <= self.__length * 8
        ...
        if bit_idx % access_width > 0:
            ...
            mask = self.bitmask(
                bit_idx % access_width + bit_count - 1, bit_idx % access_width
            )
            acc = (self.read(byte_idx, access_width)
                   & mask) >> (bit_idx % access_width)
    ...
    def write_field(self, name, value):
        ...
        if bit_idx % access_width > 0:
            ...
            self.write(
                byte_idx,
                (v & mask_of_write)
                | (self.read(byte_idx, access_width) & mask_of_keep),
                access_width,
            )
...
class Buffer(BufferBase):
    ...
    def read(self, byte_idx, bit_width):
        ...
        return int.from_bytes(
            self.__data[byte_idx: (byte_idx + byte_width)], sys.byteorder
        )

    def write(self, byte_idx, value, bit_width):
        byte_width = min(bit_width // 8, len(self.__data) - byte_idx)
        self.__data[byte_idx: (byte_idx + byte_width)] = value.to_bytes(
            byte_width, sys.byteorder
        )
...
class IndexedIOBuffer(BufferBase):
    ...
    def read(self, byte_idx, bit_width):
        assert (
            bit_width == 8
        ), f"Indexed I/O buffers can only be read one byte at a time"
...
class OperationRegion(Object):
    ...
    def open_system_memory(cls, name, offset, length):
        ...
        logging.info(
            f"Open system memory space {name}: [{hex(offset)}, {hex(offset + length - 1)}]"
        )
        offset_page_aligned = (offset >> 12) << 12
        length_page_aligned = ceil(
            ((offset & 0xFFF) + length) / 0x1000) * 0x1000
        try:
            mm = mmap.mmap(
                cls.devmem.fileno(),
                length_page_aligned,
                ...
                prot=mmap.PROT_READ,
                offset=offset_page_aligned,
            )
        except PermissionError as e:
            logging.warning(
                f"Do not have permission to access [{hex(offset_page_aligned)}, {hex(offset_page_aligned + length_page_aligned)}] by /dev/mem."
            )
            logging.warning(
                f"You may need to add `iomem=relaxed` to the Linux kernel command line in your bootloader configuration file."
            )
            raise
    ...
    def open_system_io(cls, name, offset, length):
        ...
        logging.info(
            f"Open system I/O space {name}: [{hex(offset)}, {hex(offset + length - 1)}]"
        )
    ...
    def open_pci_configuration_space(cls, bus_number, device_adr, offset, length):
        ...
        sysfs_path = "/sys/devices/pci0000:%02x/0000:%02x:%02x.%d/config" % (
            bus,
            bus,
            device,
            function,
        ...
        try:
            ...
            logging.warning(
                f"Cannot read the configuration space of %02x:%02x.%d from {sysfs_path}. Assume the PCI device does not exist."
                % (bus, device, function)
            )
    ...
    def open_indexed_region(cls, index_register, data_register):
        logging.info(
            f"Open I/O region indexed by index register {index_register.to_string()} and data register {data_register.to_string()}."
        )
