...
class LEAF_0(CPUID):
    ...
    max_leaf = cpuidfield(
        ...
        31,
        0,
        doc="Highest value the CPUID recognizes for returning basic processor information",
    )
...
class LEAF_1(CPUID):
    ...
    CLFLUSH_line_size = cpuidfield(
        EBX, 15, 8, doc="CLFLUSH instruction cache line size (in 8-byte words)"
    )
    max_logical_processor_ids = cpuidfield(
        EBX,
        23,
        16,
        doc="The maximum number of addressable IDs for logical processors in the physical package.",
    )
...
class LEAF_2(CPUID):
    ...
    times_to_run = cpuidfield(
        ...
        7,
        0,
        doc="Number of times CPUID must be executed with EAX = 2 to retrieve a complete description of the processor's TLB, Cache, and Prefetch hardware",
    )
...
class LEAF_4(CPUID):
    ...
    self_initializing = cpuidfield(
        EAX, 8, 8, doc="Self Initializing Cache Level")
    fully_associative = cpuidfield(EAX, 9, 9, doc="Fully Associative Cache")
    max_logical_processors_sharing_cache_z = cpuidfield(
        EAX,
        25,
        14,
        doc="Max number of addressable IDs for logical processors sharing this cache (zero based)",
    )
    max_cores_sharing_cache_z = cpuidfield(
        EAX,
        31,
        26,
        doc="Max number of addressable IDs for processor cores in the physical package (zero based)",
    )

    line_size_z = cpuidfield(
        EBX, 11, 0, doc="System Coherency Line Size (zero-based)")
    partitions_z = cpuidfield(
        EBX, 21, 12, doc="Physical Line Partitions (zero-based)")
    ways_z = cpuidfield(EBX, 31, 22, doc="Ways of associativity (zero-based)")

    sets_z = cpuidfield(ECX, 31, 0, doc="Sets (zero-based)")

    write_back_invalidate = cpuidfield(
        EDX, 0, 0, doc="Write-back Invalidate/Invalidate"
    )
    cache_inclusiveness = cpuidfield(EDX, 1, 1, doc="Cache Inclusiveness")
    complex_cache_indexing = cpuidfield(
        EDX, 2, 2, doc="Complex Cache indexing")
...
class LEAF_5(CPUID):
    ...
    interrupt_break_event_supported = cpuidfield(
        ...
        1,
        1,
        doc="Supports treating interrupts as break-events for MWAIT, even when interrupts disabled",
    )

    c0 = cpuidfield(
        EDX, 3, 0, doc="Number of C0 sub C-states supported using MWAIT")
    c1 = cpuidfield(
        EDX, 7, 4, doc="Number of C1 sub C-states supported using MWAIT")
    c2 = cpuidfield(
        EDX, 11, 8, doc="Number of C2 sub C-states supported using MWAIT")
    c3 = cpuidfield(
        EDX, 15, 12, doc="Number of C3 sub C-states supported using MWAIT")
    c4 = cpuidfield(
        EDX, 19, 16, doc="Number of C4 sub C-states supported using MWAIT")
...
class LEAF_6(CPUID):
    ...
    pln_supported = cpuidfield(
        EAX, 4, 4, doc="Power limit notification controls are supported if set"
    )
    ecmd_supported = cpuidfield(
        EAX, 5, 5, doc="Clock modulation duty cycle extension is supported if set"
    )
    ...
    num_interrupt_thresholds = cpuidfield(
        EBX, 3, 0, doc="Number of interrupt thresholds in digital thermal sensor"
    )
...
class LEAF_7(CPUID):
    ...
    sgx = cpuidfield(
        ...
        2,
        2,
        doc="Supports Intel® Software Guard Extensions (Intel® SGX Extensions) if 1",
    )
    bmi1 = cpuidfield(EBX, 3, 3)
    hle = cpuidfield(EBX, 4, 4)
    avx2 = cpuidfield(EBX, 5, 5)
    fdp_excptn_only = cpuidfield(
        EBX, 6, 6, doc="x87 FPU Data Pointer updated only on x87 exceptions if 1"
    )
    smep = cpuidfield(
        EBX, 7, 7, doc="Supports Supervisor Mode Execution Protection if 1"
    )
    ...
    invpcid = cpuidfield(
        ...
        10,
        10,
        doc="Supports INVPCID instruction for system software that manages process-context identifiers if 1",
    )
    rtm = cpuidfield(EBX, 11, 11)
    qm = cpuidfield(
        EBX, 12, 12, doc="Supports Quality of Service Monitoring capability if 1"
    )
    deprecate_fpu = cpuidfield(
        EBX, 13, 13, doc="Deprecates FPS CS and FPU DS values if 1"
    )
    mpx = cpuidfield(
        EBX, 14, 14, doc="Supports Intel® Memory Protection Extensions if 1"
    )
    rdt_a = cpuidfield(
        EBX,
        15,
        15,
        doc="Supports Intel® Resource Director Technology (Intel® RDT) Allocation capability if 1",
    )
    ...
    smap = cpuidfield(
        ...
        20,
        20,
        doc="Supports Supervisor-Mode Access Prevention (and the CLAC/STAC instructions) if 1",
    )
    ...
    sha = cpuidfield(
        ...
        29,
        29,
        doc="Supports Intel® Secure Hash Algorithm Extensions (Intel® SHA Extensions) if 1",
    )
    ...
    umip = cpuidfield(
        ECX, 2, 2, doc="Supports user-mode instruction prevention if 1")
    pku = cpuidfield(
        ECX, 3, 3, doc="Supports protection keys for user-mode pages if 1")
    ospke = cpuidfield(
        ECX,
        4,
        4,
        doc="If 1, OS has set CR4.PKE to enable protection keys (and the RDPKRU/WRPKRU instructions)",
    )
    waitpkg = cpuidfield(ECX, 5, 5)
    avx512_vbmi2 = cpuidfield(ECX, 6, 6)
    cet_ss = cpuidfield(
        ECX, 7, 7, doc="Supports CET shadow stack features if 1")
    ...
    la57 = cpuidfield(
        ECX, 16, 16, doc="Supports 57-bit linear addresses and five-level paging if 1"
    )
    mawau = cpuidfield(
        ECX,
        21,
        17,
        doc="The value of MAWAU used by the BNDLDX and BNDSTX instructions in 64-bit mode",
    )
    rdpid = cpuidfield(
        ECX, 22, 22, doc="RDPID and IA32_TSC_AUX are available if 1")
    ...
    sgx_lc = cpuidfield(
        ECX, 30, 30, doc="Supports SGX Launch Configuration if 1")
    pks = cpuidfield(
        ECX, 31, 31, doc="Supports protection keys for supervisor-mode pages if 1"
    )
    ...
    ibrs_ibpb = cpuidfield(
        ...
        26,
        26,
        doc="Enumerates support for indirect branch restricted speculation (IBRS) and the indirect branch predictor barrier (IBPB)",
    )
    stibp = cpuidfield(
        EDX,
        27,
        27,
        doc="Enumerates support for single thread indirect branch predictors (STIBP)",
    )
    l1d_flush = cpuidfield(EDX, 28, 28, doc="Enumerates support for L1D_FLUSH")
    ia32_arch_capabilities = cpuidfield(
        EDX, 29, 29, doc="Enumerates support for the IA32_ARCH_CAPABILITIES MSR"
    )
    ia32_core_capabilities = cpuidfield(
        EDX, 30, 30, doc="Enumerates support for the IA32_CORE_CAPABILITIES MSR"
    )
    ssbd = cpuidfield(
        EDX,
        31,
...
class LEAF_9(CPUID):
    ...
    platform_dca_cap = cpuidfield(
        EAX, 31, 0, doc="Value of bits of IA32_PLATFORM_DCA_CAP MSR (address 1F8H)"
    )
...
class LEAF_A(CPUID):
    ...
    gp_performance_monitor_counters = cpuidfield(
        ...
        15,
        8,
        doc="Number of general-purpose performance monitoring counter per logical processor",
    )
    gp_performance_counter_width = cpuidfield(
        EAX, 23, 16, doc="Bit width of general-purpose, performance monitoring counter"
    )
    ebx_bit_vector_length = cpuidfield(
        EAX,
        31,
        24,
        doc="Length of EBX bit vector to enumerate architectural performance monitoring events",
    )

    core_cycle_event = cpuidfield(
        EBX, 0, 0, doc="Core cycle event not available if 1")
...
class LEAF_B(CPUID):
    ...
    num_bit_shift = cpuidfield(
        ...
        4,
        0,
        doc="Number of bits to shift right on x2APID ID to get a unique topology ID of the next level type",
    )
    ...
    x2apic_id = cpuidfield(
        EDX, 31, 0, doc="x2APIC ID of the current logical processor")
...
class LEAF_D(CPUID):
    ...
    valid_bits_xcr0_lower = cpuidfield(
        ...
        31,
        0,
        doc="Reports the valid bit fields of the lower 32 bits of XCR0. If a bit is 0, the corresponding bit field in XCR0 is reserved",
    )
    ...
    max_size_enabled_xcr0 = cpuidfield(
        ...
        31,
        0,
        doc="Maximum size (bytes, from the beginning of the XSAVE/XRSTOR save area) required by enabled features in XCR0. May be different than ECX if some features at the end of the XSAVE save area are not enabled.",
    )
    ...
    max_size_supported_xcr0 = cpuidfield(
        ...
        31,
        0,
        doc="Maximum size (bytes, from the beginning of the XSAVE/XRSTOR save area) of the XSAVE/XRSTOR save area required by all supported features in the processor, i.e all the valid bit fields in XCR0.",
    )
    ...
    valid_bits_xcr0_upper = cpuidfield(
        ...
        31,
        0,
        doc="The valid bit fields of the upper 32 bits of XCR0. If a bit is 0, the corresponding bit field in XCR0 is reserved.",
    )
...
class LEAF_D_n(CPUID):
    ...
    size = cpuidfield(
        ...
        31,
        0,
        doc="The size in bytes (from the offset specified in EBX) of the save area for an extended state feature associated with a valid sub-leaf index",
    )
    ...
    offset = cpuidfield(
        ...
        31,
        0,
        doc="The offset in bytes of this extended state component's save area from the beginning of the XSAVE/XRSTOR area.",
    )
...
class LEAF_F(CPUID):
    ...
    max_range_rmid_z = cpuidfield(
        ...
        31,
        0,
        doc="Maximum range (zero-based) of RMID within this physical processor of all types.",
    )
...
class LEAF_F_1(CPUID):
    ...
    qm_ctr_conversion_factor = cpuidfield(
        ...
        31,
        0,
        doc="Conversion factor from reported IA32_QM_CTR value to occupancy metric (bytes).",
    )
    ...
    max_range_rmid_z = cpuidfield(
        ECX, 31, 0, doc="Maximum range (zero-based) of RMID of this resource type"
    )
...
class LEAF_10_1(CPUID):
    ...
    capacity_mask_length_z = cpuidfield(
        ...
        4,
        0,
        doc="Length of the capacity bit mask for the corresponding ResID (zero based).",
    )
    isolation_map = cpuidfield(
        EBX, 31, 0, doc="Bit-granular map of isolation/contention of allocation units."
    )
    code_and_data_prioritization = cpuidfield(
        ECX, 2, 2, doc="Code and Data Prioritization Technology supported if 1."
    )
...
class LEAF_10_3(CPUID):
    ...
    max_throttling_value_z = cpuidfield(
        ...
        11,
        0,
        doc="Reports the maximum MBA throttling value supported for the corresponding ResID (zero based).",
    )
    linear_response_delay = cpuidfield(
        ECX, 2, 2, doc="Reports whether the response of the delay values is linear."
    )
    clos_number_z = cpuidfield(
        EDX, 15, 0, doc="Highest COS number supported for this ResID."
    )
...
class LEAF_80000000(CPUID):
    ...
    max_extended_leaf = cpuidfield(
        EAX, 31, 0, doc="Highest extended function input value understood by CPUID"
    )
...
class LEAF_80000001(CPUID):
    ...
    lahf_sahf_64 = cpuidfield(
        ECX, 0, 0, doc="LAHF/SAHF available in 64-bit mode")
    ...
    execute_disable = cpuidfield(
        EDX, 20, 20, doc="Execute Disable Bit available")
    ...
    intel_64 = cpuidfield(
        EDX, 29, 29, doc="Intel(R) 64 Architecture available if 1")
...
class LEAF_80000002(CPUID):
    ...
    def brandstring(self):
        """Processor Brand String"""
        return struct.pack(
            "IIII", self.regs.eax, self.regs.ebx, self.regs.ecx, self.regs.edx
        ).rstrip(b"\x00")
...
class LEAF_80000008(CPUID):
    ...
    leaf = 0x80000008
    physical_address_bits = cpuidfield(
        EAX, 7, 0, doc="# Physical Address bits")
