...
def write_shmem_regions(config):
    ...
    for raw_shm in raw_shmem_regions:
        ...
        if (
            ...
            shmem_regions.append(
                (
                    raw_shm_splited[0].strip(),
                    raw_shm_splited[1].strip(),
    ...
    if len(shmem_regions) > 0:
        ...
        for shmem_region in shmem_regions:
            print("#define IVSHMEM_SHM_REGION_%d\t" %
                  shmem_cnt, end="", file=config)
    ...
    print("#define IVSHMEM_SHM_SIZE\t{}UL".format(
        hex(total_shm_size)), file=config)
    ...
    if len(shmem_regions) == 0:
        ...
        for shmem in shmem_regions:
            print("\t{ \\", file=config)
            print(
                "\t\t.name = IVSHMEM_SHM_REGION_{}, \\".format(shmem_cnt), file=config
            )
            try:
