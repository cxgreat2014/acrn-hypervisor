...
C_HEADER = (
    scenario_cfg_lib.HEADER_LICENSE
    + r"""
...
def vuart0_output(i, vm_type, vm_info, config):
    ...
    print("\t\t\t.type = {0},".format(
        vm_info.vuart.v0_vuart[i]["type"]), file=config)
    if vm_info.vuart.v0_vuart[i]["base"] == "INVALID_COM_BASE":
        ...
        if "SOS_" in vm_type:
            ...
            print(
                "\t\t\t.irq = {0},".format(vm_info.vuart.v0_vuart[i]["irq"]),
                file=config,
            )
    else:
        if "SOS_" in vm_type:
            ...
            print(
                "\t\t\t.addr.port_base = {0},".format(
                    vm_info.vuart.v0_vuart[i]["base"]
                ),
                file=config,
            )
            print(
...
def vuart_map_enable(vm_info):
    ...
    for i, vm_type in common.VM_TYPES.items():
        ...
        des_t_vuart_i = vm_info.vuart.v1_vuart[int(
            src_t_vm_i)]["target_uart_id"]

        if (
            int(des_t_vm_i) == i
            and int(des_t_vuart_i) == 1
            and des_t_vuart_i == src_t_vuart_i
        ):
            map_dic[i] = True
        else:
            map_dic[i] = False
...
def vuart1_output(i, vm_type, vuart1_vmid_dic, vm_info, config):
    ...
    print("\t\t\t.type = {0},".format(
        vm_info.vuart.v1_vuart[i]["type"]), file=config)
    if vm_info.vuart.v1_vuart[i]["base"] != "INVALID_COM_BASE" and vuart_enable[i]:
        print(
            "\t\t\t.addr.port_base = {0},".format(
                vm_info.vuart.v1_vuart[i]["base"]),
            file=config,
        )
    else:
        print("\t\t\t.addr.port_base = INVALID_COM_BASE,", file=config)
    ...
    if vuart1_vmid_dic and i in vuart1_vmid_dic.keys():
        if "SOS_VM" == scenario_cfg_lib.VM_DB[vm_type]["load_type"]:
            if (
                vm_info.vuart.v1_vuart[i]["base"] != "INVALID_COM_BASE"
                and vuart_enable[i]
            ):
        ...
        if vm_info.vuart.v1_vuart[i]["base"] != "INVALID_COM_BASE" and vuart_enable[i]:
            print(
                "\t\t\t.t_vuart.vm_id = {0}U,".format(
                    vm_info.vuart.v1_vuart[i]["target_vm_id"]
                ),
                file=config,
            )
            print(
                "\t\t\t.t_vuart.vuart_id = {0}U,".format(
                    vm_info.vuart.v1_vuart[i]["target_uart_id"]
                ),
                file=config,
            )
...
def gen_sos_vm(vm_type, vm_i, scenario_items, config):
    ...
    print("", file=config)
    print(
        "\t\t/* Allow SOS to reboot the host since "
        + "there is supposed to be the highest severity guest */",
        file=config,
    )
    if sos_guest_flags:
    ...
    print(
        "\t\t\t.start_hpa = {}UL,".format(
            vm_info.mem_info.mem_start_hpa[vm_i]),
        file=config,
    )
    print("\t\t},", file=config)
    print("\t\t.os_config = {", file=config)
    print('\t\t\t.name = "{0}",'.format(
        vm_info.os_cfg.kern_name[vm_i]), file=config)
    print(
        "\t\t\t.kernel_type = {0},".format(vm_info.os_cfg.kern_type[vm_i]), file=config
    )
    print(
        '\t\t\t.kernel_mod_tag = "{0}",'.format(vm_info.os_cfg.kern_mod[vm_i]),
        file=config,
    )
    print("\t\t\t.bootargs = {0},".format(
        vm_info.os_cfg.kern_args[vm_i]), file=config)
    if vm_info.os_cfg.ramdisk_mod[vm_i].strip():
        print(
            '\t\t\t.ramdisk_mod_tag = "{0}",'.format(
...
def gen_pre_launch_vm(vm_type, vm_i, scenario_items, config):
    ...
    clos_output(scenario_items, vm_i, config)
    print("\t\t.memory = {", file=config)
    print("\t\t\t.start_hpa = VM{0}_CONFIG_MEM_START_HPA,".format(
        vm_i), file=config)
    print("\t\t\t.size = VM{0}_CONFIG_MEM_SIZE,".format(vm_i), file=config)
    print("\t\t\t.start_hpa2 = VM{0}_CONFIG_MEM_START_HPA2,".format(
        vm_i), file=config)
    print("\t\t\t.size_hpa2 = VM{0}_CONFIG_MEM_SIZE_HPA2,".format(
        vm_i), file=config)
    print("\t\t},", file=config)
    ...
    if (
        vm_i in vm_info.os_cfg.kern_load_addr.keys()
        and vm_info.os_cfg.kern_entry_addr[vm_i]
    ):
        print(
            "\t\t\t.kernel_load_addr = {0},".format(
                vm_info.os_cfg.kern_load_addr[vm_i]
            ),
            file=config,
        )
    if (
        vm_i in vm_info.os_cfg.kern_entry_addr.keys()
        and vm_info.os_cfg.kern_entry_addr[vm_i]
    ):
        print(
            "\t\t\t.kernel_entry_addr = {0},".format(
...
def declare_pci_devs(vm_info, config):
    ...
    for vm_i, vm_type in common.VM_TYPES.items():
        if vm_type == "SOS_VM":
            print(
                "extern struct acrn_vm_pci_dev_config "
...
def generate_file(scenario_items, config):
    ...
    if (
        ...
        print(
            "extern struct pt_intx_config vm0_pt_intx[{}U];".format(
    ...
    print(
        "struct acrn_vm_config vm_configs[CONFIG_MAX_VM_NUM] = {", file=config)
    for vm_i, vm_type in common.VM_TYPES.items():
