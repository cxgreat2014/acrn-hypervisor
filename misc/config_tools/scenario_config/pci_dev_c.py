...
class BusDevFunc(namedtuple("BusDevFunc", ["bus", "dev", "func"])):

    PATTERN = re.compile(
        r"(?P<bus>[0-9a-f]{2}):(?P<dev>[0-9a-f]{2})\.(?P<func>[0-7]{1})"
    )
    ...
    def __init__(self, *args, **kwargs):
        if not (0x00 <= self.bus <= 0xFF):
            raise ValueError(
                "Invalid bus number (0x00 ~ 0xff): {:#04x}".format(self.bus)
            )
        if not (0x00 <= self.dev <= 0x1F):
            raise ValueError(
                "Invalid device number (0x00 ~ 0x1f): {:#04x}".format(self.dev)
            )
        if not (0x0 <= self.func <= 0x7):
            raise ValueError(
                "Invalid function number (0 ~ 7): {:#x}".format(self.func))
...
def generate_file(vm_info, config):
    ...
    for vm_i, vm_type in common.VM_TYPES.items():
        ...
        if (
            not scenario_cfg_lib.get_pci_dev_num_per_vm()[vm_i]
            and scenario_cfg_lib.VM_DB[vm_type]["load_type"] != "SOS_VM"
        ):
            continue
        if (
            not scenario_cfg_lib.get_pci_dev_num_per_vm()[vm_i]
        ...
        if scenario_cfg_lib.VM_DB[vm_type]["load_type"] == "PRE_LAUNCHED_VM":
            ...
            if (
                vm_info.shmem.shmem_enabled == "y"
                and vm_i in vm_info.shmem.shmem_regions
            ...
            if pciHostbridge:
                print("\t{", file=config)
                print("\t\t.emu_type = {},".format(
                    PCI_DEV_TYPE[0]), file=config)
                print(
        ...
        if vm_i in vm_info.cfg_pci.pci_devs.keys():
            ...
            if pci_bdf_devs_list:
                for pci_bdf_dev in pci_bdf_devs_list:
                    ...
                    print("\t{", file=config)
                    print("\t\t.emu_type = {},".format(
                        PCI_DEV_TYPE[1]), file=config)
                    print(
                        "\t\t.vbdf.bits = {{.b = 0x00U, .d = 0x{0:02x}U, .f = 0x00U}},".format(
                            pci_cnt
                        ),
                        file=config,
                    )
                    for (
                        bdf,
                        bar_attr,
                    ) in board_cfg_lib.PCI_DEV_BAR_DESC.pci_dev_dic.items():
                        if bdf == pci_bdf_dev:
                            print(
                                "\t\tPTDEV({}),".format(
                                    board_cfg_lib.PCI_DEV_BAR_DESC.pci_dev_dic[
                                        bdf
                                    ].name_w_i_cnt
                                ),
                                file=config,
        ...
        if (
            ...
            for shm in raw_shm_list:
                ...
                if scenario_cfg_lib.VM_DB[vm_type]["load_type"] == "SOS_VM":
                    free_bdf = find_unused_bdf(
                        sos_used_bdf, "ivshmem", last_bdf)
                    last_bdf = free_bdf
                    print(
                        "\t\t.vbdf.bits = {{.b = 0x00U, .d = 0x{:02x}U, .f = 0x{:02x}U}},".format(
                            free_bdf.dev, free_bdf.func
                        ),
                        file=config,
                    )
                ...
                for shm_name, _ in board_cfg_lib.PCI_DEV_BAR_DESC.shm_bar_dic.items():
                    ...
                    if shm_name == shm_splited[0].strip():
                        if (
                            ...
                            print(
                                "\t\t.shm_region_name = IVSHMEM_SHM_REGION_{},".format(
                                    region
                                ),
                                file=config,
                            )
                            print(
                                "\t\tIVSHMEM_DEVICE_{}_VBAR,".format(index), file=config
                            )
                            ...
                            print(
                                "\t\tSOS_IVSHMEM_DEVICE_{}_VBAR,".format(
                                    index),
                                file=config,
                            )
                        else:
        ...
        if vm_i in vuarts.keys():
            ...
            for vuart_id in vuarts[vm_i].keys():
                ...
                if (
                    vuart_id == 0
                    and scenario_cfg_lib.VM_DB[vm_type]["load_type"]
                    == "POST_LAUNCHED_VM"
                ):
                    continue
                ...
                if (
                    vuart_id == 0
                    and vm_i in vuart0_setting
                    and vuart0_setting[vm_i]["base"] != "INVALID_COM_BASE"
                ):
                    continue
                # Skip pci vuart 1 if the legacy vuart 1 is enabled
                if (
                    vuart_id == 1
                    and vm_i in vuart1_setting
                    and vuart1_setting[vm_i]["base"] != "INVALID_COM_BASE"
                ):
                    continue
                ...
                if scenario_cfg_lib.VM_DB[vm_type]["load_type"] != "POST_LAUNCHED_VM":
                    print(
                        "\t\tVM{:1d}_VUART_{:1d}_VBAR,".format(vm_i, vuart_id),
                        file=config,
                    )
                    if (
                        scenario_cfg_lib.VM_DB[vm_type]["load_type"]
                        ...
                        free_bdf = find_unused_bdf(
                            vm_used_bdf, "vuart", last_bdf)
                        vm_used_bdf.append(free_bdf)
                    elif scenario_cfg_lib.VM_DB[vm_type]["load_type"] == "SOS_VM":
                        free_bdf = find_unused_bdf(
                            sos_used_bdf, "vuart", last_bdf)
                        sos_used_bdf.append(free_bdf)
                    print(
                        "\t\t.vbdf.bits = {{.b = 0x00U, .d = 0x{:02x}U, .f = 0x00U}},".format(
                            free_bdf.dev, free_bdf.func
                        ),
                        file=config,
                    )
                ...
                if vuart_id != 0:
                    print(
                        "\t\t.t_vuart.vm_id = {},".format(
                            vuarts[vm_i][vuart_id]["target_vm_id"]
                        ),
                        file=config,
                    )
                    print(
                        "\t\t.t_vuart.vuart_id = {},".format(
