...
def scenario_vm_num(scenario_items, config):
    ...
    print("#define PRE_VM_NUM\t\t\t{}U".format(
        load_type_cnt.pre_vm), file=config)
    print("#define SOS_VM_NUM\t\t\t{}U".format(
        load_type_cnt.sos_vm), file=config)
    print("#define MAX_POST_VM_NUM\t\t\t{}U".format(
        load_type_cnt.post_vm), file=config)
    print(
        "#define CONFIG_MAX_KATA_VM_NUM\t\t{}U".format(
            scenario_cfg_lib.KATA_VM_COUNT),
        file=config,
    )
...
def gen_pre_launch_vm(scenario_items, config):
    ...
    for vm_type in common.VM_TYPES.values():
        ...
        print(
            "#define VM{0}_CONFIG_MEM_START_HPA        {1}UL".format(
                vm_i, vm_info.mem_info.mem_start_hpa[vm_i]
            ),
            file=config,
        )
        print(
            "#define VM{0}_CONFIG_MEM_SIZE             {1}UL".format(
                vm_i, vm_info.mem_info.mem_size[vm_i]
            ),
            file=config,
        )
        if vm_info.mem_info.mem_start_hpa2[vm_i] not in (None, ""):
            print(
                "#define VM{0}_CONFIG_MEM_START_HPA2       {1}UL".format(
                    vm_i, vm_info.mem_info.mem_start_hpa2[vm_i]
                ),
                file=config,
            )
            print(
                "#define VM{0}_CONFIG_MEM_SIZE_HPA2        {1}UL".format(
                    vm_i, vm_info.mem_info.mem_size_hpa2[vm_i]
                ),
                file=config,
            )
...
def get_dm_owned_guest_flag_mask(vm_info, config):
    ...
    if "SOS_VM" not in common.VM_TYPES.values():
        ...
        print(
            "/* Bits mask of guest flags that can be programmed by device model."
