...
sys.path.append(
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "hv_config")
)
sys.path.append(
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "acpi_gen")
)
...
def get_scenario_item_values(board_info, scenario_info):
    ...
    scenario_item_values.update(
        scenario_cfg_lib.avl_vuart_ui_select(scenario_info))
    scenario_item_values["vm,console_vuart,base"] = [
        "INVALID_PCI_BASE", "PCI_VUART"]
    scenario_item_values["vm,communication_vuart,base"] = [
        "INVALID_PCI_BASE",
        "PCI_VUART",
    ]

    # board
    (scenario_item_values["vm,board_private,rootfs"], num) = board_cfg_lib.get_rootfs(
        board_info
    )

    scenario_item_values["hv,DEBUG_OPTIONS,RELEASE"] = hv_cfg_lib.N_Y
    scenario_item_values["hv,DEBUG_OPTIONS,NPK_LOGLEVEL"] = hv_cfg_lib.get_select_range(
        "DEBUG_OPTIONS", "LOG_LEVEL"
    )
    scenario_item_values["hv,DEBUG_OPTIONS,MEM_LOGLEVEL"] = hv_cfg_lib.get_select_range(
        "DEBUG_OPTIONS", "LOG_LEVEL"
    )
    scenario_item_values[
        "hv,DEBUG_OPTIONS,CONSOLE_LOGLEVEL"
    ] = hv_cfg_lib.get_select_range("DEBUG_OPTIONS", "LOG_LEVEL")
    scenario_item_values[
        "hv,DEBUG_OPTIONS,SERIAL_CONSOLE"
    ] = board_cfg_lib.get_native_ttys_info(board_info)
    scenario_item_values[
        "hv,DEBUG_OPTIONS,LOG_DESTINATION"
    ] = hv_cfg_lib.get_select_range("DEBUG_OPTIONS", "LOG_DESTINATION_BITMAP")

    scenario_item_values["hv,CAPACITIES,MAX_IOAPIC_NUM"] = hv_cfg_lib.get_select_range(
        "CAPACITIES", "IOAPIC_NUM"
    )

    scenario_item_values["hv,FEATURES,MULTIBOOT2"] = hv_cfg_lib.N_Y
    scenario_item_values[
        "hv,FEATURES,RDT,RDT_ENABLED"
    ] = board_cfg_lib.get_rdt_select_opt()
    scenario_item_values[
...
def validate_scenario_schema(scenario_info):
    ...
    xsd_doc.xinclude()
    my_schema = xmlschema.XMLSchema11(
        etree.tostring(xsd_doc, encoding="unicode"))
    ...
    for idx, validation_error in enumerate(it, start=1):
        ...
        if not validation_error:
            ...
            element = "'" + path[-1] + "' "
            reason = (
                validation_error.reason + ": last call: " +
                str(validation_error.obj)
            )
...
def apply_data_checks(board_info, scenario_info):
    ...
    xsd_doc.xinclude()
    datachecks_schema = xmlschema.XMLSchema11(
        etree.tostring(xsd_doc, encoding="unicode")
    )
...
def main(args):
    ...
    if common.VM_COUNT > common.MAX_VM_NUM:
        err_dic[
            "vm count"
        ] = "Number of VMs in scenario xml file should be no greater than {}!".format(
            common.MAX_VM_NUM
        )
        return err_dic
    ...
    if params["--out"]:
        if os.path.isabs(params["--out"]):
            ...
            scen_output = ACRN_PATH + \
                params["--out"] + "/scenarios/" + scenario + "/"
    else:
    ...
    (err_dic, scenario_items) = validate_scenario_setting(
        params["--board"], params["--scenario"]
    )
    if err_dic:
